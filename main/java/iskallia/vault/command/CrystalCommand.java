...
public class CrystalCommand extends Command {
    ...
    public void build(final LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(Commands.literal("preventRandomModifiers").then(Commands
                .argument("random", (ArgumentType) BoolArgumentType.bool()).executes(this::setRollsRandom)));
        ...
        builder.then(Commands.literal("setModifiable").then(Commands
                .argument("modifiable", (ArgumentType) BoolArgumentType.bool()).executes(this::setModifiable)));
        builder.then(Commands.literal("addModifier").then(Commands
        ...
        builder.then(Commands.literal("type")
                .then(Commands
                        .argument("crystalType",
                                (ArgumentType) EnumArgument.enumArgument((Class) CrystalData.Type.class))
    ...
    private int addRoom(final CommandContext<CommandSource> ctx) throws CommandSyntaxException {
        ...
        if (VaultRoomNames.getName(roomKey) == null) {
            ((CommandSource) ctx.getSource()).getPlayerOrException().sendMessage(
                    (ITextComponent) new StringTextComponent("Unknown Room: " + roomKey), Util.NIL_UUID);
            return 0;
        }
    ...
    private int addModifier(final CommandContext<CommandSource> ctx) throws CommandSyntaxException {
        ...
        if (modifier == null) {
            ((CommandSource) ctx.getSource()).getPlayerOrException().sendMessage(
                    (ITextComponent) new StringTextComponent("Unknown Modifier: " + modifierStr), Util.NIL_UUID);
            return 0;
        }
    ...
    private int setObjective(final CommandContext<CommandSource> ctx) throws CommandSyntaxException {
        ...
        if (objective == null) {
            ((CommandSource) ctx.getSource()).getPlayerOrException().sendMessage(
                    (ITextComponent) new StringTextComponent("Unknown Objective: " + objectiveStr),
                    Util.NIL_UUID);
            return 0;
        }
    ...
    private ItemStack getCrystal(final CommandContext<CommandSource> ctx) throws CommandSyntaxException {
        ...
        if (held.isEmpty() || !(held.getItem() instanceof VaultCrystalItem)) {
            player.sendMessage((ITextComponent) new StringTextComponent("Not holding crystal!"),
                    Util.NIL_UUID);
