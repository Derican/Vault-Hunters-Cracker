...
public class GiveLootCommand extends Command {
    ...
    public void build(final LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(Commands.literal("raffle_boss_crate")
                .then(Commands.argument("boss_name", (ArgumentType) StringArgumentType.word())
                        .executes(ctx -> this.giveRaffleBossCrate((CommandContext<CommandSource>) ctx,
                                ((CommandSource) ctx.getSource()).getPlayerOrException(),
                                StringArgumentType.getString(ctx, "boss_name")))));
        builder.then(Commands.literal("normal_boss_crate")
                .executes(ctx -> this.giveNormalBossCrate((CommandContext<CommandSource>) ctx,
                        ((CommandSource) ctx.getSource()).getPlayerOrException())));
        builder.then(Commands.literal("raid_reward_crate")
                .executes(ctx -> this.giveRaidRewardCrate((CommandContext<CommandSource>) ctx,
                        ((CommandSource) ctx.getSource()).getPlayerOrException())));
        ...
        builder.then(Commands.literal("challenge_crystal")
                .then(Commands.argument("index", (ArgumentType) IntegerArgumentType.integer())
                        .executes(this::giveChallengeCrystal)));
    }
    ...
    public int giveTrophyBox(final CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        final LootContext.Builder builder = new LootContext.Builder(sWorld).withRandom(sWorld.random)
                .withLuck(sPlayer.getLuck());
        final int playerLevel = PlayerVaultStatsData.get(sWorld).getVaultStats(sPlayer.getUUID())
                .getVaultLevel();
        final LootTablesConfig.Level config = ModConfigs.LOOT_TABLES.getForLevel(playerLevel);
        final LootTable bossBonusTbl = sPlayer.getServer().getLootTables()
                .get(config.getScavengerCrate());
        final NonNullList<ItemStack> quickBossLoot = (NonNullList<ItemStack>) NonNullList.create();
        quickBossLoot.addAll(
        ...
        sPlayer.addItem(box);
        sPlayer.sendMessage(
                (ITextComponent) new StringTextComponent("Generated Recordbox for Vault level " + playerLevel),
                Util.NIL_UUID);
        return 0;
    }
    ...
    public int giveNormalBossCrate(final CommandContext<CommandSource> context, final ServerPlayerEntity player) {
        ...
        stacks.addAll((Collection) world.getServer().getLootTables()
                .get(ModConfigs.LOOT_TABLES.getForLevel(level).getBossCrate()).getRandomItems(ctx));
    ...
    public int giveRaffleBossCrate(final CommandContext<CommandSource> context, final ServerPlayerEntity player,
        ...
        final int level = PlayerVaultStatsData.get(world).getVaultStats((PlayerEntity) player).getVaultLevel();
        final List<ItemStack> items = world.getServer().getLootTables()
                .get(ModConfigs.LOOT_TABLES.getForLevel(level).getBossCrate()).getRandomItems(ctx);
