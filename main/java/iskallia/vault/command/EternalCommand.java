...
public class EternalCommand extends Command {
    ...
    public void build(final LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(Commands.literal("remove").then(Commands
                .argument("uuid", (ArgumentType) UUIDArgument.uuid()).executes(this::removeEternal)));
        builder.then(Commands.literal("list").then(Commands
                .argument("playerId", (ArgumentType) UUIDArgument.uuid()).executes(this::listEternals)));
        builder.then(Commands.literal("set").then(Commands
                .argument("uuid", (ArgumentType) UUIDArgument.uuid()).executes(this::setEternal)));
    }
    ...
    private int setEternal(final CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (held.isEmpty() || !(held.getItem() instanceof BlockItem)
                || !(((BlockItem) held.getItem()).getBlock() instanceof CryoChamberBlock)) {
            sPlayer.sendMessage((ITextComponent) new StringTextComponent("Not holding cryochamber!")
        ...
        if (eternal == null) {
            sPlayer.sendMessage((ITextComponent) new StringTextComponent("Specified eternal does not exist!")
                    .withStyle(TextFormatting.RED), Util.NIL_UUID);
            return 0;
        }
        ...
        sPlayer.sendMessage(
                (ITextComponent) new StringTextComponent("Eternal set!").withStyle(TextFormatting.GREEN),
                Util.NIL_UUID);
        return 0;
    }
    ...
    private int listEternals(final CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        sPlayer.sendMessage(
                (ITextComponent) new StringTextComponent("Eternals:").withStyle(TextFormatting.GREEN),
                Util.NIL_UUID);
        for (final EternalData eternal : group.getEternals()) {
            final IFormattableTextComponent txt = (IFormattableTextComponent) new StringTextComponent(
                    eternal.getId().toString() + " / " + eternal.getName());
            txt.withStyle(style -> {
            ...
            txt.withStyle(style -> style
                    .withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, eternal.getId().toString())));
    ...
    private int removeEternal(final CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (data.removeEternal(eternalUUID)) {
            sPlayer.sendMessage(
                    (ITextComponent) new StringTextComponent("Eternal removed!").withStyle(TextFormatting.GREEN),
                    Util.NIL_UUID);
            return 0;
        }
        sPlayer.sendMessage((ITextComponent) new StringTextComponent("Specified eternal does not exist!")
