...
public class VaultCrateContainer extends Container {
    ...
    public VaultCrateContainer(final int windowId, final World world, final BlockPos pos,
            ...
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                ...
                for (int l = 0; l < 3; ++l) {
                    for (int j2 = 0; j2 < 9; ++j2) {
                        this.addSlot(
                                new Slot((IInventory) playerInventory, j2 + l * 9 + 9, 8 + j2 * 18, 103 + l * 18 + i));
                    }
                }
    ...
    private int addSlotBox(final IItemHandler handler, int index, final int x, int y, final int horAmount, final int dx,
            final int verAmount, final int dy) {
    ...
    private int addSlotRange(final IItemHandler handler, int index, int x, final int y, final int amount,
            final int dx) {
    ...
    public void removed(final PlayerEntity player) {
        ...
        player.level.playSound((PlayerEntity) null, player.getX(), player.getY(),
                player.getZ(), SoundEvents.BARREL_CLOSE, SoundCategory.BLOCKS, 1.0f, 1.0f);
    }
}
