                ...
                if (clientStackChanged) {
                    for (final IContainerListener icontainerlistener : this.containerListeners) {
                        if (icontainerlistener instanceof ServerPlayerEntity && i == 36) {
                            ...
                            ModNetwork.CHANNEL.sendTo((Object)new SyncOversizedStackMessage(this.containerId, i, itemstack2), playerEntity.connection.getConnection(), NetworkDirection.PLAY_TO_CLIENT);
                        }
                        else {
        ...
        if (!this.containerListeners.contains(listener)) {
            ...
            if (listener instanceof ServerPlayerEntity) {
                ...
                for (int i = 0; i < this.slots.size(); ++i) {
                    ...
                    ServerScheduler.INSTANCE.schedule(0, () -> ModNetwork.CHANNEL.sendTo((Object)new SyncOversizedStackMessage(this.containerId, slotIndex, stack), player.connection.getConnection(), NetworkDirection.PLAY_TO_CLIENT));
                }
