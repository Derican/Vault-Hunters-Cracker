...
public class TransmogTableInventory extends RecipeInventory {
    ...
    public boolean recipeFulfilled() {
        ...
        if (gearStack.getItem() instanceof VaultArmorItem
                && appearanceStack.getItem() instanceof VaultArmorItem) {
            return this.armorRecipeFulfilled(gearStack, appearanceStack, bronzeStack);
        }
        return gearStack.getItem() instanceof VaultSwordItem
                && appearanceStack.getItem() instanceof VaultSwordItem
                && this.swordRecipeFulfilled(gearStack, appearanceStack, bronzeStack);
    }

    private boolean armorRecipeFulfilled(final ItemStack armorStack, final ItemStack appearanceStack,
            final ItemStack bronzeStack) {
        final VaultGear.Rarity armorRarity = ModAttributes.GEAR_RARITY.getBase(armorStack)
                .orElse(VaultGear.Rarity.SCRAPPY);
        final VaultGear.Rarity appearanceRarity = ModAttributes.GEAR_RARITY.getBase(appearanceStack)
                .orElse(VaultGear.Rarity.SCRAPPY);
        ...
        if (appearanceSpecialModel != -1) {
            final ModModels.SpecialGearModel specialGearModel = ModModels.SpecialGearModel.getModel(appearanceSlot,
                    appearanceSpecialModel);
            if (specialGearModel != null && !specialGearModel.getModelProperties().doesAllowTransmogrification()) {
                return false;
            }
        }
        return bronzeStack.getItem() == ModItems.VAULT_BRONZE
                && bronzeStack.getCount() >= this.requiredVaultBronze();
    }

    private boolean swordRecipeFulfilled(final ItemStack swordStack, final ItemStack appearanceStack,
            final ItemStack bronzeStack) {
        final VaultGear.Rarity swordRarity = ModAttributes.GEAR_RARITY.getBase(swordStack)
                .orElse(VaultGear.Rarity.SCRAPPY);
        final VaultGear.Rarity appearanceRarity = ModAttributes.GEAR_RARITY.getBase(appearanceStack)
                .orElse(VaultGear.Rarity.SCRAPPY);
        ...
        final int appearanceSpecialModel = ModAttributes.GEAR_SPECIAL_MODEL.getBase(appearanceStack).orElse(-1);
        if (appearanceSpecialModel != -1) {
            final ModModels.SpecialSwordModel specialSwordModel = ModModels.SpecialSwordModel
                    .getModel(appearanceSpecialModel);
