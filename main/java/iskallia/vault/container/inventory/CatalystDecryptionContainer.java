...
public class CatalystDecryptionContainer extends Container {
    ...
    public CatalystDecryptionContainer(final int windowId, final World world, final BlockPos pos,
        ...
        if (te instanceof CatalystDecryptionTableTileEntity) {
            te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY)
                    .ifPresent(tableInventory -> this.initSlots(tableInventory, playerInventory));
        }
    }
    ...
    private void initSlots(final IItemHandler tableInventory, final PlayerInventory playerInventory) {
        for (int row = 0; row < 3; ++row) {
            for (int column = 0; column < 9; ++column) {
                this.addSlot(
                        new Slot((IInventory) playerInventory, column + row * 9 + 9, 8 + column * 18, 152 + row * 18));
            }
        }
        for (int hotbarSlot = 0; hotbarSlot < 9; ++hotbarSlot) {
            this.addSlot(new Slot((IInventory) playerInventory, hotbarSlot, 8 + hotbarSlot * 18, 210));
        }
        final Predicate<ItemStack> catalystFilter = stack -> stack.getItem() instanceof VaultCatalystItem
                || stack.getItem() instanceof VaultInhibitorItem;
        final Predicate<ItemStack> crystalFilter = stack -> stack.getItem() instanceof VaultCrystalItem;
        for (int slotY = 0; slotY < 5; ++slotY) {
            this.addCatalystSlot(
                    (Slot) new FilteredSlot(tableInventory, slotY * 2, 56, 15 + slotY * 26, catalystFilter));
            this.addCatalystSlot(
                    (Slot) new FilteredSlot(tableInventory, slotY * 2 + 1, 104, 15 + slotY * 26, catalystFilter));
        }
    ...
    public boolean stillValid(final PlayerEntity player) {
        ...
        return world.getBlockEntity(this.tilePos) instanceof CatalystDecryptionTableTileEntity
                && player.distanceToSqr(this.tilePos.getX() + 0.5, this.tilePos.getY() + 0.5,
                        this.tilePos.getZ() + 0.5) <= 64.0;
    }
