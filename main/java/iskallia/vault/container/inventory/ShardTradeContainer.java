...
public class ShardTradeContainer extends AbstractPlayerSensitiveContainer {
    ...
    protected boolean mergeItemStack(final Slot fromSlot, final PlayerEntity player, final ItemStack stack,
    ...
    public static class ShardSellSlot extends SellSlot implements PlayerSensitiveSlot {
        ...
        public boolean mayPickup(final PlayerEntity player) {
            ...
            if (player instanceof ServerPlayerEntity) {
                final SoulShardTraderData tradeData = SoulShardTraderData
                        .get(((ServerPlayerEntity) player).getLevel());
        ...
        @Override
        public ItemStack modifyTakenStack(final PlayerEntity player, final ItemStack taken, final LogicalSide side,
            ...
            if (ItemShardPouch.reduceShardAmount(player.inventory, shardCost, simulate)) {
                if (side.isServer() && !simulate) {
                    if (player instanceof ServerPlayerEntity) {
                        ...
                        final SoulShardTraderData.SelectedTrade trade = tradeData.getTrades()
                                .get(this.getSlotIndex() - 1);
                        if (trade != null && trade.isInfinite()) {
                            player.containerMenu.lastSlots.set(this.index,
