...
public class EtchingBuySlot extends SlotItemHandler {
    ...
    public EtchingBuySlot(final EtchingTradeContainer etchingTradeContainer, final IItemHandler itemHandler,
            final int tradeId, final int index, final int xPosition, final int yPosition) {
    ...
    public ItemStack onTake(final PlayerEntity player, final ItemStack stack) {
        ...
        vendor.playSound(SoundEvents.VILLAGER_CELEBRATE, 1.0f,
                (vendor.level.random.nextFloat() - vendor.level.random.nextFloat()) * 0.2f
                        + 1.0f);
        return stack;
    }
}
