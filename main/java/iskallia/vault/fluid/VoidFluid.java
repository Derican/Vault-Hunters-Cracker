...
public abstract class VoidFluid extends FlowingFluid {
    ...
    protected void beforeDestroyingBlock(@Nonnull final IWorld world, @Nonnull final BlockPos pos,
    ...
    protected boolean canBeReplacedWith(final FluidState fluidState, @Nonnull final IBlockReader blockReader,
            @Nonnull final BlockPos pos, @Nonnull final Fluid fluid, @Nonnull final Direction direction) {
    ...
    public int getSpreadDelay(@Nonnull final World world, final BlockPos pos, final FluidState p_215667_3_,
            final FluidState p_215667_4_) {
        int i = this.getTickDelay((IWorldReader) world);
        if (!p_215667_3_.isEmpty() && !p_215667_4_.isEmpty()
                && !(boolean) p_215667_3_.getValue((Property) VoidFluid.FALLING)
                && !(boolean) p_215667_4_.getValue((Property) VoidFluid.FALLING) && p_215667_4_
                        .getHeight((IBlockReader) world, pos) > p_215667_3_.getHeight((IBlockReader) world, pos)
    ...
    protected BlockState createLegacyBlock(@Nonnull final FluidState state) {
        return (BlockState) ModBlocks.VOID_LIQUID_BLOCK.defaultBlockState()
                .setValue((Property) FlowingFluidBlock.LEVEL, (Comparable) getLegacyLevel(state));
    }

    @Nonnull
    protected FluidAttributes createAttributes() {
        return FluidAttributes.Water
                .builder(Vault.id("block/fluid/void_liquid"), Vault.id("block/fluid/flowing_void_liquid"))
                .overlay(new ResourceLocation("block/water_overlay")).translationKey("block.the_vault.void_liquid")
                .density(3000).viscosity(6000).temperature(1300).color(16777215)
                .sound(SoundEvents.BUCKET_FILL, SoundEvents.BUCKET_EMPTY).build((Fluid) this);
    }
