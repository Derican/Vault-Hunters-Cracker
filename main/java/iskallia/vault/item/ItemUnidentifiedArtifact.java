...
public class ItemUnidentifiedArtifact extends Item {
    ...
    public ActionResult<ItemStack> use(final World world, final PlayerEntity player, final Hand hand) {
        if (!world.isClientSide) {
            ...
            ((ServerWorld) world).playSound((PlayerEntity) null, position.x, position.y,
                    position.z, ModSounds.BOOSTER_PACK_SUCCESS_SFX, SoundCategory.PLAYERS, 1.0f, 1.0f);
            ((ServerWorld) world).sendParticles((IParticleData) ParticleTypes.DRAGON_BREATH, position.x,
                    position.y, position.z, 500, 1.0, 1.0, 1.0, 0.5);
