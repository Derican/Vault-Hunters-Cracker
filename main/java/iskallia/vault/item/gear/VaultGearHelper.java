...
public class VaultGearHelper
{
    ...
    private static boolean removeAttribute(final VAttribute<?, ?> attribute, final ListNBT attributes) {
        return attributes.stream().map(nbt -> (CompoundNBT)nbt).filter(compoundNBT -> attribute.getId().equals((Object)new ResourceLocation(compoundNBT.getString("Id")))).findFirst().map((Function<? super Object, ? extends Boolean>)attributes::remove).orElse(false);
    }
    ...
    public static boolean hasModifier(final ItemStack stack, final VAttribute<?, ?> attribute) {
        return !stack.isEmpty() && stack.getItem() instanceof VaultGear && attribute.exists(stack) && (attribute != ModAttributes.EXTRA_EFFECTS || !ModAttributes.EXTRA_EFFECTS.getValue(stack).map((Function<? super List<EffectTalent>, ? extends Boolean>)List::isEmpty).orElse(false));
    }
    ...
    public static List<VAttribute<?, ?>> getModifiers(final ItemStack stack) {
        ...
        return getRollableGearModifiers().stream().filter(modifier -> hasModifier(stack, modifier)).collect((Collector<? super Object, ?, List<VAttribute<?, ?>>>)Collectors.toList());
    }
