...
public interface VaultGear<T extends Item & VaultGear<? extends Item>> extends IForgeItem
{
        ...
        ModAttributes.GEAR_RARITY.get(stack).map(attribute -> attribute.getValue(stack)).ifPresent(rarity -> {
            ...
            if (displayDetails) {
                ModAttributes.GEAR_MODEL.get(stack).map(attribute -> attribute.getValue(stack)).ifPresent(model -> {
                    ...
                    final Object o;
                    ((IFormattableTextComponent)o).append((ITextComponent)stringTextComponent.withStyle(TextFormatting.GRAY));
                    return;
                });
            }
    ...
    default void addModifierInformation(final ItemStack stack, final List<ITextComponent> tooltip, final boolean displayDetails) {
        ...
        ModAttributes.EXTRA_LEECH_RATIO.get(stack).map((Function<? super FloatAttribute, ?>)VAttribute.Instance::getBaseValue).ifPresent(value -> {
        ...
        ModAttributes.EXTRA_HEALTH.get(stack).map((Function<? super FloatAttribute, ?>)VAttribute.Instance::getBaseValue).ifPresent(value -> {
        ...
        ModAttributes.EXTRA_PARRY_CHANCE.get(stack).map((Function<? super FloatAttribute, ?>)VAttribute.Instance::getBaseValue).ifPresent(value -> {
        ...
        ModAttributes.EXTRA_RESISTANCE.get(stack).map((Function<? super FloatAttribute, ?>)VAttribute.Instance::getBaseValue).ifPresent(value -> {
        ...
        ModAttributes.EFFECT_IMMUNITY.get(stack).map(attribute -> ((VAttribute.Instance<List<?>>)attribute).getValue(stack)).ifPresent(value -> value.forEach(effect -> tooltip.add(new StringTextComponent("+").append((ITextComponent)new TranslationTextComponent(effect.toEffect().getDescriptionId())).append((ITextComponent)new StringTextComponent(" Immunity")).setStyle(Style.EMPTY.withColor(Color.fromRgb(10801083))))));
        ModAttributes.EFFECT_CLOUD.get(stack).map(attribute -> ((VAttribute.Instance<List<?>>)attribute).getValue(stack)).ifPresent(value -> value.forEach(effect -> {
        ...
        ModAttributes.EXTRA_EFFECTS.get(stack).map(attribute -> ((VAttribute.Instance<List<?>>)attribute).getValue(stack)).ifPresent(value -> value.forEach(effect -> {
    ...
    default Multimap<Attribute, AttributeModifier> getAttributeModifiers(final T item, final EquipmentSlotType slot, final ItemStack stack, final Multimap<Attribute, AttributeModifier> parent) {
        ...
        parent.forEach((attribute, modifier) -> {
            if (attribute == Attributes.ATTACK_DAMAGE && attackDamage.isPresent()) {
                builder.put((Object)Attributes.ATTACK_DAMAGE, (Object)new AttributeModifier(modifier.getId(), "Weapon modifier", (double)attackDamage.get().getValue(stack), AttributeModifier.Operation.ADDITION));
            }
            else if (attribute == Attributes.ATTACK_SPEED && attackSpeed.isPresent()) {
                builder.put((Object)Attributes.ATTACK_SPEED, (Object)new AttributeModifier(modifier.getId(), "Weapon modifier", (double)attackSpeed.get().getValue(stack), AttributeModifier.Operation.ADDITION));
            }
            else if (attribute == Attributes.ARMOR && armor.isPresent()) {
                builder.put((Object)Attributes.ARMOR, (Object)new AttributeModifier(VaultGear.ARMOR_MODIFIERS[slot.getIndex()], "Armor modifier", (double)armor.get().getValue(stack), AttributeModifier.Operation.ADDITION));
            }
            else if (attribute == Attributes.ARMOR_TOUGHNESS && armorToughness.isPresent()) {
                builder.put((Object)Attributes.ARMOR_TOUGHNESS, (Object)new AttributeModifier(VaultGear.ARMOR_MODIFIERS[slot.getIndex()], "Armor toughness", (double)armorToughness.get().getValue(stack), AttributeModifier.Operation.ADDITION));
            }
            else if (attribute == Attributes.KNOCKBACK_RESISTANCE && knockbackResistance.isPresent()) {
                builder.put((Object)Attributes.KNOCKBACK_RESISTANCE, (Object)new AttributeModifier(VaultGear.ARMOR_MODIFIERS[slot.getIndex()], "Armor knockback resistance", (double)knockbackResistance.get().getValue(stack), AttributeModifier.Operation.ADDITION));
            }
            else {
        ...
        if (health != 0.0f) {
            builder.put((Object)Attributes.MAX_HEALTH, (Object)new AttributeModifier(itemHash(item, 0L), "Extra Health", (double)health, AttributeModifier.Operation.ADDITION));
        }
        reach.ifPresent(attribute -> builder.put((Object)ForgeMod.REACH_DISTANCE.get(), (Object)new AttributeModifier(itemHash(item, 1L), "Reach", (double)attribute.getValue(stack), AttributeModifier.Operation.ADDITION)));
        return (Multimap<Attribute, AttributeModifier>)builder.build();
    }
    
    default int getCooldownReduction(final ServerPlayerEntity player, final AbilityGroup<?, ?> abilityGroup, int cooldown) {
