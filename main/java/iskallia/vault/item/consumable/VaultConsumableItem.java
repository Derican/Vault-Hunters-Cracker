...
public class VaultConsumableItem extends Item {
    ...
    public VaultConsumableItem(final ResourceLocation id) {
        super(new Item.Properties().tab(ModItems.VAULT_MOD_GROUP)
                .food(VaultConsumableItem.VAULT_FOOD).stacksTo(64));
    ...
    public ItemStack finishUsingItem(final ItemStack stack, final World world, final LivingEntity entityLiving) {
        if (entityLiving instanceof ServerPlayerEntity) {
            ...
            if (entry.shouldAddAbsorption()) {
                final TalentTree talents = PlayerTalentsData.get(sPlayer.getLevel())
                        .getTalents((PlayerEntity) sPlayer);
                if (talents.hasLearnedNode(ModConfigs.TALENTS.WARD)
                        || talents.hasLearnedNode(ModConfigs.TALENTS.BARBARIC)) {
                    return stack;
                }
    ...
    private void addEffect(final PlayerEntity player, final EffectInstance effectInstance,
            final ConsumableEffect effect) {
        player.removeEffect(effectInstance.getEffect());
        final EffectInstance newEffect = new EffectInstance(effectInstance.getEffect(), effect.getDuration(),
                effectInstance.getAmplifier() + effect.getAmplifier(), effectInstance.isAmbient(),
    ...
    private void applyPowerup(final PlayerEntity player, final ConsumableEffect effect) {
        final EffectInstance powerup = new EffectInstance(ModEffects.VAULT_POWERUP, effect.getDuration(), 0,
    ...
    private void applyEffect(final PlayerEntity player, final Effect effect, final ConsumableEffect setting) {
        final EffectInstance newEffect = new EffectInstance(effect, setting.getDuration(), setting.getAmplifier() - 1,
                setting.isAmbient(), setting.shouldShowParticles(), setting.shouldShowIcon());
        player.addEffect(newEffect);
    }

    private void addEffectDuration(final PlayerEntity player, final EffectInstance effectInstance,
            final int newDuration) {
        player.removeEffect(effectInstance.getEffect());
        final EffectInstance newEffect = new EffectInstance(effectInstance.getEffect(), newDuration,
    ...
    static {
        VaultConsumableItem.VAULT_FOOD = new Food.Builder().saturationMod(0.0f).nutrition(0).fast()
                .alwaysEat().build();
    }
}
