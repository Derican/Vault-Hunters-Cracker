...
public class VaultCrystalItem extends Item {
    ...
    public ITextComponent getName(final ItemStack stack) {
        ...
        if (data.getEchoData().getEchoCount() > 0) {
            return (ITextComponent) new StringTextComponent("Echoed Vault Crystal")
    ...
    public ActionResultType useOn(final ItemUseContext context) {
        ...
        if (this.tryCreatePortal(context.getLevel(), pos, context.getClickedFace(), data)) {
            ...
            final String suffix = (data.getType() == CrystalData.Type.FINAL_LOBBY) ? " opened the Final Vault!"
                    : " opened a Vault!";
            ...
            context.getLevel().getServer().getPlayerList().broadcastMessage(
                    (ITextComponent) new StringTextComponent("").append((ITextComponent) playerName)
                            .append((ITextComponent) suffixComponent),
                    ChatType.CHAT, context.getPlayer().getUUID());
    ...
    private boolean tryCreatePortal(final World world, final BlockPos pos, final Direction facing,
            final CrystalData data) {
        final Optional<VaultPortalSize> optional = VaultPortalSize.getPortalSize((IWorld) world,
                pos.relative(facing), Direction.Axis.X, VaultPortalBlock.FRAME);
        if (optional.isPresent()) {
            final VaultPortalSize portal = optional.get();
            final BlockState state = (BlockState) ModBlocks.VAULT_PORTAL.defaultBlockState()
                    .setValue((Property) VaultPortalBlock.AXIS, (Comparable) portal.getAxis());
            ...
            for (int i = -1; i <= portal.getWidth(); ++i) {
                for (int j = -1; j <= portal.getHeight(); ++j) {
                    if (i == -1 || j == -1 || i == portal.getWidth() || j == portal.getHeight()) {
                        final BlockPos p = portal.getBottomLeft().relative(portal.getRightDir(), i)
                                .above(j);
                        final TileEntity te = world.getBlockEntity(p);
                        data.frameData.tiles.add(new FrameData.Tile(world.getBlockState(p).getBlock(),
                                (te == null) ? new CompoundNBT() : te.serializeNBT(), p));
                    }
                }
            }
            data.updateDelegate();
    ...
    private void attemptApplyEcho(final ServerPlayerEntity player, final ItemStack stack) {
        if (shouldApplyEcho(stack)) {
            ...
            if (remainder > 0) {
                EntityHelper.giveItem((PlayerEntity) player,
                        new ItemStack((IItemProvider) ModItems.ECHO_GEM, remainder));
            }
    ...
    private void handleCloning(final ServerPlayerEntity player, final ItemStack stack) {
        ...
        if (nbt.getInt("ShouldClone") == -1) {
            ...
            player.getCommandSenderWorld().playSound((PlayerEntity) null, player.blockPosition(),
                    SoundEvents.FIRE_EXTINGUISH, SoundCategory.PLAYERS, 1.0f, 1.0f);
            ...
            player.getCommandSenderWorld().playSound((PlayerEntity) null, player.blockPosition(),
                    SoundEvents.PLAYER_LEVELUP, SoundCategory.PLAYERS, 0.8f, 1.5f);
        }
    }
