    ...
    public void fillItemCategory(final ItemGroup group, final NonNullList<ItemStack> items) {
        ...
        if (this.allowdedIn(group)) {
            ...
            for (final TalentGroup<?> talentGroup : ModConfigs.TALENTS.getAll()) {
                ...
                if (!talentName.equals("Trader") && !talentName.equals("Looter") && !talentName.equals("Artisan")
    ...
    public void appendHoverText(final ItemStack stack, @Nullable final World world, final List<ITextComponent> tooltip,
        ...
        if (skillableStr != null) {
            ModConfigs.ABILITIES.getAbility(skillableStr).ifPresent(abilityGrp -> {
                final ITextComponent ability = (ITextComponent) new StringTextComponent(abilityGrp.getParentName())
            ...
            ModConfigs.TALENTS.getTalent(skillableStr).ifPresent(talentGrp -> {
                final ITextComponent talent = (ITextComponent) new StringTextComponent(talentGrp.getParentName())
    ...
    public ActionResult<ItemStack> use(final World world, final PlayerEntity player, final Hand hand) {
        ...
        if (world.isClientSide()) {
            ...
            if (abilityOpt.isPresent()) {
                final AbilityTree abilityTree = PlayerAbilitiesData.get(((ServerPlayerEntity) player).getLevel())
                        .getAbilities(player);
                ...
                if (node.getLevel() == 1) {
                    final List<AbilityGroup<?, ?>> dependentNodes = ModConfigs.SKILL_GATES.getGates()
                            .getAbilitiesDependingOn(node.getGroup().getParentName());
            ...
            if (talentOpt.isPresent()) {
                final TalentTree talentTree = PlayerTalentsData.get(((ServerPlayerEntity) player).getLevel())
                        .getTalents(player);
                ...
                if (node2.getLevel() == 1) {
                    final List<TalentGroup<?>> dependentNodes2 = ModConfigs.SKILL_GATES.getGates()
                            .getTalentsDependingOn(node2.getGroup().getParentName());
        ...
        if (world instanceof ServerWorld && entityLiving instanceof ServerPlayerEntity) {
            ...
            ModConfigs.ABILITIES.getAbility(skillableStr).ifPresent(ability -> {
                ...
                if (node.isLearned()) {
                    ...
                    PlayerVaultStatsData.get(sWorld).spendSkillPts(player,
                            -((AbilityConfig) node.getAbilityConfig()).getLearningCost());
            ...
            ModConfigs.TALENTS.getTalent(skillableStr).ifPresent(talent -> {
                ...
                if (node2.isLearned()) {
                    ...
                    PlayerVaultStatsData.get(sWorld).spendSkillPts(player,
                            -((PlayerTalent) node2.getTalent()).getCost());
