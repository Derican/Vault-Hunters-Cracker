...
public class SuperSecretCakes {
    ...
    public static void onCakeEat(final PlayerInteractEvent.RightClickBlock event) {
        ...
        if (world.getBlockState(event.getPos()).getBlock() instanceof CakeBlock) {
            if (world.isClientSide()) {
                ...
                final String cakeQuote = SuperSecretCakes.CAKE_QUOTES[random
                        .nextInt(SuperSecretCakes.CAKE_QUOTES.length)];
                final StringTextComponent text = new StringTextComponent("\"" + cakeQuote + "\"");
                text.setStyle(Style.EMPTY.withItalic(Boolean.valueOf(true))
                        .withColor(Color.fromRgb(-15343)));
                ...
                if (raid != null) {
                    raid.getGenerator().getPiecesAt(event.getPos(), VaultRoom.class)
                            .forEach(room -> room.setCakeEaten(true));
                    raid.getActiveObjective(CakeHuntObjective.class).ifPresent(
                            cakeObjective -> cakeObjective.expandVault(sWorld, sPlayer, event.getPos(), raid));
                }
                event.setCanceled(true);
            }
        }
    ...
    static {
        CAKE_QUOTES = new String[] { "The cake is a lie", "You can have cake and eat it too?",
                "Would like some tea with that?", "The cake equals \u00cf\u20ac (Pi) ?", "This cake is made with love",
                "DONT GET GREEDY", "The cake is a pine?", "That'll go right to your thighs", "Have you got the coffee?",
                "When life gives you cake you eat it", "The cake says 'goodbye'", "The pie want to cry",
                "It's a piece of cake to bake a pretty cake", "The cherries are a lie", "1000 calories",
                "Icing on the cake!", "Happy Birthday! Is it your birthday?", "This is caketastic!",
                "An actual pie chart", "Arrr! I'm a Pie-rate",
                "Not every pies in the world is round, sometimes... pi * r ^ 2", "HALLO!",
                "#NeverLeaving cause cake sticks to you", "Tell me lies, tell me sweet little pies",
                "Diet...what diet!!!!", "I'll take the three story pie and a diet coke... don't want to get fat",
