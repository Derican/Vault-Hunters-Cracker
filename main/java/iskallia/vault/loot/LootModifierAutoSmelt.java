...
public class LootModifierAutoSmelt extends LootModifier {
    ...
    protected List<ItemStack> doApply(final List<ItemStack> generatedLoot, final LootContext context) {
        if (!LootUtils.doesContextFulfillSet(context, LootParameterSets.BLOCK)
                || !context.hasParam(LootParameters.THIS_ENTITY)) {
        ...
        return generatedLoot.stream().filter(stack -> !stack.isEmpty()).map(stack -> {
            final Optional<Tuple<ItemStack, Float>> furnaceResult = RecipeUtil
                    .findSmeltingResult((World) context.getLevel(), stack);
            furnaceResult.ifPresent(result -> {
                ...
                if (exp > 0.0f) {
                    ...
                    while (iExp > 0) {
                        ...
                        iExp -= expPart;
                        world.addFreshEntity((Entity) new ExperienceOrbEntity((World) world, pos.x() + 0.5,
                                pos.y() + 0.5, pos.z() + 0.5, expPart));
                    }
                }
                return;
            });
            return (ItemStack) furnaceResult
                    .map((Function<? super Tuple<ItemStack, Float>, ? extends ItemStack>) Tuple::getA)
                    .orElse(stack);
    ...
    public static class Serializer extends GlobalLootModifierSerializer<LootModifierAutoSmelt> {
        public LootModifierAutoSmelt read(final ResourceLocation location, final JsonObject object,
