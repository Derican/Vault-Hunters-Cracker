...
public class ArenaBossEntity extends FighterEntity {
    ...
    public ArenaBossEntity(final EntityType<? extends ZombieEntity> type, final World world) {
        super(type, world);
        this.teleportTask = new TeleportRandomly<ArenaBossEntity>(this,
                (TeleportRandomly.Condition<ArenaBossEntity>[]) new TeleportRandomly.Condition[] {
    ...
    protected void addBehaviourGoals() {
        ...
        this.goalSelector.addGoal(1,
                (Goal) new ThrowProjectilesGoal<Object>(this, 96, 10, ArenaBossEntity.SNOWBALLS));
    ...
    private float knockbackAttack(final Entity entity) {
        for (int i = 0; i < 20; ++i) {
            ...
            ((ServerWorld) this.level).sendParticles((IParticleData) ParticleTypes.POOF,
                    entity.getX() + this.level.random.nextDouble() - d0,
                    entity.getY() + this.level.random.nextDouble() - d2,
                    entity.getZ() + this.level.random.nextDouble() - d3, 10, d0, d2, d3, 1.0);
        }
        this.level.playSound((PlayerEntity) null, entity.blockPosition(), SoundEvents.IRON_GOLEM_HURT,
    ...
    public boolean hurt(final DamageSource source, final float amount) {
        return (source instanceof RampageDotAbility.PlayerDamageOverTimeSource
                || source.getEntity() instanceof PlayerEntity || source.getEntity() instanceof EternalEntity
                || source == DamageSource.OUT_OF_WORLD) && !this.isInvulnerableTo(source)
                && source != DamageSource.FALL
                && (this.teleportTask.attackEntityFrom(source, amount) || super.hurt(source, amount));
    }

    public static AttributeModifierMap.MutableAttribute getAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.FOLLOW_RANGE, 35.0)
