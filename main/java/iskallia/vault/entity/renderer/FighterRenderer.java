...
public class FighterRenderer extends LivingRenderer<FighterEntity, FighterModel>
{
    ...
    public Vector3d getRenderOffset(final FighterEntity entityIn, final float partialTicks) {
        return entityIn.isCrouching() ? new Vector3d(0.0, -0.125, 0.0) : super.getRenderOffset((Entity)entityIn, partialTicks);
    }
    ...
    private void renderItem(final MatrixStack matrixStackIn, final IRenderTypeBuffer buffer, final int combinedLight, final FighterEntity entity, final ModelRenderer rendererArm, final ModelRenderer rendererArmWear) {
        ...
        rendererArm.render(matrixStackIn, buffer.getBuffer(RenderType.entitySolid(this.getEntityTexture(entity))), combinedLight, OverlayTexture.NO_OVERLAY);
        rendererArmWear.xRot = 0.0f;
        rendererArmWear.render(matrixStackIn, buffer.getBuffer(RenderType.entityTranslucent(this.getEntityTexture(entity))), combinedLight, OverlayTexture.NO_OVERLAY);
    }
