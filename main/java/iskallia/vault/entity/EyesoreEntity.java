...
public class EyesoreEntity extends GhastEntity implements VaultBoss {
    ...
    public void tick() {
        ...
        if (!this.level.isClientSide) {
            ...
            if (vault != null && this.level.getGameTime() % 40L == 0L) {
                vault.getPlayers().stream()
                        .map(p -> p.getServerPlayer(((ServerWorld) this.level).getServer()))
                        .filter(Optional::isPresent).map((Function<? super Object, ?>) Optional::get)
                        .forEach(p -> this.level.playSound((PlayerEntity) null, p.getX(),
                                p.getY(), p.getZ(), SoundEvents.CONDUIT_AMBIENT,
                                SoundCategory.HOSTILE, 1.0f, 0.5f));
            }
            this.brain.tick();
            this.path.tick(this);
            final float healthPercentage = this.getHealth() / this.getMaxHealth();
            final int tentaclesRemaining = (int) this.entityData
                    .get((DataParameter) EyesoreEntity.TENTACLES_REMAINING);
            final int expectedTentacles = MathHelper.clamp((int) (healthPercentage * 10.0f), 0, 9);
            if (tentaclesRemaining != expectedTentacles) {
                this.entityData.set((DataParameter) EyesoreEntity.TENTACLES_REMAINING,
                        (Object) expectedTentacles);
                for (int i = 0; i < tentaclesRemaining - expectedTentacles; ++i) {
                    final EyestalkEntity eyestalkEntity = (EyestalkEntity) ModEntities.EYESTALK
                            .create((World) serverWorld);
                    if (eyestalkEntity != null) {
                        eyestalkEntity.moveTo(this.getX(), this.getY() + 7.0,
                                this.getZ(), this.xRot, this.yRot);
        ...
        final LivingEntity target = ((Optional) this.getEntityData()
                .get((DataParameter) EyesoreEntity.LASER_TARGET))
                .map(playerId -> this.getCommandSenderWorld().getPlayerByUUID(playerId)).orElse(null);
        if (target != null) {
            ++this.laserTick;
        ...
        if ((boolean) this.getEntityData().get((DataParameter) EyesoreEntity.WATCH_CLIENT)
    ...
    private boolean teleportTo(final double x, final double y, final double z) {
        ...
        while (blockpos$mutable.getY() > 0
                && !this.level.getBlockState((BlockPos) blockpos$mutable).getMaterial().blocksMotion()) {
        ...
        if (flag3 && !this.isSilent()) {
            this.level.playSound((PlayerEntity) null, this.xo, this.yo,
                    this.zo, SoundEvents.ENDERMAN_TELEPORT, this.getSoundSource(), 1.0f, 1.0f);
    ...
    public void playerTouch(@Nonnull final PlayerEntity playerEntity) {
        ...
        player.hurt(DamageSource.mobAttack((LivingEntity) this),
                ModConfigs.EYESORE.meleeAttack.getDamage(this));
        this.applyKnockback((Entity) player, ModConfigs.EYESORE.meleeAttack.knockback,
                posEyesore.x - posPlayer.x, posEyesore.z - posPlayer.z);
    }
    ...
    public void applyKnockback(final Entity target, final float strength, final double ratioX, final double ratioZ) {
        if (strength > 0.0f) {
            ...
            final Vector3d vector3d2 = new Vector3d(ratioX, 0.0, ratioZ).normalize()
                    .scale((double) strength);
            target.setDeltaMovement(vector3d.x / 2.0 - vector3d2.x,
                    this.onGround ? Math.min(0.4, vector3d.y / 2.0 + strength)
                            : vector3d.y,
                    vector3d.z / 2.0 - vector3d2.z);
        }
    }
    ...
    public static AttributeModifierMap.MutableAttribute getAttributes() {
        return MonsterEntity.createMonsterAttributes().add(Attributes.FOLLOW_RANGE, 100.0)
                .add(Attributes.MOVEMENT_SPEED, 0.25).add(Attributes.ATTACK_DAMAGE, 3.0)
                .add(Attributes.ATTACK_KNOCKBACK, 3.0).add(Attributes.KNOCKBACK_RESISTANCE, 0.4)
    ...
    static {
        ...
        TENTACLES_REMAINING = EntityDataManager.defineId((Class) EyesoreEntity.class,
                DataSerializers.INT);
