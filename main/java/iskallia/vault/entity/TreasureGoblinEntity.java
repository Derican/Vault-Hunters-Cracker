...
public class TreasureGoblinEntity extends MonsterEntity {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(3, (Goal) new AvoidEntityGoal((CreatureEntity) this,
                (Class) PlayerEntity.class, 6.0f, 1.7000000476837158, 2.0));
    ...
    public void disappear(final ServerWorld world) {
        ...
        if (this.lastAttackedPlayer != null) {
            final StringTextComponent bailText = (StringTextComponent) new StringTextComponent(
                    "Treasure Goblin escaped from you.")
                    .withStyle(Style.EMPTY.withColor(Color.fromRgb(8042883)));
            ...
            world.playSound(this.lastAttackedPlayer, this.getX(), this.getY(),
                    this.getZ(), ModSounds.GOBLIN_BAIL, SoundCategory.MASTER, 0.7f, 1.0f);
        } else {
            world.playSound((PlayerEntity) null, this.getX(), this.getY(),
                    this.getZ(), ModSounds.GOBLIN_BAIL, SoundCategory.MASTER, 0.7f, 1.0f);
        }
    }
    ...
    public static void onWorldTick(final TickEvent.WorldTickEvent event) {
        ...
        final List<TreasureGoblinEntity> goblins = world.getEntities()
                .filter(entity -> entity instanceof TreasureGoblinEntity).map(entity -> (TreasureGoblinEntity) entity)
                .collect((Collector<? super Object, ?, List<TreasureGoblinEntity>>) Collectors.toList());
