...
public class TeleportRandomly<T extends LivingEntity> implements INBTSerializable<CompoundNBT> {
            ...
            if (this.entity.level.random.nextDouble() < chance) {
                for (int i = 0; i < 64; ++i) {
                    if (this.teleportRandomly()) {
                        this.entity.level.playSound((PlayerEntity) null, this.entity.xo,
                                this.entity.yo, this.entity.zo, ModSounds.BOSS_TP_SFX,
                                this.entity.getSoundSource(), 1.0f, 1.0f);
                        return true;
                    }
                }
            }
    ...
    private boolean teleportRandomly() {
        if (!this.entity.level.isClientSide() && this.entity.isAlive()) {
            final double d0 = this.entity.getX()
                    + (this.entity.level.random.nextDouble() - 0.5) * 64.0;
            final double d2 = this.entity.getY()
                    + (this.entity.level.random.nextInt(64) - 32);
            final double d3 = this.entity.getZ()
                    + (this.entity.level.random.nextDouble() - 0.5) * 64.0;
