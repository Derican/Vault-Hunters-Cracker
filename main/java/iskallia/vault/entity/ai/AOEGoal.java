...
public class AOEGoal<T extends MobEntity> extends GoalTask<T> {
    ...
    public void tick() {
        ...
        if (!this.started && this.delay < 0 && this.getEntity().isOnGround()) {
            this.getWorld().playSound((PlayerEntity) null, this.getEntity().getX(),
                    this.getEntity().getY(), this.getEntity().getZ(), SoundEvents.DRAGON_FIREBALL_EXPLODE,
                    this.getEntity().getSoundSource(), 1.0f, 1.0f);
            ((ServerWorld) this.getWorld()).sendParticles((IParticleData) ParticleTypes.EXPLOSION,
                    this.getEntity().getX() + 0.5, this.getEntity().getY() + 0.1,
                    this.getEntity().getZ() + 0.5, 10, this.getRandom().nextGaussian() * 0.02,
                    this.getRandom().nextGaussian() * 0.02, this.getRandom().nextGaussian() * 0.02, 1.0);
        ...
        if (this.started) {
            ...
            this.getWorld().getEntitiesOfClass((Class) LivingEntity.class,
                    new AxisAlignedBB(this.shockwave).inflate(max, max, max), e -> {
                        ...
                        final Vector3d direction = new Vector3d(e.getX() - this.shockwave.getX(),
                                e.getY() - this.shockwave.getY(),
                                e.getZ() - this.shockwave.getZ()).scale(0.5);
                        final Vector3d direction2 = direction.normalize().add(0.0,
                                1.0 - 0.02 * (this.tick + 1), 0.0);
