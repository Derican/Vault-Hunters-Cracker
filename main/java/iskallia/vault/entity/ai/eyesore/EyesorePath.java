...
public class EyesorePath {
    ...
    public void stayInRange(final Vector3d center, final Entity other, final double speed, final double distance,
            final double error) {
        this.speed = speed;
    ...
    public void stayInRange(final EyesoreEntity entity, final Entity other, final double speed, final double distance,
            final double error) {
        this.speed = speed;
        if (this.target == null || Math.abs(this.target.distanceTo(other.position()) - distance) > error) {
    ...
    public void tick(final EyesoreEntity entity) {
        ...
        if (this.target.distanceTo(entity.position()) <= 1.0) {
            ...
            entity.setDeltaMovement(
                    this.target.subtract(entity.position()).normalize().scale(this.speed));
        }
    }
}
