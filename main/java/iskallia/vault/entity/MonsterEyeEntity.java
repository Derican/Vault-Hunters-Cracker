...
public class MonsterEyeEntity extends SlimeEntity implements VaultBoss {
    ...
    public MonsterEyeEntity(final EntityType<? extends SlimeEntity> type, final World worldIn) {
        ...
        this.teleportTask = new TeleportRandomly<MonsterEyeEntity>(this,
                (TeleportRandomly.Condition<MonsterEyeEntity>[]) new TeleportRandomly.Condition[] {
    ...
    protected void dealDamage(final LivingEntity entityIn) {
        if (this.isAlive()) {
            ...
            if (this.distanceToSqr((Entity) entityIn) < 0.8 * i * 0.8 * i && this.canSee((Entity) entityIn)
                    && entityIn.hurt(DamageSource.mobAttack((LivingEntity) this), this.getAttackDamage())) {
