...
public class EyestalkEntity extends VexEntity {
    ...
    public void tick() {
        ...
        if (!this.level.isClientSide) {
            if (this.currentTarget == null || this.currentTarget.equals(this.mother)) {
                ...
                if (vault != null) {
                    players = vault.getPlayers().stream().filter(player -> player instanceof VaultRunner)
                            .map((Function<? super Object, ?>) VaultPlayer::getPlayerId)
                            .collect((Collector<? super Object, ?, List<UUID>>) Collectors.toList());
                    players.removeAll(this.targetCooldown.entrySet().stream().filter(e -> e.getValue().cooldown > 0)
                            .map((Function<? super Object, ?>) Map.Entry::getKey)
            ...
            if (this.currentTarget != null) {
                ...
                if (entity != null) {
                    if (entity instanceof LivingEntity) {
                        ...
                        this.moveControl.setWantedPosition(vector3d.x, vector3d.y,
                                vector3d.z, 1.0);
                    }
                    if (this.getBoundingBox().intersects(entity.getBoundingBox())
                            && !entity.getUUID().equals(this.mother)) {
    ...
    public boolean doHurtTarget(final Entity entity) {
        ...
        if (flag) {
            if (f2 > 0.0f && entity instanceof LivingEntity) {
                this.applyKnockback(entity, f2 * 0.5f, MathHelper.sin(this.yRot * 0.017453292f),
                        -MathHelper.cos(this.yRot * 0.017453292f));
    ...
    protected void actuallyHurt(final DamageSource source, final float damageAmount) {
        ...
        if ((direct != null && direct.getUUID().equals(this.currentTarget))
                || (indirect != null && indirect.getUUID().equals(this.currentTarget))) {
