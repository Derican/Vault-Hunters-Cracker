...
public class EyesoreFireballEntity extends ProjectileItemEntity {
    ...
    protected void onHit(final RayTraceResult result) {
        ...
        if (!this.level.isClientSide) {
            this.level.explode((Entity) null, this.getX(), this.getY(),
                    this.getZ(), (float) this.explosionPower, Explosion.Mode.NONE);
            this.remove();
        }
    }
    ...
    protected void onHitEntity(final EntityRayTraceResult result) {
        ...
        if (!this.level.isClientSide) {
            ...
            final DamageSource source = new IndirectEntityDamageSource("fireball", (Entity) this, shooter)
                    .setMagic();
    ...
    public void playEffects() {
        ...
        for (int i = 0; i < 5; ++i) {
            final Vector3d v = vec.add(
                    ...
                    (double) (this.random.nextFloat() * 0.4f * (this.random.nextBoolean() ? 1 : -1)),
                    (double) (this.random.nextFloat() * 0.4f * (this.random.nextBoolean() ? 1 : -1)));
            this.level.addParticle((IParticleData) ModParticles.RED_FLAME.get(), v.x,
                    v.y, v.z, 0.0, 0.0, 0.0);
            this.level.addParticle((IParticleData) ParticleTypes.FLAME, v.x,
                    v.y, v.z, 0.0, 0.0, 0.0);
        }
    }
