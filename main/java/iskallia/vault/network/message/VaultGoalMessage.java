...
public class VaultGoalMessage extends OpcodeMessage<VaultGoal> {
    ...
    public static VaultGoalMessage obeliskGoal(final int touched, final int max) {
        final ITextComponent text = (ITextComponent) new StringTextComponent("Obelisks")
    ...
    public static VaultGoalMessage killBossGoal() {
        final ITextComponent text = (ITextComponent) new StringTextComponent("Boss Battle")
                .withStyle(TextFormatting.BOLD).withStyle(TextFormatting.GREEN);
        return OpcodeMessage.composeMessage(new VaultGoalMessage(), VaultGoal.OBELISK_MESSAGE,
                payload -> payload.putString("Message", ITextComponent.Serializer.toJson(text)));
    }
    ...
    public static VaultGoalMessage architectEvent(final float completedPercent, final int ticksUntilNextVote,
            final int totalTicksUntilNextVote, @Nullable final VotingSession activeVotingSession) {
    ...
    public static VaultGoalMessage architectFinalEvent(final int killedBosses, final int totalBosses,
            final int knowledge, final int totalKnowledge, @Nullable final VotingSession activeVotingSession,
            final boolean killCurrentBoss) {
        ITextComponent text;
        if (killCurrentBoss) {
            text = (ITextComponent) new StringTextComponent("Kill the Boss!").withStyle(TextFormatting.BOLD)
                    .withStyle(TextFormatting.RED);
        } else {
            text = (ITextComponent) new StringTextComponent("Gather Knowledge!").withStyle(TextFormatting.BOLD)
    ...
    public static VaultGoalMessage raidChallenge(final int wave, final int totalWaves, final int aliveMobs,
            final int totalMobs, final int tickWaveDelay, final int completed, final int target,
            final List<ITextComponent> positiveModifiers, final List<ITextComponent> negativeModifiers) {
        return OpcodeMessage.composeMessage(new VaultGoalMessage(), VaultGoal.RAID_GOAL, payload -> {
            ...
            positiveModifiers.forEach(modifier -> positives
                    .add((Object) StringNBT.valueOf(ITextComponent.Serializer.toJson(modifier))));
            payload.put("positives", (INBT) positives);
            final ListNBT negatives = new ListNBT();
            negativeModifiers.forEach(modifier -> negatives
                    .add((Object) StringNBT.valueOf(ITextComponent.Serializer.toJson(modifier))));
