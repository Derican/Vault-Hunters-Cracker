...
public class ModStructures {
    ...
    public static void register(final RegistryEvent.Register<Structure<?>> event) {
        ModStructures.VAULT_STAR = register((IForgeRegistry<Structure<?>>) event.getRegistry(), "vault_star",
                new VaultStructure(VaultStructure.Config.CODEC));
        ModStructures.ARCHITECT_EVENT = register((IForgeRegistry<Structure<?>>) event.getRegistry(), "architect_event",
                new ArchitectEventStructure(ArchitectEventStructure.Config.CODEC));
        ModStructures.RAID_CHALLENGE = register((IForgeRegistry<Structure<?>>) event.getRegistry(), "raid_challenge",
                new RaidChallengeStructure(RaidChallengeStructure.Config.CODEC));
        ModStructures.VAULT_TROVE = register((IForgeRegistry<Structure<?>>) event.getRegistry(), "trove",
                new VaultTroveStructure(VaultTroveStructure.Config.CODEC));
        ModStructures.FINAL_VAULT_LOBBY = register((IForgeRegistry<Structure<?>>) event.getRegistry(),
                "final_vault_lobby", new FinalVaultLobbyStructure(FinalVaultLobbyStructure.Config.CODEC));
        ModStructures.FINAL_VAULT_BOSS = register((IForgeRegistry<Structure<?>>) event.getRegistry(),
                "final_vault_boss", new FinalVaultBossStructure(FinalVaultBossStructure.Config.CODEC));
        PoolElements.register(event);
    }

    private static <T extends Structure<?>> T register(final IForgeRegistry<Structure<?>> registry, final String name,
            final T structure) {
    ...
    public static class PoolElements {
        ...
        public static void register(final RegistryEvent.Register<Structure<?>> event) {
            PoolElements.PALETTED_SINGLE_POOL_ELEMENT = register("paletted_single_pool_element",
                    PalettedSinglePoolElement.CODEC);
            PoolElements.PALETTED_LIST_POOL_ELEMENT = register("paletted_list_pool_element",
                    PalettedListPoolElement.CODEC);
        }

        static <P extends JigsawPiece> IJigsawDeserializer<P> register(final String name, final Codec<P> codec) {
            return (IJigsawDeserializer<P>) Registry.register(Registry.STRUCTURE_POOL_ELEMENT, Vault.id(name),
                    (Object) (() -> codec));
        }
    }
}
