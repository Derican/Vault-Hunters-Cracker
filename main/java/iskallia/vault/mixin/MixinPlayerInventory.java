...
public class MixinPlayerInventory implements InventorySnapshotData.InventoryAccessor {
    ...
    @ModifyArg(method = {
            "hurtArmor" }, index = 0, at = @At(value = "INVOKE", target = "Lnet/minecraft/item/ItemStack;damageItem(ILnet/minecraft/entity/LivingEntity;Ljava/util/function/Consumer;)V"))
    public int limitMaxArmorDamage(int damageAmount) {
        ...
        if (this.player.getCommandSenderWorld() instanceof ServerWorld) {
            ...
            if (vault != null) {
                ...
                for (final DurabilityDamageModifier modifier : vault
                        .getActiveModifiersFor(PlayerFilter.of(this.player), DurabilityDamageModifier.class)) {
