...
public abstract class MixinBossOverlayGui {
    ...
    @Redirect(method = {
            "render" }, at = @At(value = "INVOKE", target = "Ljava/util/Map;values()Ljava/util/Collection;"))
    private Collection<ClientBossInfo> thing(final Map<UUID, ClientBossInfo> map) {
        ...
        Minecraft.getInstance().level.entitiesForRendering()
                .forEach(entity -> entities.put(entity.getUUID(), entity));
        ...
        })).map((Function<? super Object, ?>) Map.Entry::getValue)
                .collect((Collector<? super Object, ?, Collection<ClientBossInfo>>) Collectors.toList());
    }
}
