...
public class AuraManager {
    ...
    public static void onTick(final TickEvent.WorldTickEvent event) {
        ...
        final Set<ActiveAura> auras = AuraManager.INSTANCE.activeAuras.getOrDefault(event.world.dimension(),
    ...
    public Collection<ActiveAura> getAurasAffecting(final Entity entity) {
        final Collection<ActiveAura> worldAuras = this.activeAuras
                .getOrDefault(entity.getCommandSenderWorld().dimension(), Collections.emptySet());
        if (worldAuras.isEmpty()) {
            return worldAuras;
        }
        return worldAuras.stream().filter(aura -> aura.isAffected(entity))
                .collect((Collector<? super ActiveAura, ?, Collection<ActiveAura>>) Collectors.toSet());
    }
