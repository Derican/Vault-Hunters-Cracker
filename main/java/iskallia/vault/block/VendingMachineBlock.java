...
public class VendingMachineBlock extends Block {
    ...
    public VendingMachineBlock(final AbstractBlock.Properties properties) {
        ...
        this.registerDefaultState((BlockState) ((BlockState) ((BlockState) this.stateDefinition.any())
                .setValue((Property) VendingMachineBlock.FACING, (Comparable) Direction.NORTH))
                .setValue((Property) VendingMachineBlock.HALF, (Comparable) DoubleBlockHalf.LOWER));
    }
    ...
    public BlockState getStateForPlacement(final BlockItemUseContext context) {
        ...
        if (pos.getY() < 255 && world.getBlockState(pos.above()).canBeReplaced(context)) {
            return (BlockState) ((BlockState) this.defaultBlockState().setValue((Property) VendingMachineBlock.FACING,
        ...
        if (!worldIn.isClientSide && player.isCreative()) {
            ...
            if (half == DoubleBlockHalf.UPPER) {
                ...
                if (blockstate.getBlock() == state.getBlock()
                        && blockstate.getValue((Property) VendingMachineBlock.HALF) == DoubleBlockHalf.LOWER) {
    ...
    public BlockState updateShape(final BlockState stateIn, final Direction facing, final BlockState facingState,
        ...
        if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
            return (BlockState) ((facingState.is((Block) this)
                    && facingState.getValue((Property) VendingMachineBlock.HALF) != half)
                            ? stateIn.setValue((Property) VendingMachineBlock.FACING,
                                    facingState.getValue((Property) VendingMachineBlock.FACING))
    ...
    public void setPlacedBy(final World worldIn, final BlockPos pos, final BlockState state,
        ...
        worldIn.setBlock(pos.above(), (BlockState) state.setValue((Property) VendingMachineBlock.HALF,
                (Comparable) DoubleBlockHalf.UPPER), 3);
    }
        ...
        if (heldStack.getItem() instanceof ItemTraderCore) {
            ...
            NetworkHooks.openGui((ServerPlayerEntity) player, (INamedContainerProvider) new INamedContainerProvider() {
                ...
                public Container createMenu(final int windowId, final PlayerInventory playerInventory,
                    ...
                    return new VendingMachineContainer(windowId, world, vendingMachinePos, playerInventory,
                            playerEntity);
                }
            }, buffer -> {
                final BlockState blockState = world.getBlockState(pos);
    ...
    public static BlockPos getTileEntityPos(final BlockState state, final BlockPos pos) {
        return (state.getValue((Property) VendingMachineBlock.HALF) == DoubleBlockHalf.UPPER) ? pos.below()
                : pos;
    }
