...
public class KeyPressBlock extends FallingBlock {
    ...
    public KeyPressBlock() {
        super(AbstractBlock.Properties.of(Material.HEAVY_METAL, MaterialColor.METAL)
                .sound(SoundType.ANVIL).strength(2.0f, 3600000.0f));
    }

    public BlockState getStateForPlacement(final BlockItemUseContext context) {
        return (BlockState) this.defaultBlockState().setValue((Property) KeyPressBlock.FACING,
                (Comparable) context.getHorizontalDirection().getClockWise());
    }
    ...
    public void onLand(final World worldIn, final BlockPos pos, final BlockState fallingState,
            final BlockState hitState, final FallingBlockEntity fallingBlock) {
    ...
    public BlockState rotate(final BlockState state, final Rotation rot) {
        return (BlockState) state.setValue((Property) KeyPressBlock.FACING,
                (Comparable) rot.rotate((Direction) state.getValue((Property) KeyPressBlock.FACING)));
    }
    ...
    static {
        ...
        X_AXIS_AABB = VoxelShapes.or(KeyPressBlock.PART_BASE,
                new VoxelShape[] { KeyPressBlock.PART_LOWER_X, KeyPressBlock.PART_MID_X, KeyPressBlock.PART_UPPER_X });
        Z_AXIS_AABB = VoxelShapes.or(KeyPressBlock.PART_BASE,
                new VoxelShape[] { KeyPressBlock.PART_LOWER_Z, KeyPressBlock.PART_MID_Z, KeyPressBlock.PART_UPPER_Z });
    }
}
