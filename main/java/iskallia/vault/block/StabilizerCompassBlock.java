...
public class StabilizerCompassBlock extends Block {
    ...
    public StabilizerCompassBlock() {
        super(AbstractBlock.Properties.of(Material.STONE).sound(SoundType.STONE)
                .strength(-1.0f, 3.6E8f).noDrops());
        this.registerDefaultState((BlockState) ((BlockState) this.stateDefinition.any())
                .setValue((Property) StabilizerCompassBlock.DIRECTION, (Comparable) Direction.NORTH));
    }
            ...
            if (vault != null) {
                vault.getActiveObjective(ArchitectSummonAndKillBossesObjective.class).ifPresent(objective -> {
                    if (objective.getActiveSession() instanceof SummonAndKillBossesVotingSession) {
                        final SummonAndKillBossesVotingSession session = (SummonAndKillBossesVotingSession) objective
                                .getActiveSession();
                        final Direction direction = (Direction) state
                                .getValue((Property) StabilizerCompassBlock.DIRECTION);
                        final BlockPos stabilizerPos = pos.above().relative(direction.getOpposite());
                        if (stabilizerPos.equals((Object) session.getStabilizerPos())
                                && session.hasDirectionChoice(direction)) {
