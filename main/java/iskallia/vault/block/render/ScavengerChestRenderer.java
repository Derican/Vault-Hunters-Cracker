...
public class ScavengerChestRenderer extends TileEntityRenderer<ScavengerChestTileEntity> {
    ...
    public void render(final ScavengerChestTileEntity tileEntity, final float partialTicks,
        ...
        final BlockState renderState = (BlockState) (isInWorldRender ? tileEntity.getBlockState()
                : ModBlocks.SCAVENGER_CHEST.defaultBlockState().setValue((Property) ChestBlock.FACING,
                        (Comparable) Direction.SOUTH));
        final float hAngle = ((Direction) renderState.getValue((Property) ChestBlock.FACING))
                .toYRot();
        ...
        float lidRotation = ((Float2FloatFunction) lidCallback
                .apply(ScavengerChestBlock.opennessCombiner((IChestLid) tileEntity))).get(partialTicks);
        lidRotation = 1.0f - lidRotation;
        lidRotation = 1.0f - lidRotation * lidRotation * lidRotation;
        ScavengerChestRenderer.CHEST_MODEL.setLidAngle(lidRotation);
        final int combinedLidLight = ((Int2IntFunction) lidCallback
                .apply((TileEntityMerger.ICallback) new DualBrightnessCallback())).applyAsInt(combinedLight);
        final IVertexBuilder vb = ScavengerChestRenderer.MATERIAL.buffer(buffer,
                (Function) RenderType::entityCutout);
        matrixStack.pushPose();
        matrixStack.translate(0.5, 0.5, 0.5);
        matrixStack.mulPose(Vector3f.YP.rotationDegrees(-hAngle));
        ScavengerChestRenderer.CHEST_MODEL.renderToBuffer(matrixStack, vb, combinedLidLight, combinedOverlay, 1.0f,
                1.0f, 1.0f, 1.0f);
        matrixStack.popPose();
    }
