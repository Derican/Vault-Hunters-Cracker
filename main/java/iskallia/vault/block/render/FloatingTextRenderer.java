...
public class FloatingTextRenderer extends TileEntityRenderer<FloatingTextTileEntity> {
    ...
    public void render(@Nonnull final FloatingTextTileEntity tileEntity, final float partialTicks,
            @Nonnull final MatrixStack matrixStack, @Nonnull final IRenderTypeBuffer buffer, final int combinedLightIn,
        ...
        for (int i = length - 1; i >= 0; --i) {
            ...
            if (text != null) {
                ...
                fontRenderer.drawInBatch((ITextComponent) text, offset, 0.0f, color, false, matrix4f, buffer, false,
                        opacity, combinedLightIn);
