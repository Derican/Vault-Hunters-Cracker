...
public class VaultAltarRenderer extends TileEntityRenderer<VaultAltarTileEntity> {
    ...
    public void render(final VaultAltarTileEntity altar, final float partialTicks, final MatrixStack matrixStack,
        ...
        this.renderItem(new ItemStack((IItemProvider) ModItems.VAULT_ROCK), new double[] { 0.5, 1.35, 0.5 },
                Vector3f.YP.rotationDegrees(180.0f - player.yRot), matrixStack, buffer,
                partialTicks, combinedOverlay, lightLevel);
        ...
        for (int i = 0; i < items.size(); ++i) {
            ...
            this.renderItem(stack, translation,
                    Vector3f.YP.rotationDegrees(this.getAngle(player, partialTicks) * 5.0f), matrixStack,
                    buffer, partialTicks, combinedOverlay, lightLevel);
        ...
        if (recipe.isPogInfused()) {
            ...
            final IBakedModel ibakedmodel = this.mc.getItemRenderer().getModel(pogStack, (World) null,
                    (LivingEntity) null);
            for (int j = 0; j < 3; ++j) {
                final double r = 1.0;
                ...
                matrixStack.mulPose(Vector3f.YP
                        .rotationDegrees(j * 120 + this.getAngle(player, partialTicks) * (infusing ? 25.0f : 15.0f)));
                matrixStack.translate(r, Math.sin(this.getAngle(player, partialTicks) * 0.25 + j) * 0.2, 0.0);
                matrixStack.mulPose(
                        Vector3f.YP.rotationDegrees(this.getAngle(player, partialTicks) * 10.0f));
                this.mc.getItemRenderer().render(pogStack, ItemCameraTransforms.TransformType.GROUND, true,
                        matrixStack, buffer, lightLevel, combinedOverlay, ibakedmodel);
                matrixStack.popPose();
            }
        }
    }

    private void renderItem(final ItemStack stack, final double[] translation, final Quaternion rotation,
            final MatrixStack matrixStack, final IRenderTypeBuffer buffer, final float partialTicks,
            final int combinedOverlay, final int lightLevel) {
        matrixStack.pushPose();
    ...
    private void renderLabel(final RequiredItem item, final MatrixStack matrixStack, final IRenderTypeBuffer buffer,
            final int lightLevel, final double[] corner, final StringTextComponent text, final int color) {
        ...
        fontRenderer.drawInBatch((ITextComponent) text, offset, 0.0f, color, false, matrix4f, buffer, false, opacity,
                lightLevel);
        if (player.isShiftKeyDown()) {
            ...
            fontRenderer.drawInBatch(item.getItem().getHoverName(), offset, 0.0f, color, false, matrix4f, buffer,
                    false, opacity, lightLevel);
        }
        matrixStack.popPose();
    }
