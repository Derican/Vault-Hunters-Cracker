...
public class StabilizerBlock extends Block {
    ...
    public StabilizerBlock() {
        super(AbstractBlock.Properties.of(Material.GLASS).sound(SoundType.GLASS)
                .strength(-1.0f, 3600000.0f).noOcclusion().noDrops());
        this.registerDefaultState((BlockState) ((BlockState) this.stateDefinition.any())
                .setValue((Property) StabilizerBlock.HALF, (Comparable) DoubleBlockHalf.LOWER));
    }
    ...
    private void spawnNoVoteParticles(final World world, final BlockPos pos) {
        for (int i = 0; i < 40; ++i) {
            final Vector3d particlePos = new Vector3d(
                    pos.getX() - 0.5 + StabilizerBlock.rand.nextFloat() * 2.0f,
                    (double) (pos.getY() + StabilizerBlock.rand.nextFloat() * 8.0f),
                    pos.getZ() - 0.5 + StabilizerBlock.rand.nextFloat() * 2.0f);
            final EffectMessage pkt = new EffectMessage(EffectMessage.Type.COLORED_FIREWORK, particlePos)
                    .addData(buf -> buf.writeInt(10027008));
    ...
    private boolean startPoll(final ServerWorld world, final BlockPos pos) {
        ...
        return vault != null && (vault.getActiveObjective(ArchitectObjective.class)
                .map((Function<? super ArchitectObjective, ?>) ArchitectObjective::getActiveSession)
                .map((Function<? super Object, ?>) VotingSession::getStabilizerPos)
                .map(stabilizer -> stabilizer.equals((Object) pos)).orElse(false)
                || vault.getActiveObjective(ArchitectObjective.class)
                        .map(objective -> objective.createVotingSession(world, pos)).orElse(false));
    }
