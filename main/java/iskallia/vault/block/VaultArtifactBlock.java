...
public class VaultArtifactBlock extends FacedBlock
{
    ...
    public VaultArtifactBlock() {
        ...
        this.registerDefaultState((BlockState)((BlockState)this.stateDefinition.any()).setValue((Property)VaultArtifactBlock.FACING, (Comparable)Direction.SOUTH));
    }
    ...
    public BlockState getStateForPlacement(final BlockItemUseContext context) {
        ...
        return (BlockState)super.getStateForPlacement(context).setValue((Property)VaultArtifactBlock.ORDER_PROPERTY, (Comparable)this.getOrder(artifactBlockItem));
    }
    ...
    private static List<BlockPos> hasFullArtifactSet(final ServerWorld world, final BlockPos start, final Direction facing) {
        ...
        for (int order = 0; order < 25; ++order) {
            ...
            final int orderAt = (25 - (int)offsetState.getValue((Property)VaultArtifactBlock.ORDER_PROPERTY) + 24) % 25;
            if (order != orderAt) {
                return Collections.emptyList();
            }
            positions.add(at);
