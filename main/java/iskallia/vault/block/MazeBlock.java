...
public class MazeBlock extends Block {
    ...
    public MazeBlock() {
        ...
        this.registerDefaultState((BlockState) ((BlockState) this.stateDefinition.any())
                .setValue((Property) MazeBlock.COLOR, (Comparable) MazeColor.RED));
    }
    ...
    public void stepOn(final World worldIn, final BlockPos pos, final Entity entityIn) {
        ...
        if (scoreboard.getObjective("Color") == null) {
            scoreboard.addObjective("Color", ScoreCriteria.DUMMY,
                    (ITextComponent) new StringTextComponent("Color"), ScoreCriteria.RenderType.INTEGER);
        }
        final ScoreObjective colorObjective = scoreboard.getObjective("Color");
        assert colorObjective != null;
        final Score colorScore = worldIn.getScoreboard().getOrCreatePlayerScore(player.getDisplayName().getString(),
                colorObjective);
        ...
        if (playerColor != blockColor) {
            ...
            colorScore
                    .setScore((playerColor == MazeColor.RED) ? MazeColor.BLUE.ordinal() : MazeColor.RED.ordinal());
            player.teleportTo(nextPosition.getX() + 0.5, (double) nextPosition.getY(),
                    nextPosition.getZ() + 0.5);
        }
