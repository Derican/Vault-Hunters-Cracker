...
public class CatalystDecryptionTableBlock extends Block {
    ...
    public CatalystDecryptionTableBlock() {
        super(AbstractBlock.Properties.of(Material.STONE).strength(1.5f, 6.0f)
                .noOcclusion());
    }

    public BlockState getStateForPlacement(final BlockItemUseContext context) {
        return (BlockState) this.defaultBlockState().setValue((Property) CatalystDecryptionTableBlock.FACING,
            ...
            if (tileentity instanceof CatalystDecryptionTableTileEntity) {
                tileentity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
                    for (int i = 0; i < handler.getSlots(); ++i) {
                        InventoryHelper.dropItemStack(worldIn, (double) pos.getX(),
                                (double) pos.getY(), (double) pos.getZ(), handler.getStackInSlot(i));
                    }
                });
            }
        }
    ...
    public BlockState rotate(final BlockState state, final Rotation rot) {
        return (BlockState) state.setValue((Property) CatalystDecryptionTableBlock.FACING, (Comparable) rot
                .rotate((Direction) state.getValue((Property) CatalystDecryptionTableBlock.FACING)));
    }

    public BlockState mirror(final BlockState state, final Mirror mirrorIn) {
        return state.rotate(mirrorIn
                .getRotation((Direction) state.getValue((Property) CatalystDecryptionTableBlock.FACING)));
    }
