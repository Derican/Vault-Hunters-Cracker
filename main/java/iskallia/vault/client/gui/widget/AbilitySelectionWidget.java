...
public class AbilitySelectionWidget extends Widget {
    ...
    public AbilitySelectionWidget(final int x, final int y, final AbilityNode<?, ?> abilityNode,
            final double angleBoundary) {
    ...
    public Rectangle getBounds() {
        return new Rectangle(this.x - 12, this.y - 12, this.width,
                this.height);
    }
    ...
    public boolean isMouseOver(final double mouseX, final double mouseY) {
        ...
        final double dot = towardsWidget.x * towardsMouse.x
                + towardsWidget.y * towardsMouse.y;
        final double angleBetween = Math
                .acos(dot / (MathUtilities.length(towardsWidget) * MathUtilities.length(towardsMouse)));
    ...
    public void render(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        final Minecraft minecraft = Minecraft.getInstance();
        final String styleKey = (this.abilityNode.getSpecialization() != null) ? this.abilityNode.getSpecialization()
                : this.abilityNode.getGroup().getParentName();
        ...
        if (cooldown > 0) {
            ...
            AbstractGui.fill(matrixStack, bounds.x + 4, bounds.y + 4 + (16 - cooldownHeight),
                    bounds.x + 4 + 16, bounds.y + 4 + 16, -1711276033);
            RenderSystem.enableBlend();
        }
        if (thisAbility.equals(ClientAbilityData.getSelectedAbility())) {
            minecraft.getTextureManager().bind(AbilitySelectionWidget.HUD_RESOURCE);
