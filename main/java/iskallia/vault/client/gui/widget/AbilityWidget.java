...
public class AbilityWidget extends Widget implements ConnectableWidget, ComponentWidget {
    ...
    public AbilityWidget(final String abilityName, final AbilityTree abilityTree, final SkillStyle style) {
        super(style.x, style.y, 48,
                ...
                (ITextComponent) new StringTextComponent("the_vault.widgets.ability"));
        this.selected = false;
        this.hoverable = true;
    ...
    public AbilityNode<?, ?> makeAbilityNode() {
        ...
        return new AbilityNode<Object, Object>(this.getAbility().getAbilityGroupName(), level,
                this.isSpecialization() ? this.abilityName : null);
    }
    ...
    public boolean isLocked() {
        if (this.isSpecialization()) {
            ...
            if (!existing.isLearned() || (existing.getSpecialization() != null
                    && !existing.getSpecialization().equals(this.abilityName))) {
                return true;
            }
        }
        return VaultBarOverlay.vaultLevel < ((AbilityConfig) this.makeAbilityNode().getAbilityConfig())
                .getLevelRequirement();
    }
    ...
    private void renderHover(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        if (this.isSpecialization()) {
            tTip.add((ITextComponent) new StringTextComponent(node.getSpecializationName())
                    .withStyle(TextFormatting.ITALIC).withStyle(TextFormatting.GOLD));
        }
        if (this.isLocked() && this.isSpecialization() && existing.getSpecialization() != null
                && !existing.getSpecialization().equals(node.getSpecialization())) {
            tTip.add((ITextComponent) new StringTextComponent("Specialization already in use:")
                    .withStyle(TextFormatting.RED));
            tTip.add((ITextComponent) new StringTextComponent(existing.getSpecializationName())
                    .withStyle(TextFormatting.RED));
        }
        final int levelRequirement = ((AbilityConfig) node.getGroup().getAbilityConfig(this.abilityName,
                Math.max(existing.getLevel() - 1, 0))).getLevelRequirement();
    ...
    public void renderIcon(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        int vOffset = 0;
        if (this.isSpecialization() && abilityNode.isLearned()
                && this.abilityName.equals(abilityNode.getSpecialization())) {
            vOffset = 31;
