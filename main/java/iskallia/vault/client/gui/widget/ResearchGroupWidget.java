...
public class ResearchGroupWidget extends Widget {
    ...
    public ResearchGroupWidget(final ResearchGroupStyle style, final ResearchTree researchTree,
            final Supplier<ResearchWidget> selectedResearchSupplier) {
        super(style.getX(), style.getY(), getGroupWidth(style), getGroupHeight(style),
                (ITextComponent) new StringTextComponent("the_vault.widgets.research_group"));
    ...
    private void renderHeaderInformation(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
        final FontRenderer fr = Minecraft.getInstance().font;
        int titleOffset = 5;
        ...
        if (group != null) {
            ...
            final IReorderingProcessor bidiTitle = LanguageMap.getInstance()
                    .getVisualOrder((ITextProperties) new StringTextComponent(title));
            final IRenderTypeBuffer.Impl renderBuf = IRenderTypeBuffer
                    .immediate(Tessellator.getInstance().getBuilder());
            fr.drawInBatch(bidiTitle, (float) titleOffset, 6.0f, this.groupStyle.getHeaderTextColor(), true,
                    matrixStack.last().pose(), (IRenderTypeBuffer) renderBuf, false, 0,
        ...
        final int selectedAdditionalDisplay = Math.round(currentAdditional + selectedAdditional)
                - currentAdditionalDisplay;
        if (currentAdditionalDisplay == 0 && !displayAdditional) {
            return;
        }
        final String displayStr = (currentAdditionalDisplay >= 0) ? ("+" + currentAdditionalDisplay)
                : String.valueOf(currentAdditionalDisplay);
        final StringTextComponent costDisplay = new StringTextComponent(displayStr);
        if (displayAdditional) {
            final String selectedStr = (selectedAdditionalDisplay >= 0) ? ("+" + selectedAdditionalDisplay)
                    : String.valueOf(selectedAdditionalDisplay);
        ...
        final IRenderTypeBuffer.Impl renderBuf2 = IRenderTypeBuffer
                .immediate(Tessellator.getInstance().getBuilder());
        fr.drawInBatch(bidiCost, (float) costRightOffset, 6.0f, this.groupStyle.getHeaderTextColor(), true,
                matrixStack.last().pose(), (IRenderTypeBuffer) renderBuf2, false, 0,
                LightmapHelper.getPackedFullbrightCoords());
        renderBuf2.endBatch();
        RenderSystem.enableDepthTest();
        RenderSystem.enableBlend();
    ...
    private void renderHeaderIcon(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
        if (this.groupStyle.getIcon() == null) {
            return;
        }
        ...
        ScreenDrawHelper.draw(7, DefaultVertexFormats.POSITION_COLOR_TEX,
                buf -> ScreenDrawHelper
                        .rect((IVertexBuilder) buf, matrixStack, iconFrameX, iconFrameY, 0.0f, 30.0f, 30.0f)
                        .texVanilla((float) iconFrame.getU(), (float) iconFrame.getV(), (float) iconFrame.getW(),
                                (float) iconFrame.getH())
                        .draw());
        ...
        ScreenDrawHelper.draw(7, DefaultVertexFormats.POSITION_COLOR_TEX,
                buf -> ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, iconX, iconY, 0.0f, 16.0f, 16.0f)
                        .texVanilla((float) icon.getU(), (float) icon.getV(), 16.0f, 16.0f).draw());
    }

    private void renderHeaderBox(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
        final int headerColor = this.groupStyle.getHeaderColor();
        ScreenDrawHelper.draw(7, DefaultVertexFormats.POSITION_COLOR_TEX, buf -> {
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, 20.0f).texVanilla(166.0f, 0.0f, 3.0f, 20.0f)
                    .color(headerColor).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, (float) (this.width - 6), 20.0f)
                    .at(3.0f, 0.0f).texVanilla(169.0f, 0.0f, 1.0f, 20.0f).color(headerColor).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, 20.0f)
                    .at((float) (this.width - 3), 0.0f).texVanilla(170.0f, 0.0f, 3.0f, 20.0f)
                    .color(headerColor).draw();
        });
    }

    private void renderContainerBox(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
        ScreenDrawHelper.draw(7, DefaultVertexFormats.POSITION_COLOR_TEX, buf -> {
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, 3.0f).texVanilla(166.0f, 20.0f, 3.0f, 3.0f)
                    .draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, (float) (this.width - 6), 3.0f)
                    .at(3.0f, 0.0f).texVanilla(169.0f, 20.0f, 1.0f, 3.0f).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, 3.0f)
                    .at((float) (this.width - 3), 0.0f).texVanilla(170.0f, 20.0f, 3.0f, 3.0f).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, (float) (this.height - 6))
                    .at(0.0f, 3.0f).texVanilla(166.0f, 23.0f, 3.0f, 1.0f).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, (float) (this.height - 6))
                    .at((float) (this.width - 3), 3.0f).texVanilla(170.0f, 23.0f, 3.0f, 1.0f).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, 3.0f)
                    .at(0.0f, (float) (this.height - 3)).texVanilla(166.0f, 24.0f, 3.0f, 3.0f).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, (float) (this.width - 6), 3.0f)
                    .at(3.0f, (float) (this.height - 3)).texVanilla(169.0f, 24.0f, 1.0f, 3.0f).draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, matrixStack, 3.0f, 3.0f)
                    .at((float) (this.width - 3), (float) (this.height - 3))
                    .texVanilla(170.0f, 24.0f, 3.0f, 3.0f).draw();
            ScreenDrawHelper
                    .rect((IVertexBuilder) buf, matrixStack, (float) (this.width - 6),
                            (float) (this.height - 6))
                    .at(3.0f, 3.0f).texVanilla(169.0f, 23.0f, 1.0f, 1.0f).draw();
        });
    }
}
