...
public class VaultPartyOverlay {
    ...
    private static int renderEternalHUD(final MatrixStack matrixStack, final float offsetY, final int right) {
        ...
        final ITextComponent vpText = (ITextComponent) new StringTextComponent("Eternals")
    ...
    private static int renderEternalSection(final MatrixStack matrixStack,
            final ActiveEternalData.ActiveEternal eternal) {
        ...
        if (dead) {
            ...
            AbstractGui.blit(matrixStack, 0, 0, (float) heartU, (float) heartV, heartSize, heartSize, 256,
                    256);
            matrixStack.translate((double) (-gap), 0.0, 0.0);
            txt.append((ITextComponent) new StringTextComponent((int) eternal.getHealth() + "x")
    ...
    private static int renderPartyHUD(final MatrixStack matrixStack, final float offsetY, final int right) {
        ...
        matrixStack.pushPose();
        matrixStack.scale(0.8f, 0.8f, 1.0f);
        final ITextComponent vpText = (ITextComponent) new StringTextComponent("Vault Party")
    ...
    private static int renderPartyPlayerSection(final MatrixStack matrixStack, final VaultPartyData.Party party,
            final UUID playerUUID, final NetworkPlayerInfo info) {
        final int textSize = 8;
        final int headSize = 16;
        final int gap = 2;
        final boolean offline = info == null;
        final ClientPartyData.PartyMember member = offline ? null
                : ClientPartyData.getCachedMember(info.getProfile().getId());
        ...
        if (offline) {
            txt.append(
                    (ITextComponent) new StringTextComponent(prefix + "OFFLINE").withStyle(TextFormatting.GRAY));
        } else {
            final int heartSize = 9;
            final int heartU = 86;
            final int heartV = 2;
            matrixStack.translate((double) (-heartSize), 0.0, 0.0);
            Minecraft.getInstance().getTextureManager().bind(VaultPartyOverlay.VAULT_HUD_SPRITE);
            final ClientPartyData.PartyMember.Status status = (member == null)
                    ? ClientPartyData.PartyMember.Status.NORMAL
                    : member.status;
            AbstractGui.blit(matrixStack, 0, 0, (float) (heartU + getPartyPlayerStatusOffset(status)),
                    (float) heartV, heartSize, heartSize, 256, 256);
            matrixStack.translate((double) (-gap), 0.0, 0.0);
            txt.append(
                    (ITextComponent) new StringTextComponent((member == null) ? "-" : ((int) member.healthPts + "x"))
                            .withStyle(TextFormatting.WHITE));
        }
    ...
    public static void render2DHead(final MatrixStack matrixStack, final ResourceLocation skin, final int size,
            final boolean grayscaled) {
