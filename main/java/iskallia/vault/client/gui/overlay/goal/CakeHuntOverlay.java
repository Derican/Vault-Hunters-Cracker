...
public class CakeHuntOverlay {
    ...
    public static void onArchitectBuild(final RenderGameOverlayEvent.Post event) {
        ...
        if (data instanceof CakeHuntData) {
            ...
            ITextComponent txt = (ITextComponent) new StringTextComponent("Find the cakes!")
                    .withStyle(TextFormatting.AQUA).withStyle(TextFormatting.BOLD);
            fr.drawInBatch(txt.getVisualOrderText(), 8.0f, (float) (bottom - 54), -1, true,
                    renderStack.last().pose(), (IRenderTypeBuffer) buffer, false, 0,
                    LightmapHelper.getPackedFullbrightCoords());
            txt = (ITextComponent) new StringTextComponent(
                    displayData.getFoundCakes() + " / " + displayData.getTotalCakes())
                    .withStyle(TextFormatting.AQUA).withStyle(TextFormatting.BOLD);
            fr.drawInBatch(txt.getVisualOrderText(), 12.0f, (float) (bottom - 44), -1, true,
                    renderStack.last().pose(), (IRenderTypeBuffer) buffer, false, 0,
                    LightmapHelper.getPackedFullbrightCoords());
            buffer.endBatch();
            ...
            ScreenDrawHelper.drawQuad(buf -> {
                ScreenDrawHelper.rect((IVertexBuilder) buf, renderStack).at(15.0f, (float) (bottom - 31))
                        .dim(54.0f, 7.0f).texVanilla(0.0f, 105.0f, 54.0f, 7.0f).draw();
                ScreenDrawHelper.rect((IVertexBuilder) buf, renderStack).at(16.0f, (float) (bottom - 30))
