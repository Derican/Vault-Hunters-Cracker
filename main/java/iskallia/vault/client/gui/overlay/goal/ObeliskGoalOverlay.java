...
public class ObeliskGoalOverlay {
    ...
    public static void onObeliskRender(final RenderGameOverlayEvent.Post event) {
        ...
        if (data instanceof FinalArchitectGoalData) {
            ...
            renderObeliskIndicator(renderStack, displayData2.getKilledBosses(),
                    displayData2.getTotalKilledBossesNeeded());
            renderKnowledgeGatherIndicator(renderStack, (float) displayData2.getKnowledge(),
                    (float) displayData2.getTotalKnowledgeNeeded());
            renderStack.popPose();
        }
    ...
    private static void renderObeliskMessage(final MatrixStack matrixStack, final ITextComponent message) {
        ...
        fr.drawInBatch(bidiText, 0.0f, 0.0f, -1, true, matrixStack.last().pose(),
                (IRenderTypeBuffer) buffer, false, 0, LightmapHelper.getPackedFullbrightCoords());
        buffer.endBatch();
        matrixStack.popPose();
    }

    private static void renderKnowledgeGatherIndicator(final MatrixStack renderStack, final float knowledge,
            final float totalKnowledgeNeeded) {
        final Minecraft mc = Minecraft.getInstance();
        final int bottom = mc.getWindow().getGuiScaledHeight();
        ...
        ScreenDrawHelper.drawQuad(buf -> {
            ScreenDrawHelper.rect((IVertexBuilder) buf, renderStack).at(15.0f, (float) (bottom - 34))
                    .dim(perc, (float) height).texVanilla(0.0f, (float) (offsetY + height), perc, (float) height)
                    .draw();
            ScreenDrawHelper.rect((IVertexBuilder) buf, renderStack).at(15.0f, (float) (bottom - 34))
                    .dim((float) width, (float) height).texVanilla(0.0f, (float) offsetY, (float) width, (float) height)
                    .draw();
        });
    }

    private static void renderObeliskIndicator(final MatrixStack matrixStack, final int currentObelisks,
            final int maxObelisks) {
        if (maxObelisks <= 0) {
            return;
        }
        final Minecraft mc = Minecraft.getInstance();
