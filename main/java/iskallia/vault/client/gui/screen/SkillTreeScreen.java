...
public class SkillTreeScreen extends ContainerScreen<SkillTreeContainer> {
    ...
    public SkillTreeScreen(final SkillTreeContainer container, final PlayerInventory inventory,
    ...
    public Rectangle getTabBounds(final int index, final boolean active) {
        ...
        return new Rectangle(containerBounds.x + 5 + index * 31, containerBounds.y - 25 - (active ? 21 : 17), 28,
                active ? 32 : 25);
    }
    ...
    public void render(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        final Rectangle containerBounds = this.getContainerBounds();
        final List<Runnable> postRender = ((SkillTab) this.activeTabTpl.getA()).renderTab(containerBounds,
        ...
        final Rectangle dialogBounds = new Rectangle(x, y, this.width - 21 - x,
                this.height - 21 - y);
    ...
    private void renderSkillPointOverlay(final MatrixStack matrixStack) {
        if (VaultBarOverlay.unspentSkillPoints > 0) {
            ...
            final IReorderingProcessor bidiTxt = new StringTextComponent("")
                    .append(
                            (ITextComponent) new StringTextComponent(String.valueOf(VaultBarOverlay.unspentSkillPoints))
                                    .withStyle(TextFormatting.YELLOW))
                    .append(" unspent skill point" + ((VaultBarOverlay.unspentSkillPoints == 1) ? "" : "s"))
                    .getVisualOrderText();
            final int unspentWidth = mc.font.width(bidiTxt) + 5;
            mc.font.drawShadow(matrixStack, bidiTxt,
                    (float) (mc.getWindow().getGuiScaledWidth() - unspentWidth), 18.0f, -1);
        }
    }
        ...
        if (VaultBarOverlay.unspentKnowledgePoints > 0) {
            ...
            final IReorderingProcessor bidiTxt = new StringTextComponent("")
                    .append((ITextComponent) new StringTextComponent(
                            String.valueOf(VaultBarOverlay.unspentKnowledgePoints)).withStyle(TextFormatting.AQUA))
                    .append(
                            " unspent knowledge point" + ((VaultBarOverlay.unspentKnowledgePoints == 1) ? "" : "s"))
                    .getVisualOrderText();
            final int unspentWidth = mc.font.width(bidiTxt) + 5;
    ...
    private void renderVaultLevelBar(final MatrixStack matrixStack) {
        ...
        FontHelper.drawStringWithBorder(matrixStack, text, (float) (barX - textWidth - 1), (float) (barY - 1), -6601,
                -12698050);
    }
    ...
    private void renderContainerTabs(final MatrixStack matrixStack) {
        ...
        for (int i = 0; i < this.dialogs.size(); ++i) {
            ...
            this.blit(matrixStack, tabBounds.x, tabBounds.y, 63, active ? 28 : 0, tabBounds.width,
                    tabBounds.height);
            this.blit(matrixStack, tabBounds.x + 6, containerBounds.y - 25 - 11, uv.x, uv.y, 16, 16);
        }
        this.getMinecraft().font.draw(matrixStack,
                ((SkillTab) this.activeTabTpl.getA()).getTabName(), (float) containerBounds.x,
                (float) (containerBounds.y - 12), -12632257);
    }
