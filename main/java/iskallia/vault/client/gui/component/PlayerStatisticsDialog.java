...
public class PlayerStatisticsDialog extends ComponentDialog {
    ...
    private void createGearSlots() {
        ...
        final int startY = 6;
        this.slots.add(new ReadOnlySlot((IInventory) player.inventory, player.inventory.selected, startX,
                startY));
        ...
        for (final EquipmentSlotType slotType : EquipmentSlotType.values()) {
            if (slotType.getType() == EquipmentSlotType.Group.ARMOR) {
                this.slots
                        .add(new ArmorViewSlot(player, slotType, startX, startY + 36 + slotType.getIndex() * 18));
            }
        }
    }
    ...
    public Rectangle getStatBoxBounds() {
        final Rectangle ctBounds = this.getFavourBoxBounds();
        return new Rectangle(5, ctBounds.y + ctBounds.height + 5, this.bounds.width - 12,
                this.bounds.height - ctBounds.height - 16);
    }
    ...
    private void renderPlayerFavour(final MatrixStack matrixStack) {
        ...
        for (final PlayerFavourData.VaultGodType vgType2 : PlayerFavourData.VaultGodType.values()) {
            final IFormattableTextComponent name = new StringTextComponent(vgType2.getName())
                    .withStyle(vgType2.getChatColor());
            ...
            if (drawTitles) {
                final IFormattableTextComponent title = new StringTextComponent(vgType2.getTitle())
                        .withStyle(vgType2.getChatColor());
        ...
        for (final PlayerFavourData.VaultGodType vgType2 : PlayerFavourData.VaultGodType.values()) {
            ...
            fr.drawShadow(matrixStack, (ITextComponent) new StringTextComponent(String.valueOf(favour)), 0.0f, 0.0f,
                    -1052689);
    ...
    private void renderPlayerAttributes(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
        final Rectangle plBounds = this.getStatBoxBounds();
        final FontRenderer fr = Minecraft.getInstance().font;
        int maxLength = 0;
        final List<PlayerStatisticsCollector.AttributeSnapshot> snapshots = ClientStatisticsData
                .getPlayerAttributeSnapshots();
        ...
        for (int j = 0; j < snapshots.size(); ++j) {
            ...
            if (snapshot3.hasHitLimit()) {
                ...
                txt = new StringTextComponent(limitStr)
                        .withStyle(style -> style.withColor(Color.fromRgb(-8519680)));
            } else {
                txt = new StringTextComponent(numberStr)
                        .withStyle(style -> style.withColor(Color.fromRgb(-15130590)));
            }
            ...
            matrixStack.popPose();
            matrixStack.popPose();
            final Rectangle bounds = new Rectangle(this.bounds.x + offset.x + 10,
                    this.bounds.y + offset.y + 10 * j + 10 - this.descriptionComponent.getyOffset(),
                    maxLength + displayLength, 8);
            if (bounds.contains(mouseX, mouseY)) {
                if (snapshot3.hasHitLimit()) {
                    ...
                    GuiUtils.drawHoveringText(matrixStack, (List) list, offsetX, offsetY,
                            offset.x + plBounds.width - 14, offset.y + plBounds.height, -1, fr);
                    ...
                    GuiUtils.drawHoveringText(matrixStack, (List) list2, offsetX2, offsetY2,
                            offset.x + plBounds.width - 14, offset.y + plBounds.height, -1, fr);
                }
            }
        }
    ...
    private void renderPlayer(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
    ...
    private void renderPlayerItems(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
        ...
        if (hoveredSlot != null) {
            ...
            final List<ITextComponent> tooltip = toHover.getTooltipLines(
                    (PlayerEntity) Minecraft.getInstance().player,
                    (ITooltipFlag) (Minecraft.getInstance().options.advancedItemTooltips
            ...
            GuiUtils.drawHoveringText(matrixStack, (List) tooltip, mouseX - this.bounds.x, mouseY - this.bounds.y,
                    mainScreen.width - this.bounds.x, mainScreen.height - this.bounds.y, -1,
                    (fr == null) ? Minecraft.getInstance().font : fr);
    ...
    private void drawSlot(final MatrixStack matrixStack, final Slot slot) {
        ...
        if (slotStack.isEmpty()) {
            final Pair<ResourceLocation, ResourceLocation> pair = (Pair<ResourceLocation, ResourceLocation>) slot
                    .getNoItemIcon();
            if (pair != null) {
                final TextureAtlasSprite textureatlassprite = Minecraft.getInstance()
                        .getTextureAtlas((ResourceLocation) pair.getFirst()).apply(pair.getSecond());
