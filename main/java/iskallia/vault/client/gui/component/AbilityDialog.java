...
public class AbilityDialog extends ComponentDialog {
    ...
    public void refreshWidgets() {
        if (this.selectedAbility != null) {
            ...
            final AbilityNode<?, ?> existingNode = this.abilityTree
                    .getNodeOf(AbilityRegistry.getAbility(this.selectedAbility));
            ...
            if (targetAbilityNode.getSpecialization() != null) {
                ...
                activeState = (existingNode.getSpecialization() == null && existingNode.isLearned()
                        && VaultBarOverlay.vaultLevel >= ((AbilityConfig) targetAbilityNode.getAbilityConfig())
                                .getLevelRequirement());
            } else {
                if (!targetAbilityNode.isLearned()) {
                    buttonText = "Learn (" + targetAbilityGroup.learningCost() + ")";
                } else {
                    buttonText = ((existingNode.getLevel() >= targetAbilityGroup.getMaxLevel()) ? "Fully Learned"
                            : ("Upgrade (" + targetAbilityGroup.levelUpCost(targetAbilityNode.getSpecialization(),
                                    targetAbilityNode.getLevel()) + ")"));
                }
                pressAction = (button -> this.upgradeAbility());
                final AbilityConfig ability = (AbilityConfig) targetAbilityNode.getAbilityConfig();
                final int cost = (ability == null) ? targetAbilityGroup.learningCost()
                        : targetAbilityGroup.levelUpCost(targetAbilityNode.getSpecialization(),
                                targetAbilityNode.getLevel());
            ...
            this.selectButton = new Button(10, this.bounds.height - 40, this.bounds.width - 30, 20,
                    (ITextComponent) new StringTextComponent(buttonText), pressAction, (button, matrixStack, x, y) -> {
                    });
    ...
    private void upgradeAbility() {
        final AbilityNode<?, ?> abilityNode = this.abilityTree
                .getNodeByName(AbilityRegistry.getAbility(this.selectedAbility).getAbilityGroupName());
        if (abilityNode.getLevel() >= abilityNode.getGroup().getMaxLevel()) {
            return;
        }
        Minecraft.getInstance().player.playSound(
                abilityNode.isLearned() ? ModSounds.SKILL_TREE_UPGRADE_SFX : ModSounds.SKILL_TREE_LEARN_SFX, 1.0f,
                1.0f);
    ...
    private void renderHeading(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        if (targetAbilityNode.getSpecialization() != null) {
            ...
            abilityName = targetAbilityNode.getGroup().getParentName() + ": "
                    + targetAbilityNode.getSpecializationName();
        ...
        FontHelper.drawStringWithBorder(matrixStack, abilityName, (float) (abilityBounds.width + gap), 13.0f,
                learned ? -1849 : -1, learned ? -12897536 : -16777216);
    ...
    private void renderDescriptions(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        text.append((ITextComponent) ModConfigs.SKILL_DESCRIPTIONS
                .getDescriptionFor(this.selectedAbilityWidget.getAbilityName()));
        if (targetAbilityNode.getSpecialization() == null) {
    ...
    private ITextComponent getAdditionalDescription(final AbilityNode<?, ?> abilityNode) {
        ...
        final ITextComponent arrowTxt = (ITextComponent) new StringTextComponent(" " + arrow + " ")
        ...
        for (int lvl2 = 0; lvl2 < abilityNode.getGroup().getMaxLevel(); ++lvl2) {
            ...
            final int levelRequirement = ((AbilityConfig) abilityNode.getGroup().getAbilityConfig(null, lvl2))
                    .getLevelRequirement();
    ...
    private void renderFooter(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        if (targetAbilityNode.getSpecialization() == null && targetAbilityNode.isLearned()
