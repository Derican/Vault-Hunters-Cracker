...
public abstract class ComponentDialog extends AbstractGui {
    ...
    public void mouseScrolled(final double mouseX, final double mouseY, final double delta) {
        if (this.bounds == null || this.descriptionComponent == null || this.descriptionComponent.bounds == null
                || !this.descriptionComponent.bounds.contains((int) mouseX - this.bounds.x,
                        (int) mouseY - this.bounds.y)) {
            return;
        }
    ...
    protected void renderBackground(final MatrixStack matrixStack, final int mouseX, final int mouseY,
            final float partialTicks) {
        Minecraft.getInstance().getTextureManager().bind(SkillTreeScreen.UI_RESOURCE);
        fill(matrixStack, this.bounds.x + 5, this.bounds.y + 5, this.bounds.x + this.bounds.width - 5,
                this.bounds.y + this.bounds.height - 5, -3750202);
        ...
        this.blit(matrixStack, this.bounds.x + this.bounds.width - 5, this.bounds.y + this.bounds.height - 5,
                8, 52, 5, 5);
