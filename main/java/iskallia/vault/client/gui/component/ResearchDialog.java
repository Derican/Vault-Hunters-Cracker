...
public class ResearchDialog extends ComponentDialog {
    ...
    public void refreshWidgets() {
        if (this.researchName != null) {
            ...
            final String buttonText = this.researchTree.isResearched(this.researchName) ? "Researched"
                    : ("Research (" + researchCost + ")");
            this.selectButton = new Button(10, this.bounds.height - 40, this.bounds.width - 30, 20,
                    (ITextComponent) new StringTextComponent(buttonText), button -> this.research(),
            ...
            this.selectButton.active = (!this.researchTree.isResearched(this.researchName) && !isLocked
                    && (research.usesKnowledge() ? (VaultBarOverlay.unspentKnowledgePoints >= researchCost)
                            : (VaultBarOverlay.unspentSkillPoints >= researchCost)));
        }
    }
    ...
    public void research() {
        ...
        final int unspentPoints = research.usesKnowledge() ? VaultBarOverlay.unspentKnowledgePoints
    ...
    private void renderHeading(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        final int contentWidth = abilityBounds.width + gap
                + Math.max(fontRenderer.width(this.researchName), fontRenderer.width(subText));
        matrixStack.pushPose();
        matrixStack.translate(10.0, 0.0, 0.0);
        FontHelper.drawStringWithBorder(matrixStack, this.researchName, (float) (abilityBounds.width + gap), 13.0f,
                researched ? -1849 : -1, researched ? -12897536 : -16777216);
        ...
        matrixStack.translate(-this.researchWidget.getRenderWidth() / 2.0,
                -this.researchWidget.getRenderHeight() / 2.0, 0.0);
    ...
    private void renderDescriptions(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        final IFormattableTextComponent description = ModConfigs.SKILL_DESCRIPTIONS
                .getDescriptionFor(this.researchName);
        final int renderedLineCount = UIHelper.renderWrappedText(matrixStack, (ITextComponent) description,
    ...
    private void renderFooter(final MatrixStack matrixStack, final int mouseX, final int mouseY,
        ...
        final boolean researched = this.researchTree.getResearchesDone().contains(this.researchName);
        if (!researched) {
            this.blit(matrixStack, 13, this.bounds.height - 40 - 2,
