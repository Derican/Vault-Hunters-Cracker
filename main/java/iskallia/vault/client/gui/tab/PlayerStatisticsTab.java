...
public class PlayerStatisticsTab extends SkillTab
{
    ...
    private int addVaultStat(final List<Tuple<ITextComponent, Tuple<ITextComponent, Integer>>> out, final String key, final String value, final String valueLengthStr, int currentMaxOffset) {
        ...
        final Tuple<ITextComponent, Integer> valueDisplayTpl = (Tuple<ITextComponent, Integer>)new Tuple((Object)new StringTextComponent(value), (Object)valueStrLength);
        out.add((Tuple<ITextComponent, Tuple<ITextComponent, Integer>>)new Tuple((Object)new TranslationTextComponent("stat.the_vault." + key), (Object)valueDisplayTpl));
    ...
    private void renderBookBackground(final Rectangle containerBounds, final MatrixStack renderStack) {
        ...
        ScreenDrawHelper.draw(7, DefaultVertexFormats.POSITION_COLOR_TEX, buf -> ScreenDrawHelper.rect((IVertexBuilder)buf, renderStack, (float)containerBounds.width, (float)containerBounds.height).at((float)containerBounds.x, (float)containerBounds.y).texVanilla(0.0f, 0.0f, 192.0f, 192.0f).draw());
    }
}
