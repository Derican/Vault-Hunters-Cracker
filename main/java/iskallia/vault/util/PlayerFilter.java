...
public class PlayerFilter implements Predicate<UUID>
{
    ...
    public static PlayerFilter of(final PlayerEntity... players) {
        return new PlayerFilter((List<UUID>)Arrays.stream(players).map((Function<? super PlayerEntity, ?>)Entity::getUUID).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()));
    }
    
    public static PlayerFilter of(final VaultPlayer... players) {
        return new PlayerFilter((List<UUID>)Arrays.stream(players).map((Function<? super VaultPlayer, ?>)VaultPlayer::getPlayerId).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()));
    }
