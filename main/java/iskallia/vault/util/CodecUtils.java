...
public class CodecUtils
{
    ...
    public static <T> Optional<T> readNBT(final Codec<T> codec, final INBT nbt) {
        return codec.parse((DynamicOps)NBTDynamicOps.INSTANCE, (Object)nbt).resultOrPartial((Consumer)Vault.LOGGER::error);
    }
    ...
    public static <T> void writeNBT(final Codec<T> codec, final T value, final Consumer<INBT> successConsumer) {
        codec.encodeStart((DynamicOps)NBTDynamicOps.INSTANCE, (Object)value).resultOrPartial((Consumer)Vault.LOGGER::error).ifPresent(successConsumer);
    }
    
    public static <T> INBT writeNBT(final Codec<T> codec, final T value) {
        return (INBT)codec.encodeStart((DynamicOps)NBTDynamicOps.INSTANCE, (Object)value).getOrThrow(false, (Consumer)Vault.LOGGER::error);
    }
}
