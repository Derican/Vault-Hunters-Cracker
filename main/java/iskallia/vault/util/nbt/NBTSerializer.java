...
public class NBTSerializer {
    public static final <T extends INBTSerializable> CompoundNBT serialize(final T object)
    ...
    private static final <T, U extends T> INBT objectToTag(final Class<T> clazz, final U obj)
            throws IllegalAccessException, UnserializableClassException {
        if (obj == null) {
            return null;
        }
    ...
    private static final <T> ListNBT serializeCollection(final Collection<T> col)
            throws IllegalAccessException, UnserializableClassException {
    ...
    private static final <K, V> CompoundNBT serializeEntry(final Map.Entry<K, V> entry)
    ...
    private static final <K, V> CompoundNBT serializeEntry(final Map.Entry<? extends K, ? extends V> entry,
    ...
    public static final <T extends INBTSerializable> T deserialize(final Class<T> definition, final CompoundNBT data)
    ...
    public static final <T extends INBTSerializable> void deserialize(final T instance, final CompoundNBT data,
            final boolean interpretMissingFieldValuesAsNull)
            throws IllegalAccessException, InstantiationException, UnserializableClassException {
        final Field[] declaredFields;
    ...
    private static <T> Collection<T> deserializeCollection(final ListNBT list,
            final Class<? extends Collection> colClass, final Class<T> subclass, final Type subtype)
    ...
    private static <K, V> Map<K, V> deserializeMap(final ListNBT map, final Class<? extends Map> mapClass,
            final Class<K> keyClass, final Type keyType, final Class<V> valueClass, final Type valueType)
            throws InstantiationException, IllegalAccessException, UnserializableClassException {
    ...
    private static <T> T tagToObject(final INBT tag, final Class<T> clazz, final Type subtype)
            throws IllegalAccessException, InstantiationException, UnserializableClassException {
    ...
    private static int getIDFromClass(final Class<?> clazz) {
        if (clazz.isAssignableFrom(Byte.TYPE) || clazz.isAssignableFrom(Byte.class)
                || clazz.isAssignableFrom(Boolean.TYPE) || clazz.isAssignableFrom(Boolean.class)) {
            return 1;
        }
