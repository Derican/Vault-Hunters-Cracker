...
public class ResistanceHelper {
    public static float getPlayerResistancePercent(final ServerPlayerEntity player) {
        return MathHelper.clamp(getPlayerResistancePercentUnlimited(player), 0.0f,
                AttributeLimitHelper.getResistanceLimit((PlayerEntity) player));
    }
    ...
    public static float getGearResistanceChance(final Function<EquipmentSlotType, ItemStack> gearProvider) {
        ...
        for (final EquipmentSlotType slot : EquipmentSlotType.values()) {
            ...
            if (!(stack.getItem() instanceof VaultGear)
                ...
                resistancePercent += ModAttributes.EXTRA_RESISTANCE.get(stack)
                        .map(attribute -> attribute.getValue(stack)).orElse(0.0f);
                resistancePercent += ModAttributes.ADD_EXTRA_RESISTANCE.get(stack)
                        .map(attribute -> attribute.getValue(stack)).orElse(0.0f);
            }
        }
