...
public class AbsorptionHelper
{
    public static float getMaxAbsorption(final PlayerEntity player) {
        if (MiscUtils.getTalent(player, ModConfigs.TALENTS.BARBARIC).map((Function<? super TalentNode<BarbaricTalent>, ? extends Boolean>)TalentNode::isLearned).orElse(false)) {
            return 0.0f;
        }
        float limit = 12.0f;
        float maxHealthPerc = 0.0f;
        maxHealthPerc += MiscUtils.getTalent(player, ModConfigs.TALENTS.BARRIER).map((Function<? super TalentNode<AbsorptionTalent>, ?>)TalentNode::getTalent).map((Function<? super Object, ? extends Float>)AbsorptionTalent::getIncreasedAbsorptionLimit).orElse(0.0f);
