...
public class PlayerDamageHelper {
    ...
    public static DamageMultiplier applyMultiplier(final ServerPlayerEntity player, final float value,
            final Operation operation) {
        return applyMultiplier(player, value, operation, true);
    }

    public static DamageMultiplier applyMultiplier(final ServerPlayerEntity player, final float value,
            final Operation operation, final boolean showOnClient) {
        return applyMultiplier(player, value, operation, showOnClient, Integer.MAX_VALUE);
    }

    public static DamageMultiplier applyMultiplier(final ServerPlayerEntity player, final float value,
            final Operation operation, final boolean showOnClient, final int tickDuration) {
    ...
    public static DamageMultiplier applyMultiplier(final ServerPlayerEntity player, final float value,
            final Operation operation, final boolean showOnClient, final int tickDuration,
            final Consumer<ServerPlayerEntity> onTimeout) {
        return applyMultiplier(player.getServer(), player.getUUID(), new DamageMultiplier(
                player.getUUID(), value, operation, showOnClient, tickDuration, (Consumer) onTimeout));
    }

    public static DamageMultiplier applyMultiplier(final int id, final ServerPlayerEntity player, final float value,
            final Operation operation) {
    ...
    public static DamageMultiplier applyMultiplier(final int id, final ServerPlayerEntity player, final float value,
            final Operation operation, final boolean showOnClient, final int tickDuration,
            final Consumer<ServerPlayerEntity> onTimeout) {
        return applyMultiplier(player.getServer(), player.getUUID(), new DamageMultiplier(id,
                player.getUUID(), value, operation, showOnClient, tickDuration, (Consumer) onTimeout));
    }

    private static DamageMultiplier applyMultiplier(final MinecraftServer srv, final UUID playerId,
            final DamageMultiplier multiplier) {
    ...
    public static Optional<DamageMultiplier> getMultiplier(final ServerPlayerEntity player, final int id) {
        return PlayerDamageHelper.multipliers.getOrDefault(player.getUUID(), new HashSet<DamageMultiplier>())
                .stream().filter(m -> m.id == id).findFirst();
    }
    ...
    public static boolean removeMultiplier(final ServerPlayerEntity player, final int id) {
        return getMultiplier(player, id).filter(damageMultiplier -> removeMultiplier(player, damageMultiplier))
                .isPresent();
    }

    public static boolean removeMultiplier(final MinecraftServer srv, final UUID playerId,
            final DamageMultiplier multiplier) {
        final boolean removed = PlayerDamageHelper.multipliers.getOrDefault(playerId, new HashSet<DamageMultiplier>())
                .remove(multiplier);
        if (removed) {
    ...
    private static float getDamageMultiplier(final UUID playerId, final boolean ignoreClientFlag) {
        final Set<DamageMultiplier> damageMultipliers = PlayerDamageHelper.multipliers.getOrDefault(playerId,
                new HashSet<DamageMultiplier>());
    ...
    public static void sync(final ServerPlayerEntity sPlayer) {
        ...
        ModNetwork.CHANNEL.sendTo((Object) new PlayerDamageMultiplierMessage(multiplier),
                sPlayer.connection.connection, NetworkDirection.PLAY_TO_CLIENT);
    }
    ...
    public static class DamageMultiplier {
        ...
        private DamageMultiplier(final UUID playerId, final float value, final Operation operation,
                final boolean showOnClient, final int tickTimeout, final Consumer<ServerPlayerEntity> onTimeout) {
            this(DamageMultiplier.counter++ & Integer.MAX_VALUE, playerId, value, operation, showOnClient, tickTimeout,
                    onTimeout);
        }

        private DamageMultiplier(final int id, final UUID playerId, final float value, final Operation operation,
                final boolean showOnClient, final int tickTimeout, final Consumer<ServerPlayerEntity> onTimeout) {
            this.removed = false;
            this.id = id;
