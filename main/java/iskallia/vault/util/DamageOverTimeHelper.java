...
public class DamageOverTimeHelper {
    ...
    public static void applyDamageOverTime(final LivingEntity target, final DamageSource damageSource,
            final float totalDamage, final int seconds) {
        ServerScheduler.INSTANCE.schedule(1, () -> {
            final DamageOverTimeEntry entry = new DamageOverTimeEntry(seconds * 20, damageSource,
                    target.getId(), totalDamage / seconds);
            DamageOverTimeHelper.worldEntries.computeIfAbsent(
                    (RegistryKey<World>) target.getCommandSenderWorld().dimension(), key -> new ArrayList()).add(entry);
        });
    }
    ...
    public static List<DamageOverTimeEntry> getDotEntries(final Entity entity) {
        ...
        final List<DamageOverTimeEntry> allEntries = DamageOverTimeHelper.worldEntries
                .get(entityWorld.dimension());
    ...
    public static void onWorldTick(final TickEvent.WorldTickEvent event) {
        ...
        final List<DamageOverTimeEntry> entries = DamageOverTimeHelper.worldEntries
                .computeIfAbsent((RegistryKey<World>) world.dimension(), key -> new ArrayList());
    ...
    private static class DamageOverTimeEntry {
        ...
        public DamageOverTimeEntry(final int ticks, final DamageSource source, final int entityId,
