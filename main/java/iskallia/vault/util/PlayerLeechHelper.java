...
public class PlayerLeechHelper {
    ...
    public static void onLivingHurt(final LivingDamageEvent event) {
        ...
        if (ActiveFlags.IS_AOE_ATTACKING.isSet() || ActiveFlags.IS_DOT_ATTACKING.isSet()
    ...
    private static void leechHealth(final LivingEntity attacker, final float amountLeeched) {
        ...
        if (attacker.getRandom().nextFloat() <= 0.2) {
            ...
            attacker.getCommandSenderWorld().playSound((PlayerEntity) null, attacker.getX(),
                    attacker.getY(), attacker.getZ(), ModSounds.VAMPIRE_HISSING_SFX,
                    SoundCategory.MASTER, 0.020000001f, pitch);
        }
    }
}
