...
public class EntityHelper {
    ...
    public static void changeHealth(final LivingEntity entity, final int healthChange) {
        ...
        if (entity.isDeadOrDying()) {
            entity.die(
                    (entity.getLastDamageSource() != null) ? entity.getLastDamageSource() : DamageSource.GENERIC);
        }
    }
    ...
    public static <T extends Entity> List<T> getNearby(final IWorld world, final Vector3i pos, final float radius,
            final Class<T> entityClass) {
        final AxisAlignedBB selectBox = EntityHelper.BOX
                .move((double) pos.getX(), (double) pos.getY(), (double) pos.getZ())
                .inflate((double) radius);
        return world.getEntitiesOfClass((Class) entityClass, selectBox,
