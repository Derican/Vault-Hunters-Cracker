...
public class MiscUtils {
    ...
    public static <T> List<T> concat(final List<T> list1, final T... elements) {
        return Stream.concat(list1.stream(), (Stream<?>) Arrays.stream(elements))
    ...
    public static <T extends PlayerTalent> Optional<TalentNode<T>> getTalent(final PlayerEntity player,
    ...
    public static List<ItemStack> mergeItemStacks(final List<ItemStack> stacks) {
        ...
        Label_0015: for (final ItemStack stack : stacks) {
            if (stack.isEmpty()) {
                continue;
            }
    ...
    public static BlockPos getRandomPos(final AxisAlignedBB box, final Random r) {
        ...
        return new BlockPos(box.minX + r.nextInt(sizeX), box.minY + r.nextInt(sizeY),
                box.minZ + r.nextInt(sizeZ));
    }

    public static Vector3d getRandomOffset(final AxisAlignedBB box, final Random r) {
        return new Vector3d(box.minX + r.nextFloat() * (box.maxX - box.minX),
                box.minY + r.nextFloat() * (box.maxY - box.minY),
                box.minZ + r.nextFloat() * (box.maxZ - box.minZ));
    }

    public static Vector3d getRandomOffset(final BlockPos pos, final Random r) {
        return new Vector3d((double) (pos.getX() + r.nextFloat()),
                (double) (pos.getY() + r.nextFloat()), (double) (pos.getZ() + r.nextFloat()));
    }
    ...
    public static Collection<ChunkPos> getChunksContaining(final AxisAlignedBB box) {
        return getChunksContaining(new Vector3i(box.minX, box.minY, box.minZ),
    ...
    public static int getOverlayColor(final ItemStack stack) {
        ...
        return Minecraft.getInstance().getBlockColors().getColor(state, (IBlockDisplayReader) null,
                (BlockPos) null, 0);
    }
    ...
    public static void giveItem(final ServerPlayerEntity player, ItemStack stack) {
        ...
        if (player.inventory.add(stack) && stack.isEmpty()) {
            ...
            player.level.playSound((PlayerEntity) null, player.getX(), player.getY(),
                    player.getZ(), SoundEvents.ITEM_PICKUP, SoundCategory.PLAYERS, 0.2f,
    ...
    public static Vector3f getRandomCirclePosition(final Vector3f centerOffset, final Vector3f axis,
            final float radius) {
        return getCirclePosition(centerOffset, axis, radius, (float) (Math.random() * 360.0));
    }

    public static Vector3f getCirclePosition(final Vector3f centerOffset, final Vector3f axis, final float radius,
            final float degree) {
        Vector3f circleVec = normalize(perpendicular(axis));
        circleVec = new Vector3f(circleVec.x() * radius, circleVec.y() * radius,
                circleVec.z() * radius);
        final Quaternion rotQuat = new Quaternion(axis, degree, true);
        circleVec.transform(rotQuat);
        return new Vector3f(circleVec.x() + centerOffset.x(),
                circleVec.y() + centerOffset.y(),
                circleVec.z() + centerOffset.z());
    }

    public static Vector3f normalize(final Vector3f vec) {
        final float lengthSq = vec.x() * vec.x() + vec.y() * vec.y()
                + vec.z() * vec.z();
