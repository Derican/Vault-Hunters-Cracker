...
public class PlayerSet {
    ...
    public static boolean allMatch(final LivingEntity player, final BiPredicate<EquipmentSlotType, ItemStack> predicate,
            final EquipmentSlotType... slots) {
        return Arrays.stream(slots).allMatch(slot -> predicate.test(slot, player.getItemBySlot(slot)));
    }

    public static boolean allMatch(final LivingEntity player,
            final BiPredicate<EquipmentSlotType, ItemStack> predicate) {
        return allMatch(player, predicate, EquipmentSlotType.HEAD, EquipmentSlotType.CHEST, EquipmentSlotType.LEGS,
