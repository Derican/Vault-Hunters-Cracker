...
public class SetTree implements INBTSerializable<CompoundNBT>
{
    ...
    public SetTree tick(final MinecraftServer server) {
        NetcodeUtils.runIfPresent(server, this.uuid, player -> {
            ...
            final List<SetNode<?>> toRemove = this.nodes.stream().filter(SetNode::isActive).filter(setNode -> !setNode.getSet().shouldBeActive((LivingEntity)player)).collect((Collector<? super Object, ?, List<SetNode<?>>>)Collectors.toList());
