...
public class TalentTree {
    ...
    public TalentTree(final UUID uuid) {
        ...
        this.uuid = uuid;
        this.add(null, (TalentNode<?>[]) ModConfigs.TALENTS.getAll().stream()
                .map(talentGroup -> new TalentNode(talentGroup, 0)).toArray(TalentNode[]::new));
    }
    ...
    public List<TalentNode<?>> getLearnedNodes() {
        return this.getNodes().stream().filter(TalentNode::isLearned)
                .collect((Collector<? super Object, ?, List<TalentNode<?>>>) Collectors.toList());
    }
    ...
    public <T extends PlayerTalent> List<TalentNode<T>> getLearnedNodes(final Class<T> talentGroupType) {
        return this.getNodes().stream().filter(TalentNode::isLearned)
                ...
                .map(node -> node).collect((Collector<? super Object, ?, List<TalentNode<T>>>) Collectors.toList());
    }

    public boolean hasLearnedNode(final TalentGroup<?> talentGroup) {
        return this.getLearnedNodes().stream()
                .anyMatch(node -> node.getGroup().getParentName().equals(talentGroup.getParentName()));
    }

    public <T extends PlayerTalent> Collection<T> getTalents(final Class<T> talentType) {
        return this.getNodes().stream().filter(TalentNode::isLearned)
                .map((Function<? super Object, ?>) TalentNode::getTalent)
                .filter(talent -> talentType.isAssignableFrom(talent.getClass())).map(talent -> talent)
                .collect((Collector<? super Object, ?, Collection<T>>) Collectors.toList());
    }
    ...
    public <T extends PlayerTalent> TalentNode<T> getNodeByName(final String name) {
        final Optional<TalentNode<T>> talentWrapped = this.nodes.stream()
                .filter(node -> node.getGroup().getParentName().equals(name)).map(node -> node).findFirst();
    ...
    public TalentTree upgradeTalent(final MinecraftServer server, final TalentNode<?> talentNode) {
        ...
        final TalentNode<?> upgradedTalentNode = new TalentNode<Object>(talentNode.getGroup(),
                talentNode.getLevel() + 1);
    ...
    public TalentTree tick(final MinecraftServer server) {
        NetcodeUtils.runIfPresent(server, this.uuid, player -> this.nodes.stream().filter(TalentNode::isLearned)
                .forEach(node -> node.getTalent().tick((PlayerEntity) player)));
        return this;
    }
    ...
    public void syncTree(final MinecraftServer server) {
        NetcodeUtils.runIfPresent(server, this.uuid,
                player -> ModNetwork.CHANNEL.sendTo((Object) new KnownTalentsMessage(this),
    ...
    public void deserialize(final CompoundNBT nbt, final boolean migrateData) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            final TalentNode<?> talent = TalentNode.fromNBT(this.uuid, list.getCompound(i),
                    migrateData ? currentVersion : 2);
            if (talent != null) {
    ...
    @Nullable
    static TalentNode<?> migrate(@Nullable final UUID playerId, final String talentName, int talentLevel,
            final int vFrom) {
        ...
        if (vFrom <= 0 && (talentName.equalsIgnoreCase("Commander") || talentName.equalsIgnoreCase("Ward")
                || talentName.equalsIgnoreCase("Barbaric") || talentName.equalsIgnoreCase("Frenzy")
