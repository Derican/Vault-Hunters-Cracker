...
public class AbilityTree implements INBTSerializable<CompoundNBT>
{
    ...
    public AbilityTree(final UUID uuid) {
        ...
        this.uuid = uuid;
        this.add(null, (Collection<AbilityNode<?, ?>>)ModConfigs.ABILITIES.getAll().stream().map(abilityGroup -> new AbilityNode(abilityGroup.getParentName(), 0, null)).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()));
    }
    ...
    public List<AbilityNode<?, ?>> getLearnedNodes() {
        return this.getNodes().stream().filter(AbilityNode::isLearned).sorted((Comparator<? super Object>)AbilityTree.ABILITY_COMPARATOR).collect((Collector<? super Object, ?, List<AbilityNode<?, ?>>>)Collectors.toList());
    }
    ...
    public void upgradeAbility(final MinecraftServer server, final AbilityNode<?, ?> abilityNode) {
        ...
        final AbilityNode<?, ?> upgradedAbilityNode = new AbilityNode<Object, Object>(abilityNode.getGroup().getParentName(), abilityNode.getLevel() + 1, abilityNode.getSpecialization());
    ...
    public void downgradeAbility(final MinecraftServer server, final AbilityNode<?, ?> abilityNode) {
        ...
        final AbilityNode<?, ?> downgradedAbilityNode = new AbilityNode<Object, Object>(abilityNode.getGroup().getParentName(), Math.max(targetLevel, 0), abilityNode.getSpecialization());
    ...
    public void syncTree(final MinecraftServer server) {
        NetcodeUtils.runIfPresent(server, this.uuid, player -> ModNetwork.CHANNEL.sendTo((Object)new AbilityKnownOnesMessage(this), player.connection.connection, NetworkDirection.PLAY_TO_CLIENT));
    }
    ...
    public void syncFocusedIndex(final MinecraftServer server) {
        ...
        NetcodeUtils.runIfPresent(server, this.uuid, player -> ModNetwork.CHANNEL.sendTo((Object)new AbilityFocusMessage(selected.getGroup()), player.connection.connection, NetworkDirection.PLAY_TO_CLIENT));
    }
    ...
    public void notifyActivity(final MinecraftServer server) {
        ...
        NetcodeUtils.runIfPresent(server, this.uuid, player -> this.notifyActivity(server, selected.getGroup(), this.cooldowns.getOrDefault(selected, 0), ModConfigs.ABILITIES.getCooldown(selected, player), this.active));
    }
    ...
    public void notifyActivity(final MinecraftServer server, @Nonnull final AbilityGroup<?, ?> ability, final int cooldown, final int maxCooldown, final ActivityFlag activeFlag) {
        NetcodeUtils.runIfPresent(server, this.uuid, player -> ModNetwork.CHANNEL.sendTo((Object)new AbilityActivityMessage(ability, cooldown, maxCooldown, activeFlag), player.connection.connection, NetworkDirection.PLAY_TO_CLIENT));
    }
