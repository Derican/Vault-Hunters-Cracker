...
public class DashAbility<C extends DashConfig> extends AbilityEffect<C> {
    ...
    public boolean onAction(final C config, final ServerPlayerEntity player, final boolean active) {
        ...
        Vector3d dashVector = new Vector3d(lookVector.x(), lookVector.y(),
                lookVector.z());
        ...
        if (GrasshopperNinja.isGrasshopperShape((PlayerEntity) player)) {
            player.level.playSound((PlayerEntity) player, player.getX(),
                    player.getY(), player.getZ(), ModSounds.GRASSHOPPER_BRRR,
            ...
            player.level.playSound((PlayerEntity) player, player.getX(),
                    player.getY(), player.getZ(), ModSounds.DASH_SFX, SoundCategory.PLAYERS, 0.2f,
                    1.0f);
