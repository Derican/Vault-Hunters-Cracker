...
public abstract class EffectAbility extends AbilityEffect<EffectConfig> {
    ...
    public void onTick(final EffectConfig config, final PlayerEntity player, final boolean active) {
        if (!active) {
            ...
            final EffectInstance newEffect = new EffectInstance(config.getEffect(), Integer.MAX_VALUE,
    ...
    private void playEffects(final EffectConfig config, final PlayerEntity player) {
        if (config.getEffect() == Effects.INVISIBILITY) {
            player.level.playSound(player, player.getX(), player.getY(),
                    player.getZ(), ModSounds.INVISIBILITY_SFX, SoundCategory.MASTER, 0.175f, 1.0f);
            ...
            player.level.playSound(player, player.getX(), player.getY(),
                    player.getZ(), ModSounds.NIGHT_VISION_SFX, SoundCategory.MASTER, 0.0375f, 1.0f);
