...
public class ExecuteAbility<C extends ExecuteConfig> extends AbilityEffect<C> {
    ...
    public boolean onAction(final C config, final ServerPlayerEntity player, final boolean active) {
        ...
        final EffectInstance newEffect = new EffectInstance(config.getEffect(), config.getEffectDuration(),
        ...
        player.level.playSound((PlayerEntity) player, player.getX(), player.getY(),
                player.getZ(), ModSounds.EXECUTION_SFX, SoundCategory.PLAYERS, 0.4f, 1.0f);
    ...
    public void onDamage(final LivingDamageEvent event) {
        ...
        player.level.playSound((PlayerEntity) null, player.getX(), player.getY(),
                player.getZ(), SoundEvents.EVOKER_CAST_SPELL, SoundCategory.MASTER, 1.0f, 1.0f);
