...
public class VaultRaid implements INBTSerializable<CompoundNBT> {
    ...
    public VaultRaid(final VaultGenerator generator, final VaultTask initializer, final RaidProperties properties,
            final List<VaultEvent<?>> events, final Iterable<VaultPlayer> players) {
        this.timer = new VaultTimer().start(Integer.MAX_VALUE);
        this.modifiers = new VaultModifiers();
        this.properties = new RaidProperties();
        this.influence = new VaultInfluences();
    ...
    public <T extends VaultModifier> List<T> getActiveModifiersFor(final PlayerFilter filter,
            final Class<T> modifierClass) {
        final List<T> modifiers = this.getModifiers().stream()
                .filter(modifier -> modifierClass.isAssignableFrom(modifier.getClass())).map(modifier -> modifier)
        ...
        for (final VaultPlayer player : this.getPlayers()) {
            if (!(player instanceof VaultRunner) && filter.test(player.getPlayerId())) {
                player.getModifiers().stream().filter(modifier -> modifierClass.isAssignableFrom(modifier.getClass()))
                        .map(modifier -> modifier).forEach(modifiers::add);
            }
        }
        return modifiers;
    }
    ...
    public <T extends VaultObjective> Optional<T> getObjective(final Class<T> objectiveClass) {
        return this.getAllObjectives().stream()
                .filter(objective -> objectiveClass.isAssignableFrom(objective.getClass())).findFirst()
                .map(vaultObjective -> vaultObjective);
    }

    public boolean hasActiveObjective(final VaultPlayer player, final Class<? extends VaultObjective> objectiveClass) {
        return this.getActiveObjective(objectiveClass).isPresent()
                || player.getActiveObjective(objectiveClass).isPresent();
    }
    ...
    public boolean isFinished() {
        return !this.getProperties().getBaseOrDefault(VaultRaid.FORCE_ACTIVE, false)
                && (this.players.isEmpty() || this.players.stream().allMatch(VaultPlayer::hasExited));
    }
    ...
    public static VaultRaid classic(final VaultGenerator generator, final VaultTask initializer,
            final RaidProperties properties, final VaultObjective objective, final List<VaultEvent<?>> events,
            final Map<VaultPlayerType, Set<ServerPlayerEntity>> playersMap) {
        final MinecraftServer srv = (MinecraftServer) LogicalSidedProvider.INSTANCE.get(LogicalSide.SERVER);
        final VaultRaid vault = new VaultRaid(generator, initializer, properties, events,
                (Iterable<VaultPlayer>) playersMap.entrySet().stream().flatMap(entry -> {
                    ...
                    if (type == VaultPlayerType.RUNNER) {
                        return players.stream().map(player -> {
                            ...
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(
                                            VaultRaid.IS_RUNNER.and(VaultRaid.IS_DEAD
                                                    .or(VaultRaid.NO_TIME_LEFT.and(VaultRaid.OBJECTIVES_LEFT))),
                                            VaultRaid.RUNNER_TO_SPECTATOR));
                            runner.getBehaviours().add(new VaultBehaviour(
                                    VaultRaid.NO_OBJECTIVES_LEFT_GLOBALLY
                                            .and(VaultRaid.NO_TIME_LEFT.or(VaultRaid.NO_RUNNERS_LEFT)),
                                    VaultRaid.REMOVE_SCAVENGER_ITEMS.then(VaultRaid.REMOVE_INVENTORY_RESTORE_SNAPSHOTS)
                                            .then(VaultRaid.EXIT_SAFELY)));
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(
                                            VaultRaid.OBJECTIVES_LEFT_GLOBALLY.and(VaultRaid.NO_RUNNERS_LEFT),
                                            VaultRaid.REMOVE_SCAVENGER_ITEMS
                                                    .then(VaultRaid.SAVE_SOULBOUND_GEAR.then(VaultRaid.EXIT_DEATH))));
                            runner.getBehaviours().add(new VaultBehaviour(VaultRaid.IS_FINISHED.negate(),
                                    VaultRaid.TICK_SPAWNER.then(VaultRaid.TICK_CHEST_PITY)));
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(
                                            VaultRaid.AFTER_GRACE_PERIOD.and(VaultRaid.IS_FINISHED.negate()),
                                            VaultRaid.TICK_INFLUENCES));
                            runner.getBehaviours().add(new VaultBehaviour(VaultRaid.IS_RUNNER,
                                    VaultRaid.PAUSE_IN_ARENA.then(VaultRaid.CHECK_BAIL)));
                            ...
                            runner.getTimer()
                                    .start(objective.getVaultTimerStart(ModConfigs.VAULT_GENERAL.getTickCounter()));
                            return runner;
                        });
                    } else {
        ...
        vault.getAllObjectives()
                .add(objective.thenComplete(VaultRaid.LEVEL_UP_GEAR).thenComplete(VaultRaid.VICTORY_SCENE));
        vault.getAllObjectives().forEach(obj -> obj.initialize(srv, vault));
        return vault;
    }

    public static VaultRaid coop(final VaultGenerator generator, final VaultTask initializer,
            final RaidProperties properties, final VaultObjective objective, final List<VaultEvent<?>> events,
            final Map<VaultPlayerType, Set<ServerPlayerEntity>> playersMap) {
        final MinecraftServer srv = (MinecraftServer) LogicalSidedProvider.INSTANCE.get(LogicalSide.SERVER);
        final VaultRaid vault = new VaultRaid(generator, initializer, properties, events,
                (Iterable<VaultPlayer>) playersMap.entrySet().stream().flatMap(entry -> {
                    ...
                    if (type == VaultPlayerType.RUNNER) {
                        return players.stream().map(player -> {
                            ...
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(
                                            VaultRaid.NO_OBJECTIVES_LEFT_GLOBALLY.and(VaultRaid.NO_TIME_LEFT),
                                            VaultRaid.REMOVE_SCAVENGER_ITEMS
                                                    .then(VaultRaid.REMOVE_INVENTORY_RESTORE_SNAPSHOTS)
                                                    .then(VaultRaid.GRANT_EXP_COMPLETE.then(VaultRaid.EXIT_SAFELY))));
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(
                                            VaultRaid.IS_RUNNER.and(VaultRaid.IS_DEAD.or(VaultRaid.NO_TIME_LEFT)),
                                            VaultRaid.EXIT_DEATH_ALL));
                            runner.getBehaviours().add(new VaultBehaviour(VaultRaid.IS_FINISHED.negate(),
                                    VaultRaid.TICK_SPAWNER.then(VaultRaid.TICK_CHEST_PITY)));
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(
                                            ...
                                            VaultRaid.TICK_INFLUENCES));
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(VaultRaid.IS_RUNNER, VaultRaid.CHECK_BAIL_COOP));
                            runner.getBehaviours()
                                    .add(new VaultBehaviour(VaultRaid.NO_ACTIVE_RUNNERS_LEFT,
                                            VaultRaid.REMOVE_SCAVENGER_ITEMS
                                                    .then(VaultRaid.REMOVE_INVENTORY_RESTORE_SNAPSHOTS)
                                                    .then(VaultRaid.GRANT_EXP_BAIL.then(VaultRaid.EXIT_SAFELY))));
                            runner.getProperties().create(VaultRaid.SPAWNER, new VaultSpawner());
                            runner.getProperties().create(VaultRaid.CHEST_PITY, new VaultChestPity());
                            runner.getTimer()
                                    .start(objective.getVaultTimerStart(ModConfigs.VAULT_GENERAL.getTickCounter()));
    ...
    public static VaultRaid lobby(final VaultGenerator generator, final VaultTask initializer,
            final RaidProperties properties, final VaultObjective objective, final List<VaultEvent<?>> events,
            final Map<VaultPlayerType, Set<ServerPlayerEntity>> playersMap) {
        final MinecraftServer srv = (MinecraftServer) LogicalSidedProvider.INSTANCE.get(LogicalSide.SERVER);
        final VaultRaid vault = new VaultRaid(generator, initializer, properties, events,
                (Iterable<VaultPlayer>) playersMap.entrySet().stream().flatMap(entry -> {
                    ...
                    return players.stream().map(player -> {
                        ...
                        member.getBehaviours()
                                .add(new VaultBehaviour(VaultRaid.IS_DEAD.negate(), VaultRaid.TICK_LOBBY));
                        return member;
                    });
                }).collect((Collector<? super Object, ?, List<? super Object>>) Collectors.toList()));
    ...
    public static VaultRaid boss(final VaultGenerator generator, final VaultTask initializer,
            final RaidProperties properties, final VaultObjective objective, final List<VaultEvent<?>> events,
            final Map<VaultPlayerType, Set<ServerPlayerEntity>> playersMap) {
        final MinecraftServer srv = (MinecraftServer) LogicalSidedProvider.INSTANCE.get(LogicalSide.SERVER);
        final VaultRaid vault = new VaultRaid(generator, initializer, properties, events,
                (Iterable<VaultPlayer>) playersMap.entrySet().stream().flatMap(entry -> {
                    ...
                    return players.stream().map(player -> {
                        ...
                        runner.getBehaviours().add(new VaultBehaviour(VaultRaid.IS_DEAD.and(VaultRaid.IS_RUNNER),
                                VaultRaid.RUNNER_TO_SPECTATOR));
                        runner.getBehaviours()
                                .add(new VaultBehaviour(VaultRaid.NO_OBJECTIVES_LEFT_GLOBALLY, VaultRaid.EXIT_SAFELY));
    ...
    public static Builder builder(final VaultLogic logic, final int vaultLevel,
    ...
    static {
        ...
        RAID_CHALLENGE_GENERATOR = VaultGenerator
                .register(() -> new RaidChallengeGenerator(Vault.id("raid_challenge")));
        ...
        DIMENSION = new VAttribute<RegistryKey<World>, RegistryKeyAttribute<World>>(Vault.id("dimension"),
                RegistryKeyAttribute::new);
        BOUNDING_BOX = new VAttribute<MutableBoundingBox, BoundingBoxAttribute>(Vault.id("bounding_box"),
                BoundingBoxAttribute::new);
        START_POS = new VAttribute<BlockPos, BlockPosAttribute>(Vault.id("start_pos"), BlockPosAttribute::new);
        START_FACING = new VAttribute<Direction, EnumAttribute<Direction>>(Vault.id("start_facing"),
                () -> new EnumAttribute((Class<Enum>) Direction.class));
        CRYSTAL_DATA = new VAttribute<CrystalData, CompoundAttribute<CrystalData>>(Vault.id("crystal_data"),
                () -> CompoundAttribute.of(CrystalData::new));
        ...
        LOBBY = new VAttribute<VaultLobby, CompoundAttribute<VaultLobby>>(Vault.id("lobby"),
                () -> CompoundAttribute.of(VaultLobby::new));
        ...
        SPAWNER = new VAttribute<VaultSpawner, CompoundAttribute<VaultSpawner>>(Vault.id("spawner"),
                () -> CompoundAttribute.of(VaultSpawner::new));
        CHEST_PITY = new VAttribute<VaultChestPity, CompoundAttribute<VaultChestPity>>(Vault.id("chest_pity"),
                () -> CompoundAttribute.of(VaultChestPity::new));
        SAND_EVENT = new VAttribute<VaultSandEvent, CompoundAttribute<VaultSandEvent>>(Vault.id("sand_event"),
                () -> CompoundAttribute.of(VaultSandEvent::new));
        ...
        IS_RUNNER = VaultCondition.register(Vault.id("is_runner"),
                (vault, player, world) -> player instanceof VaultRunner);
        IS_SPECTATOR = VaultCondition.register(Vault.id("is_spectator"),
                (vault, player, world) -> player instanceof VaultSpectator);
        IS_OUTSIDE = VaultCondition.register(Vault.id("is_outside"), (vault, player, world) -> {
            ...
            player.runIfPresent(world.getServer(),
                    sPlayer -> outside[0] = !VaultUtils.inVault(vault, (Entity) sPlayer));
            return outside[0];
        });
        AFTER_GRACE_PERIOD = VaultCondition.register(Vault.id("after_grace_period"),
                (vault, player, world) -> vault.getTimer().getRunTime() > 300);
        ...
        TIME_LEFT = VaultCondition.register(Vault.id("time_left"),
                (vault, player, world) -> player.getTimer().getTimeLeft() > 0);
        NO_TIME_LEFT = VaultCondition.register(Vault.id("no_time_left"), VaultRaid.TIME_LEFT.negate());
        OBJECTIVES_LEFT = VaultCondition.register(Vault.id("objectives_left"),
                (vault, player, world) -> player.getObjectives().size() > 0 || vault.getActiveObjectives().size() > 0);
        NO_OBJECTIVES_LEFT = VaultCondition.register(Vault.id("no_objectives_left"),
                VaultRaid.OBJECTIVES_LEFT.negate());
        OBJECTIVES_LEFT_GLOBALLY = VaultCondition.register(Vault.id("objectives_left_globally"),
                (vault, player, world) -> vault.players.stream()
                        .anyMatch(player1 -> VaultRaid.OBJECTIVES_LEFT.test(vault, player1, world)));
        NO_OBJECTIVES_LEFT_GLOBALLY = VaultCondition.register(Vault.id("no_objectives_left_globally"),
                VaultRaid.OBJECTIVES_LEFT_GLOBALLY.negate());
        RUNNERS_LEFT = VaultCondition.register(Vault.id("runners_left"),
                (vault, player, world) -> vault.players.stream().anyMatch(player1 -> player1 instanceof VaultRunner));
        NO_RUNNERS_LEFT = VaultCondition.register(Vault.id("no_runners_left"), VaultRaid.RUNNERS_LEFT.negate());
        ACTIVE_RUNNERS_LEFT = VaultCondition.register(Vault.id("active_runners_left"),
                (vault, player, world) -> vault.players.stream()
                        .anyMatch(player1 -> player1 instanceof VaultRunner && !player1.hasExited()));
        NO_ACTIVE_RUNNERS_LEFT = VaultCondition.register(Vault.id("no_active_runners_left"),
                VaultRaid.ACTIVE_RUNNERS_LEFT.negate());
            ...
            if (vault.getTimer().getRunTime() < 200) {
                ...
                player.runIfPresent(world.getServer(), sPlayer -> {
                    if (!vault.getGenerator().getPiecesAt(sPlayer.blockPosition(), VaultStart.class).isEmpty()) {
                        ...
                        final BlockPos min = new BlockPos(box.minX + 0.001, box.minY + 0.001,
                                box.minZ + 0.001);
                        final BlockPos max = new BlockPos(box.maxX - 0.001, box.maxY - 0.001,
                                box.maxZ - 0.001);
                            ...
                            for (int xx = min.getX(); xx <= max.getX(); ++xx) {
                                for (int yy = min.getY(); yy <= max.getY(); ++yy) {
                                    for (int zz = min.getZ(); zz <= max.getZ(); ++zz) {
                                        final BlockState state = world
                                                .getBlockState((BlockPos) pos.set(xx, yy, zz));
                                        if (state.getBlock() == ModBlocks.VAULT_PORTAL) {
                                            if (sPlayer.isOnPortalCooldown()) {
                                                ...
                                                vault.getAllObjectives().forEach(
                                                        objective -> objective.notifyBail(vault, player, world));
                                                sPlayer.setPortalCooldown();
                                                VaultRaid.REMOVE_SCAVENGER_ITEMS
                                                        .then(VaultRaid.REMOVE_INVENTORY_RESTORE_SNAPSHOTS)
                                                        .then(VaultRaid.EXIT_SAFELY).execute(vault, player, world);
                                                final IFormattableTextComponent playerName = sPlayer.getDisplayName()
                                                        .copy();
                                                playerName.setStyle(Style.EMPTY
                                                        .withColor(Color.fromRgb(9974168)));
                                                ...
                                                world.getServer().getPlayerList().broadcastMessage(
                                                        (ITextComponent) new StringTextComponent("")
                                                                .append((ITextComponent) playerName)
                                                                .append((ITextComponent) suffix),
            ...
            if (vault.getTimer().getRunTime() < 200) {
                ...
                player.runIfPresent(world.getServer(), sPlayer -> {
                    if (!vault.getGenerator().getPiecesAt(sPlayer.blockPosition(), VaultStart.class).isEmpty()) {
                        ...
                        final BlockPos min2 = new BlockPos(box2.minX + 0.001, box2.minY + 0.001,
                                box2.minZ + 0.001);
                        final BlockPos max2 = new BlockPos(box2.maxX - 0.001, box2.maxY - 0.001,
                                box2.maxZ - 0.001);
                            ...
                            for (int i = min2.getX(); i <= max2.getX(); ++i) {
                                for (int j = min2.getY(); j <= max2.getY(); ++j) {
                                    for (int k = min2.getZ(); k <= max2.getZ(); ++k) {
                                        final BlockState state2 = world
                                                .getBlockState((BlockPos) pos2.set(i, j, k));
                                        if (state2.getBlock() == ModBlocks.VAULT_PORTAL) {
                                            if (sPlayer.isOnPortalCooldown()) {
                                                sPlayer.setPortalCooldown();
                                                return;
                                            } else if (!vault.canExit(player)) {
                                                final StringTextComponent text2 = new StringTextComponent(
                                                        "You cannot exit this Vault!");
                                                text2.setStyle(Style.EMPTY
                                                        .withColor(Color.fromRgb(16711680)));
                                                ...
                                                final IFormattableTextComponent playerName2 = sPlayer.getDisplayName()
                                                        .copy();
                                                playerName2.setStyle(Style.EMPTY
                                                        .withColor(Color.fromRgb(9974168)));
                                                final StringTextComponent suffix2 = new StringTextComponent(" bailed.");
                                                world.getServer().getPlayerList().broadcastMessage(
                                                        (ITextComponent) new StringTextComponent("")
                                                                .append((ITextComponent) playerName2)
                                                                .append((ITextComponent) suffix2),
                                                        ChatType.CHAT, player.getPlayerId());
                                            }
                                        }
                                    }
        ...
        CHECK_BAIL_FINAL = VaultTask.register(Vault.id("check_bail_final"),
                (vault, player, world) -> player.runIfPresent(world.getServer(), sPlayer -> {
                    if (!vault.getGenerator().getPiecesAt(sPlayer.blockPosition(), VaultStart.class).isEmpty()) {
                        final AxisAlignedBB box3 = sPlayer.getBoundingBox();
                        final BlockPos min3 = new BlockPos(box3.minX + 0.001, box3.minY + 0.001,
                                box3.minZ + 0.001);
                        final BlockPos max3 = new BlockPos(box3.maxX - 0.001, box3.maxY - 0.001,
                                box3.maxZ - 0.001);
                            ...
                            for (int l = min3.getX(); l <= max3.getX(); ++l) {
                                for (int m = min3.getY(); m <= max3.getY(); ++m) {
                                    for (int k2 = min3.getZ(); k2 <= max3.getZ(); ++k2) {
                                        final BlockState state3 = world
                                                .getBlockState((BlockPos) pos3.set(l, m, k2));
                                            ...
                                            if (sPlayer.isOnPortalCooldown()) {
                                                ...
                                                new ArrayList(vault.getPlayers()).forEach(vaultPlayer -> {
                                                    if (vaultPlayer instanceof VaultRunner) {
                                                        VaultRaid.RUNNER_TO_SPECTATOR.execute(vault, vaultPlayer,
                                                                world);
                                                    }
                                                    return;
                                                });
                                                ...
                                                VaultRaid.HIDE_OVERLAY.execute(vault, player, world);
                                                final IFormattableTextComponent playerName3 = sPlayer.getDisplayName()
                                                        .copy();
                                                playerName3.setStyle(Style.EMPTY
                                                        .withColor(Color.fromRgb(9974168)));
                                                final StringTextComponent suffix3 = new StringTextComponent(" bailed.");
                                                world.getServer().getPlayerList().broadcastMessage(
                                                        (ITextComponent) new StringTextComponent("")
                                                                .append((ITextComponent) playerName3)
                                                                .append((ITextComponent) suffix3),
                                                        ChatType.CHAT, player.getPlayerId());
                                            }
                                        }
                                    }
        ...
        TICK_SAND_EVENT = VaultTask.register(Vault.id("tick_sand_event"), (vault, player, world) -> vault
                .getProperties().getBase(VaultRaid.SAND_EVENT).ifPresent(event -> event.execute(vault, player, world)));
        TICK_CHEST_PITY = VaultTask.register(Vault.id("tick_chest_pity"), (vault, player, world) -> player
                .getProperties().getBase(VaultRaid.CHEST_PITY).ifPresent(event -> event.execute(vault, player, world)));
            ...
            if (vault.getActiveObjectives().isEmpty()) {
                ...
                player.getProperties().get(VaultRaid.SPAWNER).ifPresent(attribute -> {
                    ...
                    if (player.getTimer().getRunTime() >= 300) {
                        ...
                        spawner.configure(config -> config.withStartMaxMobs(c.getStartMaxMobs())
                                .withMinDistance(c.getMinDistance()).withMaxDistance(c.getMaxDistance())
        ...
        TICK_LOBBY = VaultTask.register(Vault.id("tick_lobby"),
                (vault, player, world) -> vault.getProperties().get(VaultRaid.LOBBY).ifPresent(attribute -> {
        ...
        TP_TO_START = VaultTask.register(Vault.id("tp_to_start"),
                (vault, player, world) -> player.runIfPresent(world.getServer(), playerEntity -> {
                    final BlockPos start = vault.getProperties().getBaseOrDefault(VaultRaid.START_POS, (BlockPos) null);
                    final Direction facing = vault.getProperties().getBaseOrDefault(VaultRaid.START_FACING,
                            (Direction) null);
                    ...
                    if (start == null) {
                        ...
                        playerEntity.teleportTo(world, (double) (box4.x0 + box4.getXSpan() / 2.0f),
                                256.0, (double) (box4.z0 + box4.getZSpan() / 2.0f),
                                playerEntity.yRot, playerEntity.xRot);
                    } else {
                        playerEntity.teleportTo(world, start.getX() + 0.5, start.getY() + 0.2,
                                start.getZ() + 0.5,
                                (facing == null) ? (world.getRandom().nextFloat() * 360.0f)
                                        : facing.getClockWise().toYRot(),
                                0.0f);
                    }
        ...
        INIT_LEVEL_COOP = VaultTask.register(Vault.id("init_level_coop"),
                (vault, player, world) -> vault.getProperties().getBase(VaultRaid.HOST).ifPresent(hostId -> {
        ...
        INIT_RELIC_TIME = VaultTask.register(Vault.id("init_relic_extension"), (vault, player, world) -> {
            ...
            sets.stream().map(ResourceLocation::new).forEach(set -> player.getTimer().addTime(
                    new RelicSetExtension(RelicSet.REGISTRY.get(set), ModConfigs.VAULT_RELICS.getExtraTickPerSet()),
                    0));
            return;
        });
        ...
        INIT_SANDS_EVENT = VaultTask.register(Vault.id("init_sand_event"), (vault, player, world) -> {
            if (ModConfigs.SAND_EVENT.isEnabled()) {
                ...
                player.getBehaviours()
                        .add(new VaultBehaviour(VaultRaid.IS_FINISHED.negate(), VaultRaid.TICK_SAND_EVENT));
            }
            return;
        });
        INIT_COW_VAULT = VaultTask.register(Vault.id("init_cow_vault"), (vault, player, world) -> {
            if (!vault.getProperties().exists(VaultRaid.COW_VAULT)) {
                final CrystalData crystalData = vault.getProperties().getBase(VaultRaid.CRYSTAL_DATA)
                        .orElse(CrystalData.EMPTY);
                if (!crystalData.getType().canBeCowVault() || crystalData.getSelectedObjective() != null
                        || !crystalData.getModifiers().isEmpty()
                        || vault.getProperties().getBaseOrDefault(VaultRaid.IS_RAFFLE, false)) {
                    ...
                    if (isCowVault) {
                        ...
                        final SummonAndKillBossObjective objective2 = new SummonAndKillBossObjective(
                                Vault.id("summon_and_kill_boss"));
                        vault.getAllObjectives().add(
                                objective2.thenComplete(VaultRaid.LEVEL_UP_GEAR).thenComplete(VaultRaid.VICTORY_SCENE));
                    }
                }
            }
        ...
        INIT_GLOBAL_MODIFIERS = VaultTask.register(Vault.id("init_global_modifiers"), (vault, player, world) -> {
            ...
            if (!vault.getModifiers().isInitialized()) {
                final CrystalData crystalData2 = vault.getProperties().getBase(VaultRaid.CRYSTAL_DATA)
                        .orElse(CrystalData.EMPTY);
        ...
        HIDE_OVERLAY = VaultTask.register(Vault.id("hide_overlay"),
                (vault, player, world) -> player.sendIfPresent(world.getServer(), VaultOverlayMessage.hide()));
        ...
        REMOVE_SCAVENGER_ITEMS = VaultTask.register(Vault.id("remove_scavenger_items"),
                (vault, player, world) -> player.runIfPresent(world.getServer(), playerEntity -> {
                    ...
                    for (int slot2 = 0; slot2 < inventory.getContainerSize(); ++slot2) {
                        ...
                        final LazyOptional itemHandler = stack2
                        ...
                        if (stack2.getItem() instanceof BlockItem
                                && ((BlockItem) stack2.getItem()).getBlock() instanceof VaultCrateBlock) {
                            final CompoundNBT tag = stack2.getTagElement("BlockEntityTag");
                            if (tag != null) {
                                final NonNullList stacks = NonNullList.withSize(54,
        ...
        SAVE_SOULBOUND_GEAR = VaultTask.register(Vault.id("save_soulbound_gear"),
                (vault, player, world) -> player.runIfPresent(world.getServer(), sPlayer -> {
        ...
        REMOVE_INVENTORY_RESTORE_SNAPSHOTS = VaultTask.register(Vault.id("remove_inventory_snapshots"),
                (vault, player, world) -> {
        ...
        FINAL_VICTORY_SCENE = VaultTask.register(Vault.id("final_victory_scene"), (vault, player, world) -> {
            if (player instanceof VaultRunner) {
                ...
                player.runIfPresent(world.getServer(), playerEntity -> {
                    new FireworkRocketEntity((World) world, playerEntity.getX(),
                            playerEntity.getY(), playerEntity.getZ(),
                            new ItemStack((IItemProvider) Items.FIREWORK_ROCKET));
                    ...
                    world.playSound((PlayerEntity) null, playerEntity.getX(),
                            playerEntity.getY(), playerEntity.getZ(), SoundEvents.UI_TOAST_CHALLENGE_COMPLETE,
                    ...
                    if (vault.getProperties().getValue(VaultRaid.CRYSTAL_DATA)
                            .getType() == CrystalData.Type.FINAL_BOSS) {
        ...
        EXIT_SAFELY = VaultTask.register(Vault.id("exit_safely"),
                (vault, player, world) -> player.runIfPresent(world.getServer(), playerEntity -> {
                    ...
                    world.playSound((PlayerEntity) null, playerEntity.getX(),
                            playerEntity.getY(), playerEntity.getZ(),
                            ModSounds.VAULT_PORTAL_LEAVE, SoundCategory.PLAYERS, 1.0f, 1.0f);
                    world.playSound((PlayerEntity) null, (Entity) playerEntity, ModSounds.VAULT_PORTAL_LEAVE,
                    ...
                    if (parentVault != null) {
                        parentVault.getProperties().getBase(VaultRaid.LOBBY).ifPresent(lobby -> {
                            ...
                            VaultRaid.TP_TO_START.execute(parentVault, member, world.getServer().getLevel(
                                    (RegistryKey) parentVault.getProperties().getValue(VaultRaid.DIMENSION)));
                            if (vault.getActiveObjectives().stream().allMatch(VaultObjective::isCompleted)) {
                                ...
                                player.runIfPresent(world.getServer(),
                                        sPlayer -> lobby.snapshots.removeSnapshot((PlayerEntity) sPlayer));
                            } else {
                                player.runIfPresent(world.getServer(), sPlayer -> {
        ...
        EXIT_DEATH = VaultTask.register(Vault.id("exit_death"),
                (vault, player, world) -> player.runIfPresent(world.getServer(), playerEntity -> {
                    if (player instanceof VaultSpectator) {
                        playerEntity.gameMode.setGameModeForPlayer(((VaultSpectator) player).oldGameType);
                    }
                    world.playSound((PlayerEntity) null, playerEntity.getX(),
                            playerEntity.getY(), playerEntity.getZ(), ModSounds.TIMER_KILL_SFX,
                            SoundCategory.PLAYERS, 1.0f, 1.0f);
                    world.playSound((PlayerEntity) null, (Entity) playerEntity, ModSounds.TIMER_KILL_SFX,
                            SoundCategory.PLAYERS, 1.0f, 1.0f);
                    playerEntity.inventory.clearOrCountMatchingItems(stack -> true, -1,
                            (IInventory) playerEntity.inventoryMenu.getCraftSlots());
                    ...
                    if (parentVault2 != null) {
                        parentVault2.getProperties().getBase(VaultRaid.LOBBY).ifPresent(lobby -> {
                            ...
                            member2.getBehaviours()
                                    .add(new VaultBehaviour(VaultRaid.IS_OUTSIDE, VaultRaid.TP_TO_START));
                            member2.getBehaviours()
                                    .add(new VaultBehaviour(VaultRaid.IS_DEAD.negate(), VaultRaid.TICK_LOBBY));
                            parentVault.getPlayers().add(member2);
                            player.runIfPresent(world.getServer(),
                                    sPlayer -> lobby.snapshots.restoreSnapshot((PlayerEntity) sPlayer));
                            parentVault.getProperties().create(VaultRaid.LOBBY, lobby);
        ...
        EXIT_DEATH_ALL = VaultTask.register(Vault.id("exit_death_all"), (vault, player,
                world) -> new ArrayList(vault.players).forEach(vPlayer -> VaultRaid.REMOVE_SCAVENGER_ITEMS
                        .then(VaultRaid.SAVE_SOULBOUND_GEAR.then(VaultRaid.GRANT_EXP_DEATH.then(VaultRaid.EXIT_DEATH)))
                        .execute(vault, vPlayer, world)));
        EXIT_DEATH_ALL_NO_SAVE = VaultTask.register(Vault.id("exit_death_all_no_save"),
                (vault, player, world) -> new ArrayList(vault.players)
                        .forEach(vPlayer -> VaultRaid.REMOVE_SCAVENGER_ITEMS.then(VaultRaid.EXIT_DEATH).execute(vault,
                                vPlayer, world)));
        VICTORY_SCENE = VaultTask.register(Vault.id("victory_scene"), (vault, player, world) -> {
            if (player instanceof VaultRunner) {
                ...
                player.runIfPresent(world.getServer(), playerEntity -> {
                    ...
                    final STitlePacket titlePacket2 = new STitlePacket(STitlePacket.Type.TITLE,
                            (ITextComponent) title2);
                    final STitlePacket subtitlePacket = new STitlePacket(STitlePacket.Type.SUBTITLE,
                            (ITextComponent) subtitle2);
        ...
        ENTER_DISPLAY = VaultTask.register(Vault.id("enter_display"),
                (vault, player, world) -> player.runIfPresent(world.getServer(), playerEntity -> {
                    final CrystalData data2 = vault.getProperties().getBaseOrDefault(VaultRaid.CRYSTAL_DATA,
                            CrystalData.EMPTY);
                    if (!data2.getType().isFinalType()) {
                        ...
                        if (vault.canExit(player)) {
                            formattableTextComponent = new StringTextComponent("Good luck, ")
                                    .append(playerEntity.getName())
                                    .append((ITextComponent) new StringTextComponent("!"));
                        } else {
                            formattableTextComponent = new StringTextComponent("No exit this time, ")
                                    .append(playerEntity.getName())
                                    .append((ITextComponent) new StringTextComponent("!"));
                        }
                        final ITextComponent subtitle3 = (ITextComponent) formattableTextComponent;
                        ((IFormattableTextComponent) subtitle3)
                                .setStyle(Style.EMPTY.withColor(Color.fromRgb(14536734)));
                        final STitlePacket titlePacket3 = new STitlePacket(STitlePacket.Type.TITLE,
                                (ITextComponent) title3);
                    ...
                    Object vaultName = vault.getActiveObjectives().stream().findFirst()
                            .map((Function<? super Object, ? extends ITextComponent>) VaultObjective::getVaultName)
                            .orElse((ITextComponent) new StringTextComponent("Vault"));
                    ...
                    switch (data2.getType()) {
                        case FINAL_LOBBY: {
                            ...
                            vaultName = new StringTextComponent("the Final Vault")
                                    .withStyle(TextFormatting.DARK_PURPLE);
                            break;
                        }
                        case FINAL_BOSS: {
                            prefix = new StringTextComponent(" is facing ");
                            vaultName = new StringTextComponent("the Final Challenge")
                                    .withStyle(TextFormatting.DARK_RED);
                            break;
                        }
                        case FINAL_VELARA: {
                            prefix = new StringTextComponent(" entered ");
                            vaultName = new StringTextComponent("Velara's Gluttony")
                                    .withStyle(PlayerFavourData.VaultGodType.BENEVOLENT.getChatColor());
                            break;
                        }
                        case FINAL_TENOS: {
                            prefix = new StringTextComponent(" entered ");
                            vaultName = new StringTextComponent("Tenos' Puzzle")
                                    .withStyle(PlayerFavourData.VaultGodType.OMNISCIENT.getChatColor());
                            break;
                        }
                        case FINAL_WENDARR: {
                            prefix = new StringTextComponent(" entered ");
                            vaultName = new StringTextComponent("Wendarr's Passage")
                                    .withStyle(PlayerFavourData.VaultGodType.TIMEKEEPER.getChatColor());
                            break;
                        }
                        case FINAL_IDONA: {
                            prefix = new StringTextComponent(" entered ");
                            vaultName = new StringTextComponent("Idona's Wrath")
                                    .withStyle(PlayerFavourData.VaultGodType.MALEVOLENCE.getChatColor());
                            break;
                        }
                    }
                    if (!vault.getModifiers().isEmpty()) {
                    ...
                    if (vault.getProperties().getBaseOrDefault(VaultRaid.COW_VAULT, false)
                        ...
                        final StringTextComponent hoverText = new StringTextComponent(
                        ...
                        ((IFormattableTextComponent) txt).setStyle(Style.EMPTY
                                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (Object) hoverText))
                                .withColor(Color.fromRgb(9974168)));
                    ...
                    world.getServer().getPlayerList()
                            .broadcastMessage(
                                    (ITextComponent) playerName4.append((ITextComponent) prefix)
                                            .append((ITextComponent) text4),
                                    ChatType.CHAT, player.getPlayerId());
                }));
        SUMMON_AND_KILL_BOSS = VaultObjective
                .register(() -> new SummonAndKillBossObjective(Vault.id("summon_and_kill_boss")));
        ...
        SUMMON_AND_KILL_ALL_BOSSES = VaultObjective
                .register(() -> new SummonAndKillAllBossesObjective(Vault.id("summon_and_kill_all_bosses")));
        ARCHITECT_KILL_ALL_BOSSES = VaultObjective
                .register(() -> new ArchitectSummonAndKillBossesObjective(Vault.id("architect_kill_all_bosses")));
        ...
        SCALE_MOB = VaultEvent.register(Vault.id("scale_mob"), LivingEvent.LivingUpdateEvent.class,
                (BiConsumer<VaultRaid, LivingEvent.LivingUpdateEvent>) EntityScaler::scaleVaultEntity);
        SCALE_MOB_JOIN = VaultEvent.register(Vault.id("scale_mob_join"), EntityJoinWorldEvent.class,
                (BiConsumer<VaultRaid, EntityJoinWorldEvent>) EntityScaler::scaleVaultEntity);
        BLOCK_NATURAL_SPAWNING = VaultEvent.register(Vault.id("block_natural_spawning"),
                LivingSpawnEvent.CheckSpawn.class, (vault, event) -> {
        ...
        PREVENT_ITEM_PICKUP = VaultEvent.register(Vault.id("prevent_item_pickup"), EntityJoinWorldEvent.class,
                (vault, event) -> {
        ...
        REPLACE_WITH_COW = VaultEvent.register(Vault.id("replace_with_cow"), EntityJoinWorldEvent.class,
                (vault, event) -> {
                    if (!(event.getWorld() instanceof ServerWorld)) {
                        ...
                        if (entity.getTags().contains("replaced_entity")) {
                            ...
                            if (entity instanceof LivingEntity && !(entity instanceof PlayerEntity)) {
                                final AggressiveCowEntity replaced = VaultCowOverrides.replaceVaultEntity(vault,
                                        (LivingEntity) entity, (ServerWorld) event.getWorld());
                                if (replaced != null) {
                                    final Vector3d pos4 = entity.position();
                                    ((LivingEntity) replaced).absMoveTo(pos4.x, pos4.y,
                                            pos4.z, entity.yRot, entity.xRot);
                                    ServerScheduler.INSTANCE.schedule(1,
                                            () -> event.getWorld().addFreshEntity((Entity) replaced));
        ...
        APPLY_SCALE_MODIFIER = VaultEvent.register(Vault.id("apply_scale_modifier"), EntityJoinWorldEvent.class,
                (vault, event) -> {
                    if (!(event.getWorld() instanceof ServerWorld)) {
                        return;
                    } else if (!VaultUtils.inVault(vault, event.getEntity())) {
                        ...
                        vault.getActiveModifiersFor(PlayerFilter.any(), ScaleModifier.class).forEach(modifier -> entity
                                .getAttribute(ModAttributes.SIZE_SCALE).setBaseValue((double) modifier.getScale()));
                        return;
                    }
                });
        APPLY_FRENZY_MODIFIERS = VaultEvent.register(Vault.id("frenzy_modifiers"), EntityJoinWorldEvent.class,
                (vault, event) -> {
                    if (!(event.getWorld() instanceof ServerWorld)) {
                        return;
                    } else if (!VaultUtils.inVault(vault, event.getEntity())) {
                        ...
                        if (entity3.getTags().contains("frenzy_scaled")) {
                            ...
                            vault.getActiveModifiersFor(PlayerFilter.any(), FrenzyModifier.class)
                                    .forEach(modifier -> modifier.applyToEntity(entity));
                            entity3.getTags().add("frenzy_scaled");
                            return;
                        }
                    }
                });
        APPLY_INFLUENCE_MODIFIERS = VaultEvent.register(Vault.id("influence_modifiers"), EntityJoinWorldEvent.class,
                (vault, event) -> {
                    if (!(!(event.getWorld() instanceof ServerWorld))) {
                        if (!(!VaultUtils.inVault(vault, event.getEntity()))) {
                                ...
                                if (!(event.getEntity() instanceof PlayerEntity)) {
                                    if (!(event.getEntity() instanceof EternalEntity)) {
                                        ...
                                        if (!entity4.getTags().contains("influenced")) {
                                            vault.getInfluences().getInfluences(MobAttributeInfluence.class)
                                                    .forEach(influence -> influence.applyTo(entity));
    ...
    public static class Builder {
        ...
        public VaultRaid build() {
            return this.logic.getFactory().create((VaultGenerator) this.generator.get(), this.initializer,
                    this.attributes, this.objective, this.events, this.players);
        }
    }

    @FunctionalInterface
    public interface Factory {
        VaultRaid create(final VaultGenerator p0, final VaultTask p1, final RaidProperties p2, final VaultObjective p3,
                final List<VaultEvent<?>> p4, final Map<VaultPlayerType, Set<ServerPlayerEntity>> p5);
    }
}
