...
public abstract class VaultObjective implements INBTSerializable<CompoundNBT>, IVaultTask {
    ...
    protected NonNullList<ItemStack> createLoot(final ServerWorld world, final VaultRaid vault,
            final LootContext context) {
        final LootTable rewardLootTable = this.getRewardLootTable(vault,
                world.getServer().getLootTables()::get);
        ...
        vault.getPlayers().stream().filter(player -> player instanceof VaultRunner).findAny().ifPresent(vPlayer -> {
            ...
            final float pTimeLeft = MathHelper.clamp(1.0f - timer.getRunTime() / (float) timer.getTotalTime(),
                    0.0f, 1.0f);
    ...
    protected void addSpecialLoot(final ServerWorld world, final VaultRaid vault, final LootContext context,
        ...
        if (eternals > 0) {
            stacks.add((Object) new ItemStack((IItemProvider) ModItems.ETERNAL_SOUL,
                    Math.min(world.random.nextInt(eternals) + 1, 64)));
        }
        ...
        for (int traders = ModConfigs.SCALING_CHEST_REWARDS.traderCount(this.getId(), VaultRarity.COMMON, level),
                i = 0; i < traders; ++i) {
            stacks.add((Object) new ItemStack((IItemProvider) ModItems.TRADER_CORE));
        }
        for (int statues = ModConfigs.SCALING_CHEST_REWARDS.statueCount(this.getId(), VaultRarity.COMMON, level),
                j = 0; j < statues; ++j) {
        ...
        if (!cannotGetArtifact && config != null) {
            ...
            for (final ArtifactChanceModifier modifier : vault.getActiveModifiersFor(PlayerFilter.any(),
                    ArtifactChanceModifier.class)) {
    ...
    public static VaultObjective fromNBT(final CompoundNBT nbt) {
        final VaultObjective objective = ((Supplier) VaultObjective.REGISTRY
                .get((Object) new ResourceLocation(nbt.getString("Id")))).get();
    ...
    public static VaultLootableTileEntity.ExtendedGenerator getObjectiveBlock() {
        return new VaultLootableTileEntity.ExtendedGenerator() {
            ...
            public BlockState generate(final ServerWorld world, final BlockPos pos, final Random random,
                ...
                final VaultObjective objective = (VaultObjective) Iterables
                        .getFirst((Iterable) vault.getAllObjectives(), (Object) null);
                if (objective == null) {
    ...
    public static class Crate implements INBTSerializable<CompoundNBT> {
        ...
        public void deserializeNBT(final CompoundNBT nbt) {
            ...
            contentsList.stream().map(inbt -> (CompoundNBT) inbt)
                    .forEach(compoundNBT -> this.contents.add(ItemStack.of(compoundNBT)));
        }
    }
}
