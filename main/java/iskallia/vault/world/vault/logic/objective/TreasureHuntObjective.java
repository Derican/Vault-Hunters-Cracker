...
public class TreasureHuntObjective extends VaultObjective {
    ...
    public void tick(final VaultRaid vault, final PlayerFilter filter, final ServerWorld world) {
        ...
        vault.getPlayers().stream().map(vPlayer -> vPlayer.getServerPlayer(world.getServer()))
                ...
                .filter(piece -> piece instanceof VaultRoom).map(piece -> piece).forEach(room -> {
    ...
    public void spawnSand(final VaultRaid vault, final ServerWorld world, final VaultRoom room) {
        for (int i = 0; i < 200; ++i) {
            final int x = TreasureHuntObjective.rand
                    .nextInt(room.getBoundingBox().x1 - room.getBoundingBox().x0 + 1)
                    + room.getBoundingBox().x0;
            final int y = TreasureHuntObjective.rand
                    .nextInt(room.getBoundingBox().y1 - room.getBoundingBox().y0 + 1)
                    + room.getBoundingBox().y0;
            final int z = TreasureHuntObjective.rand
                    .nextInt(room.getBoundingBox().z1 - room.getBoundingBox().z0 + 1)
                    + room.getBoundingBox().z0;
            final BlockPos pos = new BlockPos(x, y, z);
            if (world.isAreaLoaded(pos, 1)) {
                final BlockState state = world.getBlockState(pos);
                if (state.isAir((IBlockReader) world, pos) && state.isAir((IBlockReader) world, pos.below())
                        && state.isAir((IBlockReader) world, pos.above())) {
                    final VaultSandEntity sand = VaultSandEntity.create((World) world, pos);
                    final ItemStack stack = sand.getItem();
                    stack.getOrCreateTag().putUUID("vault_id",
                            (UUID) vault.getProperties().getValue(VaultRaid.IDENTIFIER));
                    sand.setItem(stack);
    ...
    private void addRandomModifier(final VaultRaid vault, final ServerWorld sWorld, final ServerPlayerEntity player) {
        ...
        final Set<VaultModifier> modifiers = ModConfigs.VAULT_MODIFIERS.getRandom(TreasureHuntObjective.rand, level,
                VaultModifiersConfig.ModifierPoolType.FINAL_WENDARR_ADDS, null);
        ...
        if (modifier != null) {
            ...
            final ITextComponent c2 = (ITextComponent) new StringTextComponent(" deposited ")
                    .withStyle(TextFormatting.GRAY);
            final ITextComponent c3 = (ITextComponent) new StringTextComponent("sand")
    ...
    private ItemSubmission getNewEntry(final VaultRaid vault) {
        ...
        final ScavengerHuntConfig.ItemEntry newEntry = ModConfigs.TREASURE_HUNT
                .getRandomRequiredItem(currentItems::contains);
        if (newEntry == null) {
            return null;
        }
        final ScavengerHuntConfig.SourceType sourceType = ModConfigs.TREASURE_HUNT
    ...
    public static class Config {
        ...
        public Config(final IntSupplier requiredSubmissionsGen,
                final Function<Predicate<Item>, ScavengerHuntConfig.ItemEntry> itemGen,
                final Function<ItemStack, ScavengerHuntConfig.SourceType> sourceGen) {
