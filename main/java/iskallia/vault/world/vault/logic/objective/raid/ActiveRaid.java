...
public class ActiveRaid
{
    ...
    public static ActiveRaid create(final VaultRaid vault, final ServerWorld world, final BlockPos controller) {
        ...
        final VaultRoom room = vault.getGenerator().getPiecesAt(controller, VaultRoom.class).stream().findFirst().orElse(null);
        if (room == null) {
            return null;
        }
        ...
        world.getEntitiesOfClass((Class)PlayerEntity.class, raidBox).forEach(player -> raid.participatingPlayers.add(player.getUUID()));
        ...
        this.activeEntities.removeIf(entityUid -> {
            ...
            if (!(e instanceof MobEntity)) {
                ...
                if (!(mob.getTarget() instanceof PlayerEntity)) {
                    ...
                    final List<PlayerEntity> players = this.participatingPlayers.stream().map((Function<? super Object, ?>)world::getPlayerByUUID).filter(Objects::nonNull).filter(player -> this.raidBox.inflate(10.0).contains(player.position())).collect((Collector<? super Object, ?, List<PlayerEntity>>)Collectors.toList());
                    if (!players.isEmpty()) {
