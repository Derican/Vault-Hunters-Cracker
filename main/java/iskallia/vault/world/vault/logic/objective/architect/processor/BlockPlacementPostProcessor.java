...
public class BlockPlacementPostProcessor extends VaultPieceProcessor {
    ...
    public void postProcess(final VaultRaid vault, final ServerWorld world, final VaultPiece piece,
        ...
        final float size = (float) ((box.maxX - box.minX) * (box.maxY - box.minY)
                * (box.maxZ - box.minZ));
        ...
        while (runs > 0.0f && (runs >= 1.0f || BlockPlacementPostProcessor.rand.nextFloat() < runs)) {
            ...
            for (boolean placed = false; !placed; placed = true) {
                ...
                if (state.isAir((IBlockReader) world, pos) && world.getBlockState(pos.below()).isFaceSturdy(
                        (IBlockReader) world, pos, Direction.UP) && world.setBlock(pos, this.toPlace, 2)) {
                }
            }
        }
    }
