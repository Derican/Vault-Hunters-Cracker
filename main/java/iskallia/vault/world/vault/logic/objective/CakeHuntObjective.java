...
public class CakeHuntObjective extends VaultObjective
{
    ...
    public void expandVault(final ServerWorld world, final ServerPlayerEntity player, final BlockPos cakePos, final VaultRaid vault) {
        vault.getGenerator().getPiecesAt(cakePos, VaultRoom.class).stream().findAny().ifPresent(room -> vault.getProperties().getBase(VaultRaid.START_FACING).ifPresent(vaultDir -> {
            ...
            if (this.cakeCount < this.maxCakeCount) {
                ...
                final List<VaultPiece> generatedPieces = VaultJigsawHelper.expandVault(vault, world, room, direction, VaultJigsawHelper.getRandomPiece(this.roomPool, level), VaultJigsawHelper.getRandomPiece(this.tunnelPool, level));
    ...
    private void addRandomModifier(final VaultRaid vault, final ServerWorld sWorld, final ServerPlayerEntity player) {
        if (this.healthPenalty != 0.0f) {
            vault.getPlayers().stream().map(p -> p.getServerPlayer(sWorld.getServer())).filter(Optional::isPresent).map((Function<? super Object, ?>)Optional::get).forEach(other -> {
    ...
    private void spawnRewards(final ServerWorld world, final VaultRaid vault) {
        final VaultPlayer rewardPlayer = vault.getProperties().getBase(VaultRaid.HOST).flatMap((Function<? super UUID, ? extends Optional<? extends VaultPlayer>>)vault::getPlayer).filter(vPlayer -> vPlayer instanceof VaultRunner).orElseGet(() -> vault.getPlayers().stream().filter(vPlayer -> vPlayer instanceof VaultRunner).findAny().orElse(null));
