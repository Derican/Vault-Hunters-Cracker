...
public class SummonAndKillBossObjective extends VaultObjective
{
    ...
    public void spawnBossLoot(final VaultRaid vault, final VaultPlayer player, final ServerWorld world) {
        player.runIfPresent(world.getServer(), playerEntity -> {
            final LootContext.Builder builder = new LootContext.Builder(world).withRandom(world.random).withParameter(LootParameters.THIS_ENTITY, (Object)playerEntity).withParameter(LootParameters.ORIGIN, (Object)this.getBossPos()).withParameter(LootParameters.DAMAGE_SOURCE, (Object)DamageSource.playerAttack((PlayerEntity)playerEntity)).withOptionalParameter(LootParameters.KILLER_ENTITY, (Object)playerEntity).withOptionalParameter(LootParameters.DIRECT_KILLER_ENTITY, (Object)playerEntity).withParameter(LootParameters.LAST_DAMAGE_PLAYER, (Object)playerEntity).withLuck(playerEntity.getLuck());
    ...
    public static void onBossDeath(final LivingDeathEvent event) {
        ...
        final List<SummonAndKillBossObjective> matchingObjectives = vault.getPlayers().stream().map(player -> player.getActiveObjective(SummonAndKillBossObjective.class)).filter(Optional::isPresent).map((Function<? super Object, ?>)Optional::get).filter(o -> !o.isCompleted()).filter(SummonAndKillBossObjective::allObelisksClicked).filter(o -> o.getBossId().equals(event.getEntity().getUUID())).collect((Collector<? super Object, ?, List<SummonAndKillBossObjective>>)Collectors.toList());
    ...
    public static boolean isBossInVault(final VaultRaid vault, final LivingEntity entity) {
        final List<SummonAndKillBossObjective> matchingObjectives = vault.getPlayers().stream().map(player -> player.getActiveObjective(SummonAndKillBossObjective.class)).filter(Optional::isPresent).map((Function<? super Object, ?>)Optional::get).filter(o -> !o.isCompleted()).filter(SummonAndKillBossObjective::allObelisksClicked).filter(o -> o.getBossId().equals(entity.getUUID())).collect((Collector<? super Object, ?, List<SummonAndKillBossObjective>>)Collectors.toList());
