...
public class AncientObjective extends VaultObjective {
    ...
    public BlockState getObjectiveRelevantBlock(final VaultRaid vault, final ServerWorld world, final BlockPos pos) {
        return (BlockState) ModBlocks.CRYO_CHAMBER.defaultBlockState().setValue(
                (Property) CryoChamberBlock.CHAMBER_STATE, (Comparable) CryoChamberBlock.ChamberState.RUSTY);
    }
    ...
    public void notifyBail(final VaultRaid vault, final VaultPlayer player, final ServerWorld world) {
        if (!this.foundEternals.isEmpty()) {
            ...
            player.runIfPresent(world.getServer(), sPlayer -> {
                ...
                ct.append((ITextComponent) sPlayer.getDisplayName().copy()
                        .setStyle(Style.EMPTY.withColor(Color.fromRgb(9974168))));
                ct.append(" rescued ");
                for (int i = 0; i < this.foundEternals.size(); ++i) {
                    if (i != 0) {
                        ct.append(", ");
                    }
                    ct.append((ITextComponent) new StringTextComponent((String) this.foundEternals.get(i))
    ...
    protected void addSpecialLoot(final ServerWorld world, final VaultRaid vault, final LootContext context,
        ...
        final int level = vault.getProperties().getBase(VaultRaid.LEVEL).orElse(0);
        final LootTablesConfig.Level config = ModConfigs.LOOT_TABLES.getForLevel(level);
        final LootTable ancientTable = world.getServer().getLootTables()
                .get(config.getAncientEternalBonusBox());
        for (final String eternalName : this.foundEternals) {
            ...
            box.setHoverName(
                    (ITextComponent) new StringTextComponent(eternalName).withStyle(TextFormatting.GOLD));
    ...
    public static void onBreak(final BlockEvent.BreakEvent event) {
        ...
        final TileEntity te = CryoChamberBlock.getCryoChamberTileEntity((World) sWorld, event.getPos(),
                event.getState());
        if (!(te instanceof AncientCryoChamberTileEntity)) {
            return;
        }
    ...
    public void tick(final VaultRaid vault, final PlayerFilter filter, final ServerWorld world) {
        ...
        vault.getPlayers().stream().filter(vPlayer -> filter.test(vPlayer.getPlayerId()))
                .forEach(vPlayer -> vPlayer.runIfPresent(srv, playerEntity -> {
                    final VaultGoalMessage pkt = VaultGoalMessage.ancientsHunt(this.generatedEternals.size(),
                            this.foundEternals.size());
    ...
    public void deserializeNBT(final CompoundNBT tag) {
        ...
        for (int j = 0; j < placed.size(); ++j) {
            ...
            this.placedEternals
                    .add(new BlockPos(posTag.getInt("x"), posTag.getInt("y"), posTag.getInt("z")));
        }
