...
public class RaidEventListener {
        ...
        if (entity instanceof ServerPlayerEntity) {
            ...
            if (vault != null) {
                ...
                if (raid != null && raid.isPlayerInRaid((PlayerEntity) sPlayer)) {
                    ...
                    dmg *= (float) (1.0 + vault.getActiveObjective(RaidChallengeObjective.class)
                            .map(raidObjective -> raidObjective.getModifiersOfType(DamageTakenModifier.class).values()
    ...
    private static boolean isWhitelistedItem(final ItemStack interacted) {
        ...
        return (key.getNamespace().equals("dankstorage") && key.getPath().startsWith("dank_"))
                || (key.toString().equals("quark:pickarang") || key.toString().equals("quark:flamerang"))
                || key.getNamespace().equals("simplybackpacks");
    }
    ...
    private static boolean isInLockedRaidRoom(final IWorld world, final BlockPos pos) {
        ...
        final VaultRaidRoom room = vault.getGenerator().getPiecesAt(pos, VaultRaidRoom.class).stream().findFirst()
                .orElse(null);
