...
public abstract class VaultRoomLayoutGenerator implements JigsawPoolProvider {
    ...
    public static class Room {
        ...
        public BlockPos getRoomOffset() {
            return new BlockPos(
                    this.getRoomPosition().getX() * 47 + this.getRoomPosition().getX() * 48, 0,
                    this.getRoomPosition().getZ() * 47 + this.getRoomPosition().getZ() * 48);
        }

        public BlockPos getAbsoluteOffset(final Rotation vaultRotation, final Rotation roomRotation) {
            return this.getRoomOffset().rotate(vaultRotation)
    ...
    public static class Tunnel {
        ...
        public Rotation getRandomConnectingRotation(final Random random) {
            if (this.getFrom().getRoomPosition().getX()
                    - this.getTo().getRoomPosition().getX() == 0) {
        ...
        public BlockPos getAbsoluteOffset(final Rotation vaultRotation, final Rotation tunnelRotation) {
            ...
            final Vector3i dir = new Vector3i(to.getX() - from.getX(), 0,
                    to.getZ() - from.getZ());
            BlockPos relativeOffset = this.getFrom().getRoomOffset().offset(dir.getX() * 47, 0,
                    dir.getZ() * 47);
            if (dir.getX() < 0) {
