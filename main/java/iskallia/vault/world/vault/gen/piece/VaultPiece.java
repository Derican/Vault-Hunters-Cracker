...
public abstract class VaultPiece implements INBTSerializable<CompoundNBT> {
    ...
    protected VaultPiece(final ResourceLocation id, final ResourceLocation template,
    ...
    public BlockPos getMin() {
        return new BlockPos(this.boundingBox.x0, this.boundingBox.y0,
                this.boundingBox.z0);
    }

    public BlockPos getMax() {
        return new BlockPos(this.boundingBox.x1, this.boundingBox.y1,
                this.boundingBox.z1);
    }
    ...
    public static List<VaultPiece> of(final JigsawPiece jigsaw, final MutableBoundingBox box, final Rotation rotation) {
        ...
        return elements.stream().map(element -> {
            ...
            if (path.startsWith("vault/prefab/decor/generic/obelisk")) {
                ...
                } else if (path.startsWith("vault/enigma/starts") || path.startsWith("architect_event/enigma/starts")
                        || path.startsWith("raid/enigma/starts") || path.startsWith("trove/enigma/starts")) {
                ...
                } else if (path.startsWith("final_vault/velara/rooms") || path.startsWith("final_vault/tenos/rooms")
                        || path.startsWith("final_vault/wendarr/rooms")) {
                    return (FinalVaultBoss) new VaultRoom(template, box, rotation);
                } else if (path.startsWith("final_vault/velara/tunnels") || path.startsWith("final_vault/tenos/tunnels")
