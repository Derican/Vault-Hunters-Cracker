...
public class VaultInfluenceRegistry {
    ...
    public static void init() {
        ...
        Arrays.stream(VaultAttributeInfluence.Type.values())
                .forEach(type -> register(VaultAttributeInfluence.newInstance(type)));
    }

    public static Optional<VaultInfluence> getInfluence(final ResourceLocation key) {
        return Optional.ofNullable(VaultInfluenceRegistry.influences.get(key))
                .map((Function<? super Supplier<VaultInfluence>, ? extends VaultInfluence>) Supplier::get);
    }
