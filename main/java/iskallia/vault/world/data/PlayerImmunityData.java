...
public class PlayerImmunityData extends WorldSavedData
{
    ...
    public void addEffects(final UUID playerUUID, final List<EffectInstance> effects) {
        this.effects.put(playerUUID, effects.stream().map((Function<? super Object, ?>)EffectInstance::getEffect).map((Function<? super Object, ?>)ForgeRegistryEntry::getRegistryName).filter(Objects::nonNull).map((Function<? super Object, ?>)ResourceLocation::toString).collect((Collector<? super Object, ?, List<String>>)Collectors.toList()));
        this.setDirty();
    }
    
    public List<Effect> getEffects(final UUID playerUUID) {
        return Registry.MOB_EFFECT.stream().filter(effect -> this.effects.get(playerUUID).contains(effect.getRegistryName().toString())).collect((Collector<? super Object, ?, List<Effect>>)Collectors.toList());
    }
