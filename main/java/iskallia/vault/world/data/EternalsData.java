...
public class EternalsData extends WorldSavedData
{
    ...
    public UUID getOwnerOf(final UUID eternalId) {
        return this.playerMap.entrySet().stream().filter(e -> e.getValue().containsEternal(eternalId)).map((Function<? super Object, ? extends UUID>)Map.Entry::getKey).findFirst().orElse(null);
    }
    ...
    public class EternalGroup implements INBTSerializable<CompoundNBT>
    {
        ...
        public EternalData getRandomAlive(final Random random, final Predicate<EternalData> eternalFilter) {
            final List<EternalData> aliveEternals = this.getEternals().stream().filter(EternalData::isAlive).filter((Predicate<? super Object>)eternalFilter).collect((Collector<? super Object, ?, List<EternalData>>)Collectors.toList());
        ...
        public EternalData getRandomAliveAncient(final Random random, final Predicate<EternalData> eternalFilter) {
            final List<EternalData> aliveEternals = this.getEternals().stream().filter(EternalData::isAlive).filter(EternalData::isAncient).filter((Predicate<? super Object>)eternalFilter).collect((Collector<? super Object, ?, List<EternalData>>)Collectors.toList());
            if (aliveEternals.isEmpty()) {
                return null;
            }
            return aliveEternals.get(random.nextInt(aliveEternals.size()));
