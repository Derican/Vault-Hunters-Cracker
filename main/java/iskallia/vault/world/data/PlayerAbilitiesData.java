...
public class PlayerAbilitiesData extends WorldSavedData {
    ...
    public PlayerAbilitiesData selectSpecialization(final ServerPlayerEntity player, final String ability,
            @Nullable final String specialization) {
        final AbilityTree abilityTree = this.getAbilities((PlayerEntity) player);
    ...
    public static void setAbilityOnCooldown(final ServerPlayerEntity player, final String abilityName) {
        ...
        abilities.putOnCooldown(player.getServer(), abilityNode,
                ModConfigs.ABILITIES.getCooldown(abilityNode, player));
    }
    ...
    public static PlayerAbilitiesData get(final MinecraftServer srv) {
        return (PlayerAbilitiesData) srv.overworld().getDataStorage()
                .computeIfAbsent((Supplier) PlayerAbilitiesData::new, "the_vault_PlayerAbilities");
    }
}
