...
public class PlayerFavourData extends WorldSavedData {
    ...
    public boolean addFavour(final PlayerEntity player, final VaultGodType type, final int count) {
        ...
        int favour = (int) this.favourStats.computeIfAbsent(playerUUID, key -> new HashMap())
                .getOrDefault((Object) type, (Object) 0);
    ...
    public static PlayerFavourData get(final ServerWorld world) {
        return (PlayerFavourData) world.getServer().overworld().getDataStorage()
                .computeIfAbsent((Supplier) PlayerFavourData::new, "the_vault_PlayerFavour");
    }
    ...
    public enum VaultGodType implements IStringSerializable {
        ...
        public ITextComponent getHoverChatComponent() {
            return (ITextComponent) new StringTextComponent("[Vault God] ").withStyle(TextFormatting.WHITE)
                    .append((ITextComponent) new StringTextComponent(this.name + ", " + this.title)
                            .withStyle(this.color));
        }

        public ITextComponent getIdolDescription() {
            final String s = this.getName().endsWith("s") ? "" : "s";
            return (ITextComponent) new StringTextComponent(String.format("%s'%s Idol", this.getName(), s))
                    .withStyle(this.getChatColor());
        }
        ...
        public IFormattableTextComponent getChosenPrefix() {
            ...
            final String s = this.getName().endsWith("s") ? "" : "s";
            final IFormattableTextComponent hover = new StringTextComponent(
                    String.format("%s'%s Chosen", this.getName(), s)).withStyle(this.getChatColor());
            cmp.withStyle(
                    style -> style.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (Object) hover)));
            return cmp;
        }
