...
public class PalettedListPoolElement extends JigsawPiece {
    ...
    public PalettedListPoolElement(final List<JigsawPiece> elements, final JigsawPattern.PlacementBehaviour behaviour,
            final List<Supplier<StructureProcessorList>> processors) {
        super(behaviour);
    ...
    protected static <E extends JigsawPiece> RecordCodecBuilder<E, JigsawPattern.PlacementBehaviour> projection() {
        return (RecordCodecBuilder<E, JigsawPattern.PlacementBehaviour>) JigsawPattern.PlacementBehaviour.CODEC
                .fieldOf("projection").forGetter((Function) JigsawPiece::getProjection);
    }

    protected static <E extends PalettedListPoolElement> RecordCodecBuilder<E, List<Supplier<StructureProcessorList>>> processors() {
        return (RecordCodecBuilder<E, List<Supplier<StructureProcessorList>>>) IStructureProcessorType.LIST_CODEC
                .listOf().fieldOf("processors").forGetter(piece -> piece.processors);
    }
    ...
    public List<Template.BlockInfo> getShuffledJigsawBlocks(final TemplateManager templateManager, final BlockPos pos,
        ...
        return this.elements.isEmpty() ? new ArrayList<Template.BlockInfo>()
    ...
    public MutableBoundingBox getBoundingBox(final TemplateManager templateManager, final BlockPos pos,
        ...
        this.elements.stream().map(piece -> piece.getBoundingBox(templateManager, pos, rotation))
                .forEach((Consumer<? super Object>) mutableboundingbox::expand);
    ...
    public boolean place(final TemplateManager templateManager, final ISeedReader world,
        ...
        return this.generate(templateManager, world, structureManager, chunkGen, pos1, pos2, rotation, box, random,
                keepJigsaws, 18);
    }

    public boolean generate(final TemplateManager templateManager, final ISeedReader world,
            final StructureManager structureManager, final ChunkGenerator chunkGen, final BlockPos pos1,
            final BlockPos pos2, final Rotation rotation, final MutableBoundingBox box, final Random random,
            final boolean keepJigsaws, final int updateFlags) {
        ...
        for (final JigsawPiece piece : this.elements) {
            if (piece instanceof PalettedSinglePoolElement) {
                if (!((PalettedSinglePoolElement) piece).generate(extra, templateManager, world, structureManager,
                        chunkGen, pos1, pos2, rotation, box, random, keepJigsaws, updateFlags)) {
                    return false;
                }
                continue;
            } else {
    ...
    public String toString() {
        return "PalettedList[" + this.elements.stream().map((Function<? super Object, ?>) Object::toString)
    ...
    static {
        CODEC = RecordCodecBuilder.create(instance -> instance
                .group((App) JigsawPiece.CODEC.listOf().fieldOf("elements")
                        .forGetter(piece -> piece.elements), (App) projection(), (App) processors())
                .apply((Applicative) instance, PalettedListPoolElement::new));
    }
}
