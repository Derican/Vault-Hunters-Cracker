...
public class JigsawPieceResolver {
    ...
    public List<AbstractVillagePiece> resolveJigsawPieces(final TemplateManager templateManager,
            final Registry<JigsawPattern> jigsawPatternRegistry) {
        final AbstractVillagePiece beginningPiece = new AbstractVillagePiece(templateManager, this.piece, this.pos,
                this.piece.getGroundLevelDelta(), this.pieceRotation,
        ...
        VoxelShape generationShape = VoxelShapes
                .create(AxisAlignedBB.of(pieceBox).inflate(15.0));
        for (final AxisAlignedBB additionalBoxes : this.additionalStructureBoxes) {
            generationShape = VoxelShapes.join(generationShape, VoxelShapes.create(additionalBoxes),
                    IBooleanFunction.ONLY_FIRST);
        }
        final MutableObject<VoxelShape> generationBoxRef = (MutableObject<VoxelShape>) new MutableObject(
                (Object) generationShape);
        ...
        while (!generationEntries.isEmpty()) {
            ...
            this.calculatePieces(resolvedPieces, generationEntries, generationEntry.villagePiece,
                    generationEntry.generationBox, templateManager, jigsawPatternRegistry);
        }
        return resolvedPieces;
    }

    private void calculatePieces(final List<AbstractVillagePiece> resolvedPieces, final List<Entry> generationEntries,
            final AbstractVillagePiece piece, final MutableObject<VoxelShape> generationBox,
            final TemplateManager templateMgr, final Registry<JigsawPattern> jigsawPatternRegistry) {
        ...
        Label_0086: while (true) {
            for (final Template.BlockInfo blockInfo : thisPieceBlocks) {
                ...
                final ResourceLocation connectorPool = new ResourceLocation(
                        blockInfo.nbt.getString("pool"));
                final Optional<JigsawPattern> mainJigsawPattern = jigsawPatternRegistry.getOptional(connectorPool);
                if (!mainJigsawPattern.isPresent() || (mainJigsawPattern.get().size() == 0
                        && !Objects.equals(connectorPool, JigsawPatternRegistry.EMPTY.location()))) {
                    ...
                    final Optional<JigsawPattern> fallbackJigsawPattern = jigsawPatternRegistry
                            .getOptional(fallbackConnectorPool);
                    if (!fallbackJigsawPattern.isPresent() || (fallbackJigsawPattern.get().size() == 0
                            && !Objects.equals(fallbackConnectorPool,
                                    JigsawPatternRegistry.EMPTY.location()))) {
                        ...
                        if (pieceBox.isInside((Vector3i) expectedConnectionPos)) {
                            ...
                            if (thisPieceGenerationBox.getValue() == null) {
                                thisPieceGenerationBox.setValue(
                                        (Object) VoxelShapes.create(AxisAlignedBB.of(pieceBox)));
                            }
                        } else {
                            nextGenerationBox = generationBox;
                        }
                        final WeightedList<JigsawPiece> weightedPieces = new WeightedList<JigsawPiece>();
                        if (!connectorPool.equals((Object) new ResourceLocation("empty"))
                                && this.filter.test(connectorPool)) {
                        ...
                        if (!fallbackConnectorPool.equals((Object) new ResourceLocation("empty"))
                                && this.filter.test(fallbackConnectorPool)) {
                        ...
                        while (!weightedPieces.isEmpty()) {
                            ...
                            for (final Rotation nextPieceRotation : Rotation.getShuffled(JigsawPieceResolver.rand)) {
                                ...
                                synchronized (JigsawPieceResolver.templateLoadLock) {
                                    nextPieceBlocks = nextPiece.getShuffledJigsawBlocks(templateMgr, BlockPos.ZERO,
                                            nextPieceRotation, JigsawPieceResolver.rand);
                                }
                                for (final Template.BlockInfo nextPieceBlockInfo : nextPieceBlocks) {
                                    ...
                                    final BlockPos pieceDiff = new BlockPos(
                                    ...
                                    final MutableBoundingBox nextPieceBox = nextPiece.getBoundingBox(templateMgr,
                                            pieceDiff, nextPieceRotation);
                                    final boolean isNextPieceRigid = nextPiece
                                            .getProjection() == JigsawPattern.PlacementBehaviour.RIGID;
                                    final int nextY = nextPiecePos.getY();
                                    final int l1 = jigsawYPos - nextY + JigsawBlock
                                            .getFrontFacing(nextPieceBlockInfo.state).getStepY();
                                    if (VaultPiece.shouldIgnoreCollision(nextPiece) || !VoxelShapes.joinIsNotEmpty(
                                            (VoxelShape) nextGenerationBox.getValue(),
                                            VoxelShapes.create(
                                                    AxisAlignedBB.of(nextPieceBox).deflate(0.25)),
                                            IBooleanFunction.ONLY_SECOND)) {
                                        nextGenerationBox.setValue((Object) VoxelShapes.joinUnoptimized(
                                                (VoxelShape) nextGenerationBox.getValue(),
                                                VoxelShapes.create(AxisAlignedBB.of(nextPieceBox)),
                                                IBooleanFunction.ONLY_FIRST));
                                        int l2;
                                        if (isNextPieceRigid) {
                                        ...
                                        final AbstractVillagePiece nextPieceVillagePiece = new AbstractVillagePiece(
                                                templateMgr, nextPiece, pieceDiff, l2, nextPieceRotation, nextPieceBox);
                                        resolvedPieces.add(nextPieceVillagePiece);
                                        generationEntries.add(
                                                new Entry(nextPieceVillagePiece, (MutableObject) nextGenerationBox));
                                        continue Label_0086;
                                    }
                                }
                            }
