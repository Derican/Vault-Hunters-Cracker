...
public class JigsawPiecePlacer {
    ...
    private JigsawPiecePlacer(final JigsawPiece piece, final ServerWorld world, final BlockPos pos) {
        ...
        this.jigsawPatternRegistry = (Registry<JigsawPattern>) world.getServer().registryAccess()
    ...
    public List<VaultPiece> placeJigsaw() {
        final List<AbstractVillagePiece> resolvedPieces = this.resolver.resolveJigsawPieces(this.templateManager,
                this.jigsawPatternRegistry);
        resolvedPieces.forEach(this::placeStructurePiece);
        return resolvedPieces.stream().flatMap(piece -> VaultPiece.of((StructurePiece) piece).stream())
    ...
    private void placeStructurePiece(final AbstractVillagePiece structurePiece) {
        ...
        final BlockPos generationPos = new BlockPos(center.getX(), structureBox.y0,
                center.getZ());
        final JigsawPiece toGenerate = structurePiece.getElement();
        try {
            ++JigsawPiecePlacer.generationPlacementCount;
            this.placeJigsawPiece(toGenerate, structurePiece.getPosition(), generationPos,
                    structurePiece.getRotation(), structureBox);
        } finally {
            --JigsawPiecePlacer.generationPlacementCount;
        }
    }

    private void placeJigsawPiece(final JigsawPiece jigsawPiece, final BlockPos seedPos, final BlockPos generationPos,
            final Rotation pieceRotation, final MutableBoundingBox pieceBox) {
        if (jigsawPiece instanceof PalettedListPoolElement) {
            ((PalettedListPoolElement) jigsawPiece).generate(this.templateManager, (ISeedReader) this.world,
                    this.structureManager, this.chunkGenerator, seedPos, generationPos, pieceRotation, pieceBox,
                    JigsawPiecePlacer.rand, false, 18);
        } else if (jigsawPiece instanceof PalettedSinglePoolElement) {
            ((PalettedSinglePoolElement) jigsawPiece).generate(null, this.templateManager, (ISeedReader) this.world,
                    this.structureManager, this.chunkGenerator, seedPos, generationPos, pieceRotation, pieceBox,
                    JigsawPiecePlacer.rand, false, 18);
        } else {
            jigsawPiece.place(this.templateManager, (ISeedReader) this.world, this.structureManager,
                    this.chunkGenerator, seedPos, generationPos, pieceRotation, pieceBox, JigsawPiecePlacer.rand,
                    false);
        }
    }
