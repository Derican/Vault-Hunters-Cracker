...
public class AnvilEvents {
    ...
    public static void onVaultAnvil(final AnvilUpdateEvent event) {
        ...
        if (repairItem == ModItems.MAGNETITE || repairItem == ModItems.REPAIR_CORE
                || repairItem == ModItems.REPAIR_CORE_T2) {
            return;
        }
    ...
    public static void onCombineVaultGear(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultGear
                && event.getRight().getItem() instanceof VaultGear) {
            event.setCanceled(true);
        }
    }
    ...
    public static void onApplySeal(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() instanceof ItemVaultCrystalSeal) {
            ...
            VaultRaid.init();
            final ResourceLocation objectiveKey = ((ItemVaultCrystalSeal) event.getRight().getItem())
                    .getObjectiveId();
    ...
    public static void onApplyCake(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() == Items.CAKE) {
            final ItemStack output = event.getLeft().copy();
            final CrystalData data = VaultCrystalItem.getData(output);
            if (!data.getModifiers().isEmpty()
                    || (data.getSelectedObjective() != null && data.getType() == CrystalData.Type.COOP)) {
                return;
            }
            VaultRaid.init();
    ...
    public static void onApplyRaffleSeal(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() instanceof ItemVaultRaffleSeal) {
            final ItemStack output = event.getLeft().copy();
            final CrystalData data = VaultCrystalItem.getData(output);
            if (!data.getModifiers().isEmpty() || data.getSelectedObjective() != null) {
                return;
    ...
    public static void onApplyCatalyst(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() instanceof VaultCatalystItem) {
            final ItemStack output = event.getLeft().copy();
            final CrystalData data = VaultCrystalItem.getData(output);
            if (!data.canModifyWithCrafting()) {
                return;
            }
            final List<String> modifiers = VaultCatalystItem.getCrystalCombinationModifiers(event.getRight(),
                    event.getLeft());
    ...
    public static void onApplyRune(final AnvilUpdateEvent event) {
        ...
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() instanceof VaultRuneItem) {
            final ServerPlayerEntity sPlayer = (ServerPlayerEntity) event.getPlayer();
            final int level = PlayerVaultStatsData.get(sPlayer.getLevel()).getVaultStats((PlayerEntity) sPlayer)
                    .getVaultLevel();
    ...
    public static void onCraftInhibitor(final AnvilUpdateEvent event) {
        ...
        final List<ModifierRollResult> modifiers = VaultCatalystItem.getModifierRolls(catalyst)
    ...
    public static void onApplyPainiteStar(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() == ModItems.PAINITE_STAR) {
            final ItemStack output = event.getLeft().copy();
    ...
    public static void onApplyInhibitor(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() instanceof VaultInhibitorItem) {
            final ItemStack output = event.getLeft().copy();
            final CrystalData data = VaultCrystalItem.getData(output);
            if (!data.canModifyWithCrafting()) {
                return;
            ...
            final List<String> inhibitorModifiers = VaultInhibitorItem.getCrystalCombinationModifiers(event.getRight(),
                    event.getLeft());
            if (crystalModifiers.isEmpty() || inhibitorModifiers == null || inhibitorModifiers.isEmpty()) {
                return;
            }
            inhibitorModifiers.forEach(
                    modifier -> data.removeCatalystModifier(modifier, true, CrystalData.Modifier.Operation.ADD));
    ...
    public static void onApplyEchoGemToCrystal(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() == ModItems.ECHO_GEM) {
            final ItemStack output = event.getLeft().copy();
            final CrystalData data = VaultCrystalItem.getData(output);
    ...
    public static void onApplyEchoCrystal(final AnvilUpdateEvent event) {
        ...
        final boolean success = MathUtilities.randomFloat(0.0f, 1.0f) < echoCrystalData.getEchoData()
                .getCloneSuccessRate();
    ...
    public static void onRepairDeny(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultPaxelItem
                && event.getRight().getItem() instanceof VaultPaxelItem) {
            event.setCanceled(true);
        }
        if (event.getLeft().getItem() instanceof VaultMagnetItem
                && event.getRight().getItem() instanceof VaultMagnetItem) {
            event.setCanceled(true);
        }
    }
    ...
    public static void onApplySoulFlame(final AnvilUpdateEvent event) {
        if (event.getLeft().getItem() instanceof VaultCrystalItem
                && event.getRight().getItem() == ModItems.SOUL_FLAME) {
            final ItemStack output = event.getLeft().copy();
            final CrystalData data = VaultCrystalItem.getData(output);
