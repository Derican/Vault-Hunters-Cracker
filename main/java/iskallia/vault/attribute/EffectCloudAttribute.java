...
public class EffectCloudAttribute extends PooledAttribute<List<EffectCloudEntity.Config>>
{
    ...
    public void write(final CompoundNBT nbt) {
        ...
        ((VAttribute.Instance<List>)this).getBaseValue().forEach(effect -> effectsList.add((Object)effect.serializeNBT()));
    ...
    public void read(final CompoundNBT nbt) {
        if (!nbt.contains("BaseValue", 10)) {
            ((VAttribute.Instance<ArrayList<EffectCloudEntity.Config>>)this).setBaseValue(new ArrayList<EffectCloudEntity.Config>());
            return;
        }
        final CompoundNBT tag = nbt.getCompound("BaseValue");
        final ListNBT effectsList = tag.getList("EffectClouds", 10);
        this.setBaseValue(effectsList.stream().map(inbt -> EffectCloudEntity.Config.fromNBT((CompoundNBT)inbt)).collect((Collector<? super Object, ?, List<EffectCloudEntity.Config>>)Collectors.toList()));
    }
