...
public class RegistryKeyAttribute<T> extends VAttribute.Instance<RegistryKey<T>> {
    ...
    public void read(final CompoundNBT nbt) {
        if (nbt.contains("BaseValue", 10)) {
            ...
            this.setBaseValue((RegistryKey<T>) RegistryKey.create(
                    RegistryKey.createRegistryKey(new ResourceLocation(valueNBT.getString("Parent"))),
                    new ResourceLocation(valueNBT.getString("Identifier"))));
        }
    }
}
