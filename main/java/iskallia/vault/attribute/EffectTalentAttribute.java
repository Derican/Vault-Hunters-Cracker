...
public class EffectTalentAttribute extends PooledAttribute<List<EffectTalent>>
{
    ...
    public void read(final CompoundNBT nbt) {
        ...
        this.setBaseValue(effectsList.stream().map(inbt -> (CompoundNBT)inbt).map(compoundNBT -> new EffectTalent(0, tag.getString("Id"), tag.getInt("Amplifier"), tag.getString("Type"), tag.getString("Operator"))).collect((Collector<? super Object, ?, List<EffectTalent>>)Collectors.toList()));
    }
