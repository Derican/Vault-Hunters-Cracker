...
public class FinalArchitectEventConfig extends Config
{
    ...
    public List<VoteModifier> getAll() {
        return Stream.of((List[])new List[] { this.KNOWLEDGE_MODIFIERS, this.MOB_HEALTH_MODIFIERS, this.VAULT_MODIFIER_MODIFIERS, this.VAULT_TIME_MODIFIERS }).flatMap((Function<? super List, ? extends Stream<?>>)Collection::stream).collect((Collector<? super Object, ?, List<VoteModifier>>)Collectors.toList());
    }
    ...
    protected void reset() {
        this.KNOWLEDGE_MODIFIERS = Arrays.asList(new FinalKnowledgeModifier("Knowledge1", "+3 Knowledge", 3), new FinalKnowledgeModifier("Knowledge2", "+5 Knowledge", 5));
        this.MOB_HEALTH_MODIFIERS = Arrays.asList(new FinalMobHealthModifier("MobHealth1", "+10% Mob Health", 0.1f), new FinalMobHealthModifier("MobHealth2", "+20% Mob Health", 0.2f));
        this.VAULT_MODIFIER_MODIFIERS = Arrays.asList(new FinalVaultModifierModifier("AddCrowded", "Add Crowded", "Crowded"));
        this.VAULT_TIME_MODIFIERS = Arrays.asList(new FinalVaultTimeModifier("AddMinute", "Adds 1 Minute", 60), new FinalVaultTimeModifier("RemoveMinute", "Removes 1 Minute", -60));
