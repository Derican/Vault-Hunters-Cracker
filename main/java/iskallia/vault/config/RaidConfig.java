...
public class RaidConfig extends Config
{
    ...
    public float getMobCountMultiplier(final int level) {
        return this.getAmountLevel(this.amountLevels, level).map((Function<? super AmountLevel, ? extends Float>)AmountLevel::getMobAmountMultiplier).orElse(1.0f);
    }
    ...
    protected void reset() {
        ...
        this.mobPools.put("ranged", Lists.newArrayList((Object[])new Level[] { new Level(0, new MobPool().add((EntityType<?>)EntityType.SKELETON, 1)), new Level(75, new MobPool().add((EntityType<?>)EntityType.SKELETON, 1).add((EntityType<?>)EntityType.STRAY, 1)) }));
        this.mobPools.put("melee", Lists.newArrayList((Object[])new Level[] { new Level(0, new MobPool().add((EntityType<?>)EntityType.ZOMBIE, 1)), new Level(50, new MobPool().add((EntityType<?>)EntityType.ZOMBIE, 2).add((EntityType<?>)EntityType.VINDICATOR, 1)) }));
